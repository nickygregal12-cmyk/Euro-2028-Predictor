...
- [ ] **Owner decision: does a round bound the time it is played, or do rounds tile the calendar?** ADR 0020 §79 says a rescheduled fixture moves to "the round its new kickoff falls in" and never defines *falls in*. Contract 113 implements the first reading because it invents nothing, and the consequence is that a midweek reschedule between matchweeks resolves to no round and is refused. Tiling would always resolve but requires choosing where a boundary sits between rounds, which moves a fixture into a round it was never played in. ADR 0020 §201 calls reassignment lock-critical, so this is not a detail to settle in an implementation.
...
- [ ] First reversible product slice, **data half**: the bounded season RPCs (read matchweek card; version-safe prediction, Joker and card-status writes). `season_fixtures`, `season_predictions` and `season_matchweek_jokers` revoke every browser grant by design, so the page's gateway is fixture-backed until these land through the normal migration/parity/rollout gates. MSW scenarios arrive with the RPCs they would intercept; the production route registration (route table, titles, redirects, axe matrix) lands with the data path, because a route that can only render "unavailable" serves nobody.
