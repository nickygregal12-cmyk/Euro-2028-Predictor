...
- [ ] The twelve Innovation Lab rows that did **not** ship. They are registered as `INNOV-*` rows in [`docs/quality/accepted-requirements.md`](docs/quality/accepted-requirements.md) with their dependencies, and **none of them has a disabled control or an empty shell in the product** — where the authority does not exist, the surface does not exist. `INNOV-020` (offline drafting) is the one that is **deferred rather than blocked**, and contract 177 has since supplied its server half, so it is easier rather than harder: it needs its own change with its own test suite, because a draft that reads as submitted is worse than no offline support.
