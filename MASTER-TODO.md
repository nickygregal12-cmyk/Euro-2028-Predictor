...
- [x] Supply the one fact the route registration was missing: **which matchweek the card opens at**. Contract 120's `get_season_play_matchweek` exposes `predictor_internal.next_eligible_league_round` — contract 109's authority, ordering by derived lock instant so a rescheduled season resolves correctly — rather than adding a second answer. The season id, name, season label and timezone needed no migration at all: they are columns on `public.tournaments`, which is already readable by `authenticated`. *(Repository-only; pgTAP `172_season_play_matchweek_read.sql` first runs in CI, and hosted application is the ordinary guarded fast lane.)*
- [ ] Register the production season Match Predictor route on top of it: route table, titles, redirects, axe matrix, MSW scenarios against the now-real network boundary, the end-of-season state where there is no next matchweek, and the flag flip. The fixtures behind it remain the invented development seed, not real football.
