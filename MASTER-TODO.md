...
- [ ] Close the three the same guard now names as still missing: **refreshing**, **stale** and the **blocking error** page. The first two need treatments no component owns yet (in-place progress; a freshness label decided together with the write restriction), and the third is a route-level surface rather than a component.
- [ ] Add the explicit phone and desktop width anchors the gallery still lacks, which the Playwright visual contracts will need.
