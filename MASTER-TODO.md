...
- [ ] Adopt the target foundations component by component, starting with the shared primitives (button, input, row, card, status, alert, modal, navigation) and the competition accent restrictions. Each adoption is its own reviewable change against the gallery. *(Part done: the type scale reached every stylesheet and the seven neutral tokens were repointed at the target ramp, so `--fs-*` is consumed by 64 files. Still unadopted, and measurable: the motion tokens in 2 files, `--numeric` in 5, the stacking scale in none — 8 hardcoded `z-index` values across 6 production stylesheets and roughly 44 raw pixel font-sizes in 11 files.)*
