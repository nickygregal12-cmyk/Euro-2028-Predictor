...
- [ ] **#863 / PROF-001** — same-season bounded player-profile visibility. The backend (contract 206, `get_season_player_profile_by_ref`) is **on this branch** since PR #920 was merged into it. Closure still requires the Development rollout, generated Supabase types and the vNext browser consumer — until that consumer exists, `buildLeaguesModel.destinationOf` still closes a `compare` row, which is correct against the database as deployed.
