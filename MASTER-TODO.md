...
- [ ] **Reconcile `scripts/seed-dev/seed-league-seasons.ts` with reality — it is now actively dangerous.** It still generates invented clubs and a synthetic round-robin, so re-running it against development would overwrite real football and cascade away every `provider_entity_map` row. It must stay committed, because CI and Browser E2E need a deterministic offline seed that never calls a provider. So it needs a guard that refuses to run over a season holding provider-mapped teams, and a name that says which world it belongs to.
...
- [ ] Register the production season Match Predictor route once contract 114 is applied to development and a real season serves it: route table, titles, redirects, axe matrix, MSW scenarios against the now-real network boundary, and the flag flip.
