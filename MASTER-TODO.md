...
- [x] Stage 14 — Football Hub production cutover. **Done and ON.** Nine destination flags in `netlify.toml`'s `[build.environment]`, `productionCutoverAuthorized: true` in [`config/vnext-programme.json`](config/vnext-programme.json), every legacy route element still mounted so any one destination rolls back on its own with no data rollback, and every hosted environment level at Contract 208 — the boundary these surfaces read across — so the reads exist wherever they run. Two contracts have landed in the repository since. Contract 209 is the provider fixture lifecycle; contract 210 widens the live poll window so a postponement is seen before the deadline it affects. Both hosted environments sit at contract 209, and contract 210 is not rolled out yet. Neither adds a read any vNext surface makes, so neither gates nor is gated by this cutover. [`docs/product/vnext-route-migration-matrix.md`](docs/product/vnext-route-migration-matrix.md) §13 owns the table.
