...
- [ ] Knip report-only baseline with every finding classified (production entry point / dev harness / reference prototype / historical evidence / confirmed dead code / requires investigation); no deletions in the setup PR.
- [ ] Visual foundations and component gallery in the production architecture: neutral ramp, surface levels, border ramp, radii, type scale, spacing, tabular numerals, shared primitives, competition accent restrictions, motion tokens and reduced-motion behaviour — proven in `ComponentsPreview`, not by copying `premium.css`.
