...
- [ ] **`CUP-001` — the qualification table for group sizes 5 to 20.** ADR 0028 § 6 authorises the design; `docs/predictor-cup-rules.md` § 6.2 currently covers sizes **3 and 4 only** while ADR 0014 caps a season group at twenty. It must reconcile § 6.1's `round(entrants × 2 ÷ 3)` target with § 6.3's wildcard ranking and § 7.1–7.3's bands, bracket arithmetic and avoidance, and be tested at **every** permitted size. **Do not write it into SQL alone** — the rules document is the authority.
