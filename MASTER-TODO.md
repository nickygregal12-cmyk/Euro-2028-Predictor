...
- [ ] Make the per-fixture lock the operative rule for a rescheduled fixture in the season path, with TypeScript/PostgreSQL parity. `fixtureReassignment.ts`'s destination model is superseded by the amendment and needs replacing rather than extending; its kickoff-revision and audit halves remain meaningful.
- [ ] Reconcile contract 113's round play window with the amendment. The window is still a true fact — it is when a round is played — but reassignment was its stated consumer and no longer asks the question. Either find it a justified use or record it as unused capability; do not leave the migration header claiming a purpose the amendment removed.
- [x] Make the database able to call the provider at all (contract 114, `20260805100000_provider_poll_dispatch.sql`). Measured rather than assumed: `pg_net` was available on development and `installed_version: null`, so PostgreSQL could make no outbound HTTP request — the deployed Edge Function had a scheduler that could not reach it. Installs the extension, revokes the whole `net` schema from `anon`, `authenticated` and `service_role` (an extension's functions are executable by PUBLIC by default, which would hand every authenticated session an arbitrary-URL fetch through the database), and drives `provider-poll` from `pg_cron` every five minutes at each target's declared cadence. Records no target and imports no fixture.
- [ ] Record the two `vault` secrets and at least one `provider_poll_targets` row, then observe a real dispatch. Until both exist the job returns `configured: false` / `due: 0`, which is deliberate — the alternative was a job failing loudly every five minutes until somebody finished the setup. This is the step that finally spends a credential, and it is the same step as "execute one bounded non-production request" below.
- [ ] Build the automatic fixture import and administrative reassignment workflow the Stage C1 foundation defers and ADR 0020 §Ingestion decides.
...
- [ ] Adopt the target foundations component by component, starting with the shared primitives (button, input, row, card, status, alert, modal, navigation) and the competition accent restrictions. Each adoption is its own reviewable change against the gallery.
- [ ] Close the three gallery states the first-state matrix names and the harness does not yet render: **offline, unavailable and conflict**. Loading/skeleton, empty, partial, locked, error, hostile long names and both themes are already covered across ~70 sections; the remaining gap is those three plus explicit phone and desktop widths, not a matrix built from scratch.
...
- [ ] Playwright visual screenshot contracts against stable `ComponentsPreview` anchors (fixed time and fixtures; light/dark, phone/desktop, reduced motion) with diff artefacts on failure. Baselines must be generated on the runner that will compare them, so this needs a CI-side bootstrap rather than container-generated PNGs.
- [ ] Promote the Lighthouse run to a CI job once performance drift between runners is measured and a floor can be set without false alarms.
- [ ] First reversible product slice: thin global/competition shells plus the phone-first season Match Predictor behind a route-level flag, meeting the §13.4 release gate (typed read model, explicit commands, MSW failure scenarios, old/new telemetry, immediate rollback).
- [ ] Continue the reconciled order: standings, LMS weekly selection, Championship surfaces, Hub action/social experience, public acquisition, then legacy route retirement by journey.
