...
- [x] **`UI-F07`, first slice — the gap to a rival.** A private league table now states how far the caller is behind the member above them and behind the leader, derived by subtraction over rows contract 128 already returned. Four cases where arithmetic and truth diverge each have a test. Rank/points MOVEMENT is deliberately absent and registered as `MIG-UI-03`.
...
- [x] **`UI-F11`, first half — the Match Centre gets an address.** `/competitions/:c/:s/matches/:fixtureId` renders the same panel the list opens. The link carries the fixture's day because contract 139's read is windowed by date; `MIG-UI-11` would remove the hint.
...
- [x] **`MIG-UI-12`, frontend half — the catalogue is discovered, not declared.** `fetchPublishedSeasons()` enumerates `tournaments` of `kind = 'league_season'`; `HUB_COMPETITIONS` is transitional presentation metadata supplying a route slug and copy; a published season it does not know is listed as unopenable rather than hidden. *(Guard: `tests/features/hub/serverDrivenCatalogue.test.ts`. Remaining backend gap: the route slug, and a publication state if it is to be a decision rather than a lifecycle.)*
...
- [x] **Rank context without rank movement.** `percentileLine` derives "Top 8%" from the server's rank and field size; it takes no previous rank, so it cannot drift into claiming movement, which stays `MIG-UI-03`.
...
- [ ] **Render the eight outstanding visual baselines.** The exact remaining action, which needs a GitHub runner and cannot be done from a development container: dispatch `.github/workflows/visual-contracts.yml` on branch `claude/predictor-ui-finalisation-mellse` with `update_baselines: true` and `commit_baselines: true`. That renders 32 new images (8 sections × 2 widths × 2 themes) on the runner that will later compare them and pushes them to the branch. The sections are already declared in `SECTIONS` and listed in `AWAITING_BASELINE` in the same spec — `desktop-navigation-rail`, `lms-form-guide-panel`, `fixture-consensus-panel`, the three `onboarding-*` sections, `create-a-league-journey` and `game-rules-disclosure`. `visualContractHarness.test.ts` excludes the awaiting list from its baseline count and **fails once an awaiting section has baselines on disk**, so the list has to be emptied in the same change that lands the images. All eight were reviewed by hand at 1440px and 390px in both themes.
- [ ] The private league workspace's **Matchweek** and **Members** tabs. Blocked on `MIG-UI-01`: both are league-wide named predictions, and a tab that opened onto "this needs a backend read" would be a dead control.
- [ ] The retention pillar: Matchweek Recap, rank context and percentile, Rival Watch pinning, the reusable points explanation, streaks, share cards and Season Wrapped. Each is blocked on a named `MIG-UI-*` row or on the ones above it; see [`docs/design/ui-finalisation.md`](docs/design/ui-finalisation.md) § 4A for which.
...
- [ ] The rest of the owner's UI finalisation sequence, `UI-F03`–`UI-F18`, whose per-item state is held by [`docs/design/ui-finalisation.md`](docs/design/ui-finalisation.md) § 11 rather than duplicated here. Its backend gaps are `MIG-UI-01`–`MIG-UI-07` in [`docs/quality/accepted-requirements.md`](docs/quality/accepted-requirements.md) and belong to a separate backend session.
