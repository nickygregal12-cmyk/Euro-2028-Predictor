...
- [ ] **#863 / PROF-001** — same-season bounded player-profile visibility. Backend is in PR #920 (which claims contract 206 on its own branch; this branch's two Championship migrations are contract 206 and contract 207, and whichever lands second is renumbered by its position in the chain). Closure still needs the Development rollout, generated Supabase types and the vNext consumer. **Not on `main` today**, so `LeaguePlayer.destination` still closes a `compare` row.
