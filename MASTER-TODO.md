...
- [x] **`UI-F17` onboarding as components.** Four steps built, tested and gallery-covered, deliberately not wired into first sign-in until `MIG-UI-10` can persist and resume the answers. `Follow ≠ Join game ≠ Favourite` is enforced by the draft model rather than promised in copy.
...
- [ ] The rest of the retention pillar: Rival Watch **pinning** (`MIG-UI-09`, optional), streaks, milestones, share cards and permanent Season Wrapped/history (`MIG-UI-08`). Rank context, percentile, the reusable points explanation, the Matchweek Recap and rank movement are delivered; see [`docs/design/ui-finalisation.md`](docs/design/ui-finalisation.md) § 4A for the current position of each.
