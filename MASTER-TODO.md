...
- [ ] **Add the rail and the two game panels to the visual contract.** The exact remaining action, which needs a GitHub runner and cannot be done from a development container: add `desktop-navigation-rail`, `lms-form-guide-panel` and `fixture-consensus-panel` to `SECTIONS` in `e2e/visual-gallery.spec.ts`, then dispatch `.github/workflows/visual-contracts.yml` with `update_baselines: true` and `commit_baselines: true` on this branch. That renders 12 new images (3 sections × 2 widths × 2 themes) on the runner that will later compare them and pushes them to the branch; `visualContractHarness.test.ts` counts baselines against `SECTIONS`, so the two must land together or CI goes red. All three sections exist in the gallery and were reviewed by hand at 1440px in both themes.
