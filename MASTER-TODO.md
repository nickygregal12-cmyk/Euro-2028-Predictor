...
- [ ] Finish the two adoption remainders the guard now names: the three DEV harness stylesheets, and `ProgressBar`'s 300ms width transition, which is the only routine transition still timed from a literal.
...
- [x] Supply the one fact the route registration was missing: **which matchweek the card opens at**. Contract 120's `get_season_play_context` exposes `predictor_internal.next_eligible_league_round` — contract 109's authority, ordering by derived lock instant so a rescheduled season resolves correctly — rather than adding a second answer. The season id, name, season label and timezone needed no migration at all: they are columns on `public.tournaments`, which is already readable by `authenticated`. *(Repository-only; pgTAP `172_season_play_context_read.sql` first runs in CI, and hosted application is the ordinary guarded fast lane.)*
- [x] **Register the production season Match Predictor route** at `/competitions/:competitionSlug/:seasonSlug/main-predictor`. `SeasonMatchPredictorRoute` resolves the season through contract 120, composes the card gateway from the answer and shows the end-of-season state as a state rather than an error; the route table, Netlify status rule, route title, `season-predictor` telemetry category and the hub dashboard's game link all name it. With `VITE_UI_SEASON_MATCH_PREDICTOR` off it renders the not-found page and issues no read at all, because the journey this address had before the flag was *no address*. *(Guards: `tests/features/season/seasonPlayContextModel.test.ts`, `useSeasonPlayContext.test.tsx`, `SeasonMatchPredictorRoute.test.tsx`.)*
- [ ] **Flip `VITE_UI_SEASON_MATCH_PREDICTOR`** once contract 120 is applied to development. It is set nowhere today — including deploy previews — deliberately: the read the route depends on exists in no hosted database, so a preview with the flag on would render a failed load rather than a card. The fixtures behind it will still be the invented development seed, not real football.
- [ ] Scan the season Match Predictor route with axe. Deferred in `tests/app/axeRouteCoverage.test.ts` with both blockers named: the E2E dev server does not set the flag, and the seeded harness user has no season Match Predictor entry, so with the flag on the scan would photograph the entry refusal. Both are harness work.
- [x] **Fix the route-coverage blind spot the registration exposed.** Three guards — route titles, axe coverage and Netlify SPA status — each carried their own copy of `/<Route path="([^"]+)"/g`, which requires `<Route` and `path=` on one line. Prettier wraps long declarations, so every wrapped route was invisible to all three at once. `/competitions/:competitionSlug/:seasonSlug` had been declared since the hub landed and had **no Netlify redirect**: the dashboard was served with the catch-all's 404 while the SPA rendered it perfectly — the exact soft-status inversion `spaRoutingStatus` exists to catch, sitting inside its blind spot. The idiom is now read once in `tests/app/declaredRoutes.ts`, with positive controls proving it sees a wrapped declaration.
