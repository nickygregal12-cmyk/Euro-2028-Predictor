...
// checklist / computeHubStatus exactly: groups → best-thirds → bracket. Jokers are
...
export function submitBlockers(status: HubStatus): SubmitBlocker[] {
  ...
  if (!status.groups.complete) out.push({ label: 'Groups A–F', route: '/predict/groups/A' })
  if (status.thirdPlace.state !== 'settled')
    out.push({ label: 'Best third-placed teams', route: '/predict/third-place' })
