...
// "group complete" predicate and the A–F → thirds ordering are unit-testable.
 ...
 * Group F → best third-placed teams. Linear per the doc's one-flow model
 * (Groups A → F → Best thirds → …), NOT "next incomplete". Returns null for an
 * unknown letter.
 */
export function groupContinuation(letter: string): Continuation | null {
  const i = GROUP_LETTERS.indexOf(letter.toUpperCase() as (typeof GROUP_LETTERS)[number])
  if (i === -1) return null
  const next = GROUP_LETTERS[i + 1]
  if (next) return { label: `Next: Group ${next} →`, path: `/predict/groups/${next}` }
  return { label: 'Next: Best third-placed teams →', path: '/predict/third-place' }
}
