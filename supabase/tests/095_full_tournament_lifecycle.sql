...
begin
  v_group_index := ascii(substring(p_home_name from 6 for 1)) - ascii('A');
  v_home_slot := right(p_home_name, 1)::integer;
  v_away_slot := right(p_away_name, 1)::integer;
  v_low_slot := least(v_home_slot, v_away_slot);
  v_high_slot := greatest(v_home_slot, v_away_slot);
...
insert into public.profiles (id, display_name, welcomed_at) values
  ...
  ('21000000-0000-0000-0000-000000000003', 'Lifecycle Wrong', now())
...
from (values
  ...
  ('21000000-0000-0000-0000-000000000103'::uuid, 'Team F4', 'champion'),
  ('21000000-0000-0000-0000-000000000103'::uuid, 'Team E4', 'final'),
  ('21000000-0000-0000-0000-000000000103'::uuid, 'Team D4', 'sf'),
  ('21000000-0000-0000-0000-000000000103'::uuid, 'Team C4', 'sf'),
  ('21000000-0000-0000-0000-000000000103'::uuid, 'Team B4', 'qf'),
  ('21000000-0000-0000-0000-000000000103'::uuid, 'Team A4', 'qf'),
...
select lives_ok(
  ...
  'the result-only entry submits through the authoritative validator'
);
reset role;
...
select lives_ok(
  ...
  'the deliberately wrong entry submits through the authoritative validator'
);
reset role;
...
select is(
  (
    select count(*)
    from public.predicted_group_positions position
    where position.entry_id in (
      '21000000-0000-0000-0000-000000000101',
      '21000000-0000-0000-0000-000000000102',
      '21000000-0000-0000-0000-000000000103'
    )
  ...
  72::bigint,
  'submission materialises all 24 group positions for each entry'
);
...
select is(
  (
    ...
    where member.league_id = '21000000-0000-0000-0000-000000000201'
  ),
  3::bigint,
  ...
  begin
    ...
    loop
      ...
      perform public.confirm_match_result(
        ...
        null,
        null,
        'Lifecycle simulation group result'
      );
    end loop;
  end;
  $body$;
  $test$,
  'the first 35 group results confirm through the authoritative lifecycle'
);
...
select is(
  (
    ...
    where participants.team_id is null
  ),
  16::bigint,
  'the actual Round of 16 remains unresolved before the group stage completes'
);
...
select lives_ok(
  ...
  begin
    ...
    perform public.confirm_match_result(
      ...
      null,
      null,
      'Lifecycle simulation final group result'
    );
  end;
  $body$;
  $test$,
...
select is(
  ...
  16::bigint,
  'group completion populates all 16 Round-of-16 participant slots'
);

select is(
  (
    select count(distinct participants.team_id)
    from (
      select match.home_team_id as team_id
      from public.matches match
      where match.round = 'r16'
...
select is(
  (
    select jsonb_object_agg(
      match.match_ref,
      jsonb_build_array(home_team.name, away_team.name)
    )
    from public.matches match
    join public.teams home_team on home_team.id = match.home_team_id
    join public.teams away_team on away_team.id = match.away_team_id
  ...
  'the best-third allocation matches the authoritative CDEF mapping'
);
...
select is(
  (
    ...
    where history.tournament_id = (
      select id from public.tournaments where name = 'UEFA Euro 2028'
    )
      and history.matchday_key in ('MD1', 'MD2', 'MD3')
  ),
  9::bigint,
  'group completion captures three rank checkpoints for three submitted users'
);
...
select ok(
  (
    select exact.total_points > result_only.total_points
    from public.entry_totals exact
    join public.entry_totals result_only
      on result_only.entry_id = '21000000-0000-0000-0000-000000000102'
    where exact.entry_id = '21000000-0000-0000-0000-000000000101'
  ),
  'the exact-score entry leads the result-only entry after the groups'
);

select ok(
  (
    select result_only.total_points > wrong.total_points
    from public.entry_totals result_only
    join public.entry_totals wrong
      on wrong.entry_id = '21000000-0000-0000-0000-000000000103'
    where result_only.entry_id = '21000000-0000-0000-0000-000000000102'
  ),
  'the result-only entry leads the deliberately wrong entry after the groups'
);
...
select is(
  pg_temp.capture_sqlstate($sql$
    update public.matches
      set home_team_id = (
        select id from public.teams where name = 'Team F4'
      )
...
select is(
  ...
  'Team A1',
  'the confirmed R16-1 winner propagates into QF-1'
);
...
select is(
  pg_temp.capture_sqlstate($sql$
    select public.correct_match_result(
      ...
      null,
      null,
      'Correct group winner after R16 started'
    )
  $sql$),
  '55000',
  'a group correction cannot silently rewrite a confirmed Round-of-16 fixture'
);
...
select is(
  ...
  'Team A1',
  'the blocked correction preserves the confirmed R16 participant'
);
...
select lives_ok(
  ...
  $sql$,
  'the affected Round-of-16 result clears before replay'
);
...
select lives_ok(
  $sql$
    select public.correct_match_result(
      ...
      null,
      null,
      'Official group result correction'
    )
  $sql$,
  'the group correction succeeds after the downstream result is cleared'
);
...
select is(
  (
    select away_team_id
    from public.matches
    where match_ref = 'QF-1'
  ),
  null::uuid,
  'clearing R16-1 also clears its propagated quarter-final participant'
);
...
select lives_ok(
  ...
  $body$;
  $test$,
  'all eight Round-of-16 results play through mixed result methods'
);
...
select is(
  ...
  8::bigint,
  'Round-of-16 winners populate every quarter-final slot'
);
  ...
  begin
    ...
    loop
      perform public.confirm_match_result(
        ...
        null,
        null,
        'Lifecycle quarter-final result'
      );
    end loop;
  end;
  $body$;
...
select is(
  (
    select count(*)
    from public.matches match
    where match.round in ('r16', 'qf', 'sf', 'final')
      and match.result_state in ('confirmed', 'corrected')
  ),
  15::bigint,
...
select is(
  ...
  21::bigint,
  'the complete tournament captures all seven rank checkpoints for three users'
);
...
where history.tournament_id = (
  ...
  and history.matchday_key = 'FINAL';
...
select lives_ok(
  $sql$
    select public.correct_match_result(
      ...
      4::smallint,
      5::smallint,
      'Official final winner correction'
    )
  $sql$,
  'the final can be corrected to a penalty-decided away winner'
);
...
select is(
  ...
  'Team E1',
  'the corrected penalty result changes the champion to Team E1'
);
...
select is(
  ...
  0::bigint,
  'champion scoring is removed from Team B1 after the correction'
);
...
select lives_ok(
  ...
  $sql$,
  'the final can be cleared for replay'
);
...
select is(
  ...
  'scheduled',
  'clearing the final restores its scheduled state'
);
...
select is(
  ...
  4::bigint,
  'confirm, correct, clear and replay remain in immutable final revision history'
);
...
select is(
  ...
  'later final correction and replay do not rewrite historical rank capture'
);
