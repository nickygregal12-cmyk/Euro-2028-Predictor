-- Contract 206: a postponement reaches the fixture, and a reschedule reaches it
-- back.
--
...
-- fixtures, mapped to football-data. Contract 206's rules are provider-neutral
...
values
  ...
  ('football-data', 'team', 'PFL-T1', current_setting('test.pfl_season')::uuid, null,
   (select id from public.teams where tournament_id = current_setting('test.pfl_season')::uuid
     and name = 'Lifecycle Rovers'), '252_provider_fixture_lifecycle.sql'),
  ('football-data', 'team', 'PFL-T2', current_setting('test.pfl_season')::uuid, null,
   (select id from public.teams where tournament_id = current_setting('test.pfl_season')::uuid
     and name = 'Lifecycle United'), '252_provider_fixture_lifecycle.sql'),
  ('football-data', 'team', 'PFL-T3', current_setting('test.pfl_season')::uuid, null,
   (select id from public.teams where tournament_id = current_setting('test.pfl_season')::uuid
     and name = 'Lifecycle Athletic'), '252_provider_fixture_lifecycle.sql'),
  ('football-data', 'team', 'PFL-T4', current_setting('test.pfl_season')::uuid, null,
   (select id from public.teams where tournament_id = current_setting('test.pfl_season')::uuid
     and name = 'Lifecycle Wanderers'), '252_provider_fixture_lifecycle.sql');
