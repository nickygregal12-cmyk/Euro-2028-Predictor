-- Contract 101: the Predictor Championship split is a distinct persisted phase.
...
select md5('c101-user-' || n)::uuid, format('c101-%s@example.test', n),
...
insert into public.bonus_competitions (
  ...
  md5('c101-comp')::uuid, current_setting('test.c101_tournament_id')::uuid,
...
select md5('c101-comp')::uuid, md5('c101-user-' || n)::uuid, now() - interval '2 days'
from generate_series(1, 7) n;

insert into public.bonus_competition_windows
  (id, competition_id, sequence, label, opens_at, locks_at)
values (
  md5('c101-window')::uuid, md5('c101-comp')::uuid, 34, 'Cup round 34',
  now() + interval '1 hour', now() + interval '1 day'
);

insert into public.bonus_cup_groups (id, competition_id, ordinal, size)
values (md5('c101-initial')::uuid, md5('c101-comp')::uuid, 1, 6);

insert into public.bonus_cup_groups
  (id, competition_id, ordinal, size, phase_kind, parent_group_id)
values
  (md5('c101-split-top')::uuid, md5('c101-comp')::uuid, 2, 3,
   'split', md5('c101-initial')::uuid),
  (md5('c101-split-bottom')::uuid, md5('c101-comp')::uuid, 3, 3,
   'split', md5('c101-initial')::uuid);

insert into public.bonus_cup_members
  (competition_id, user_id, group_id, draw_number)
select md5('c101-comp')::uuid, md5('c101-user-' || n)::uuid,
       md5('c101-initial')::uuid, n
from generate_series(1, 6) n;

insert into public.bonus_cup_members
  (competition_id, user_id, group_id, draw_number, phase_kind)
select md5('c101-comp')::uuid, md5('c101-user-' || n)::uuid,
       case when n <= 3 then md5('c101-split-top')::uuid
            else md5('c101-split-bottom')::uuid end,
       n, 'split'
from generate_series(1, 6) n;
...
begin
  ...
  values ('c101-parent-probe', 'Contract 101 parent probe', 'football')
  returning id into v_competition;

  insert into public.tournaments
    (name, year, competition_id, season_key, kind, display_timezone, status)
  values ('Contract 101 parent probe', 2026, v_competition, 'c101-parent',
          'league_season', 'Europe/London', 'active')
  returning id into v_tournament;

  insert into public.bonus_competitions
    (id, tournament_id, game_key, published, draw_required)
  values (md5('c101-other-comp')::uuid, v_tournament, 'predictor_cup', false, true);

  insert into public.bonus_cup_groups (id, competition_id, ordinal, size)
  values (md5('c101-other-group')::uuid, md5('c101-other-comp')::uuid, 1, 4);
end;
...
select is((select phase_kind from public.bonus_cup_groups where id = md5('c101-initial')::uuid),
  'initial', 'existing-style groups default to initial');
select is((select count(*)::integer from public.bonus_cup_groups
  where competition_id = md5('c101-comp')::uuid and phase_kind = 'split'), 2,
...
select is((select count(*)::integer from public.bonus_cup_groups child
  ...
  where child.competition_id = md5('c101-comp')::uuid and child.phase_kind = 'split'
...
select is((select count(*)::integer from public.bonus_cup_members
  where competition_id = md5('c101-comp')::uuid), 12,
  'six entrants hold twelve memberships across two phases');
select is((select count(*)::integer from public.bonus_cup_members
  where competition_id = md5('c101-comp')::uuid and phase_kind = 'initial'), 6,
  'original memberships remain');
select is((select count(*)::integer from public.bonus_cup_members
  where competition_id = md5('c101-comp')::uuid and phase_kind = 'split'), 6,
...
select is((select count(*)::integer from public.bonus_cup_members initial
  ...
  where initial.competition_id = md5('c101-comp')::uuid and initial.phase_kind = 'initial'),
...
select is(pg_temp.capture_sqlstate($sql$insert into public.bonus_cup_members
  ...
  (md5('c101-comp')::uuid,md5('c101-user-1')::uuid,md5('c101-split-bottom')::uuid,7,'split')$sql$),
  '23505', 'an entrant cannot hold two memberships in one phase');
select is(pg_temp.capture_sqlstate($sql$insert into public.bonus_cup_members
  (competition_id,user_id,group_id,draw_number,phase_kind) values
  (md5('c101-comp')::uuid,md5('c101-user-7')::uuid,md5('c101-split-top')::uuid,7,'initial')$sql$),
  '23503', 'an initial membership cannot point at a split group');
select is(pg_temp.capture_sqlstate($sql$insert into public.bonus_cup_members
  (competition_id,user_id,group_id,draw_number,phase_kind) values
  (md5('c101-comp')::uuid,md5('c101-user-7')::uuid,md5('c101-split-top')::uuid,1,'split')$sql$),
...
select is(pg_temp.capture_sqlstate($sql$insert into public.bonus_cup_members
  ...
  (md5('c101-comp')::uuid,md5('c101-user-7')::uuid,md5('c101-split-top')::uuid,7,'split',1)$sql$),
...
select is(pg_temp.capture_sqlstate($sql$insert into public.bonus_cup_groups
  ...
  (md5('c101-comp')::uuid,4,3,'split',md5('c101-split-top')::uuid)$sql$),
  '23514', 'a split group cannot parent another split group');
select is(pg_temp.capture_sqlstate($sql$insert into public.bonus_cup_groups
  (competition_id,ordinal,size,phase_kind,parent_group_id) values
  (md5('c101-comp')::uuid,4,3,'split',md5('c101-other-group')::uuid)$sql$),
...
select lives_ok($$update public.bonus_cup_members set group_position = 20
  where competition_id = md5('c101-comp')::uuid
    and user_id = md5('c101-user-1')::uuid and phase_kind = 'initial'$$,
...
select is(pg_temp.capture_sqlstate($sql$update public.bonus_cup_members set group_position = 21
  where competition_id = md5('c101-comp')::uuid
    and user_id = md5('c101-user-1')::uuid and phase_kind = 'initial'$sql$),
...
select lives_ok($sql$insert into public.bonus_cup_fixtures
  ...
  (md5('c101-split-fixture')::uuid,md5('c101-comp')::uuid,'split',
   md5('c101-split-top')::uuid,md5('c101-window')::uuid,34,
   md5('c101-user-1')::uuid,md5('c101-user-2')::uuid)$sql$,
  'a split fixture persists with the overall Cup matchday');
select is((select jsonb_build_object('stage',stage,'matchday',matchday)
  from public.bonus_cup_fixtures where id = md5('c101-split-fixture')::uuid),
...
select is(pg_temp.capture_sqlstate($sql$insert into public.bonus_cup_fixtures
  ...
  (md5('c101-comp')::uuid,'split',md5('c101-initial')::uuid,md5('c101-window')::uuid,35,
   md5('c101-user-3')::uuid,md5('c101-user-4')::uuid)$sql$),
  '23514', 'split fixtures cannot reference an initial group');
select is(pg_temp.capture_sqlstate($sql$insert into public.bonus_cup_fixtures
  (competition_id,stage,group_id,window_id,matchday,home_user_id,away_user_id) values
  (md5('c101-comp')::uuid,'group',md5('c101-split-bottom')::uuid,md5('c101-window')::uuid,35,
   md5('c101-user-3')::uuid,md5('c101-user-4')::uuid)$sql$),
...
select is(pg_temp.capture_sqlstate($sql$insert into public.bonus_cup_fixtures
  ...
  (md5('c101-comp')::uuid,'split',md5('c101-split-bottom')::uuid,md5('c101-window')::uuid,
   md5('c101-user-3')::uuid,md5('c101-user-4')::uuid)$sql$),
...
select is(pg_temp.capture_sqlstate($sql$insert into public.bonus_cup_fixtures
  ...
  (md5('c101-comp')::uuid,'split',md5('c101-split-bottom')::uuid,md5('c101-window')::uuid,36,4,
   md5('c101-user-3')::uuid,md5('c101-user-4')::uuid)$sql$),
...
select is(pg_temp.capture_sqlstate($sql$insert into public.bonus_cup_fixtures
  ...
  (md5('c101-comp')::uuid,'split',md5('c101-split-bottom')::uuid,md5('c101-window')::uuid,37,
   md5('c101-user-5')::uuid,md5('c101-user-6')::uuid,
   md5('c101-user-5')::uuid,'points',now())$sql$),
