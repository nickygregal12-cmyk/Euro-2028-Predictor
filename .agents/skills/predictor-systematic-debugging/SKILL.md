...
2. Materialize the immutable upstream workflow with `npm run agent:skill -- systematic-debugging`, then read the printed `SKILL.md` and only the referenced technique files needed for this defect.
...
6. For player-facing UI defects, retain the repository requirement for real browser evidence on relevant phone/desktop journeys; green unit tests alone are not completion evidence.
7. Never weaken fail-closed environment, permission, provider, migration, scoring or settlement rules to make a regression pass.
