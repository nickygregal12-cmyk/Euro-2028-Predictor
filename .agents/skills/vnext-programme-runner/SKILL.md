...
description: Resume and execute the vNext frontend programme across Stages 8–15 using a durable stage state machine, exact-head CI, review/correction loops, baseline repairs and explicit Production safety gates.
---
...
4. read `docs/product/ui.md`, `src/vnext/AGENTS.md` and the current stage's scoped authority;
5. inspect current `main`, current branch/PR and overlapping open PRs;
6. inspect executable source/tests for the surface being changed.

Current code, tests and canonical product/backend/deployment authorities always outrank this runner.
...
The programme completion predicate is the one recorded in `config/vnext-programme.json` and interpreted by `docs/product/vnext-programme-controller.md`.
...
4. inspect open PRs and identify any active stage PR;
5. if machine state is stale, repair the state based on evidence before doing product work;
6. derive the next executable action from the earliest stage that is not genuinely complete.

Never trust a previous chat's SHA, CI result, contract number or hosted-state claim without re-reading current evidence.

## Stage loop

For the current stage, repeat until its completion predicate is satisfied:

1. load only the stage-specific authority needed for the next slice;
2. implement a reviewable slice;
3. run focused validation;
4. run required repository gates appropriate to the delta;
5. push/update the PR;
6. inspect exact-head CI;
7. classify failures and fix them according to the failure loop below;
8. perform an independent-style review pass;
9. correct Blocker/Important findings narrowly;
10. re-review the corrective delta;
11. reconcile current `main` immediately before merge;
12. merge only when required checks and acceptance conditions are satisfied;
13. verify the merge SHA on `main`;
14. update `config/vnext-programme.json` so the completed stage is `merged` and the next stage becomes `in_progress`;
15. continue immediately into the next stage.
...
- read the current authority first;
...
2. finish every independent part safely possible;
3. use an existing parallel backend/tooling lane if one owns the missing authority;
4. continue another eligible slice/stage only when doing so does not violate stage dependencies;
5. return to the blocker when the dependency lands.

Only a load-bearing authority conflict or explicit Production gate may halt progression completely.

## Stage briefs are derived, not frozen

The stable stage order is held by the controller. The detailed prompt for Stage 10 or Stage 12 must not be blindly copied months later.

At each transition, derive the next stage brief from current:
...
The stage mission remains stable; implementation details may evolve.
...
- exact completion predicate still outstanding;
...
After the final stage is otherwise complete, execute the final programme audit defined by the controller. Do not declare the programme complete while a required route obligation, Blocker/Important finding, required CI failure or truthful Production-state requirement remains unresolved.

## Short invocation

A future session can be started with:

> Run the vNext programme runner. Resume from current repository state and continue executing, validating, reviewing, correcting and merging the next eligible work until the programme completion predicate is met or a genuinely non-resolvable authority/Production-safety blocker is reached. Do not stop merely because a PR, stage, CI repair or corrective pass completes. Persist a durable handoff before any session boundary.
