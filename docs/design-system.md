...
- Section labels/eyebrows: **11px** (2026-07-22 audit — the 10px variant is retired; several built eyebrows/table labels were at 10px, below the legibility floor, and two competing "eyebrow" sizes existed. One eyebrow size app-wide, tokenised as `--fs-eyebrow: 11px`), uppercase, letter-spacing 0.08em, `--tx3`, Space Grotesk
...
- Tap targets minimum 44x44px (score inputs are exactly 44x44). **This includes secondary and overlay controls (2026-07-22 audit):** the match-card chevron ships as a full 44×44 target (13px padding around the 18px icon, `right: 0`, card `padding-right: 44px`) — the built 30×30 target sat beside a 44px score input, so taps aimed at the away score opened the match centre instead. A sub-44 target adjacent to a 44 target is a fat-finger collision, not a tap target.

### Mobile platform contract (added 2026-07-22, post-audit — rules the doc never stated, now stated)
- **`viewport-fit=cover` is mandatory** in the viewport meta. Without it, `env(safe-area-inset-*)` resolves to **0** in mobile Safari, so the BottomNav's `padding-bottom: env(safe-area-inset-bottom)` was dead code and the primary navigation sat flush against the iPhone home-indicator swipe zone (mis-taps trigger app-switching mid-prediction). One attribute activates the padding already written.
- **`theme-color` per colour scheme:** two meta tags with `media="(prefers-color-scheme: dark/light)"` (navy / cream), plus a runtime update from `ThemeProvider` when the in-app toggle overrides the OS preference — a light-theme user must not get navy browser chrome over a cream app.
- **16px input floor** — see §3 (Typography). Stated here too because it's platform physics, not taste.

### Score input
- 44x44px, centred, 19px Space Grotesk 500
- **Border signals state (amended 2026-07-22 audit — accent = needs you, quiet = done):** an **empty** input carries the 1.5px `--acc` border in both themes (the call to action); a **filled** input drops to 1px `--line` (settled). *Supersedes "accent border on every editable input, equal prominence" — 72 permanently-green boxes meant the accent stopped meaning "act here", and it threw away the best free progress signal: a group screen now visibly calms down as it completes.*
- `inputmode="numeric"` for mobile keyboards
- **Keyboard assistance (added 2026-07-22 audit — the 72-input flow):** select-on-focus (tapping a filled box selects its value, so correcting is retype-over, never clear-first) and **single-digit auto-advance** (typing one digit moves focus home → away → done for that card; two-digit scores are the re-focus-and-continue edge case, `maxLength=2` retained — single-digit scores are ~95% of football predictions). Without this the group stage is ~144 discrete taps; with it, ~half.
...
- **Clickable:** whole card navigates to the match centre; chevron affordance on right edge — **the chevron is a full 44×44 target (2026-07-22 audit; see §4)**, never a sub-44 icon button floating beside the away score input. Score inputs and joker button stopPropagation. Feature-flagged via `FEATURES.matchCardNavigation` — **enabled since 2026-07-21** (the Match Centre + Matches tab shipped).
...
**Bottom nav — 5 tabs:** Home / Predict / **Matches** / League / More. Active tab in `--acc` (icon + label), inactive `--tx3`, 44px min targets, Space Grotesk labels, football icon for Predict, calendar for Matches, trophy for League. The 5-tab bar fits at 360px. Tabs proved to be config as designed: Matches was added (2026-07-21) and the match centre (`/match/:matchRef`) maps to the Matches tab with no nav rebuild.

**Top nav — app bar (decided 2026-07-22 — SHIPPED 2026-07-28 as `AppBar` in the design system, wired in `AppShell`; sign-out stays on More until the Account page ships):** a slim bar on every signed-in screen. Left: the page's title/eyebrow (context, not a logo — the wordmark belongs to auth screens and the landing page). Right, 44px targets: **theme toggle** (sun/moon icon — flips dark/light in place, persisted per user; relocated from More) and the **avatar chip** (32px initials circle) — tapping it opens the user's **own Profile**. `--bg` surface, optional hairline `--line` bottom rule; never taller than it needs to be — the bottom nav remains the primary navigation, this bar is identity + theme only.

**One title system (2026-07-22 audit):** page headers are **content-owned** — the in-content eyebrow + 22px title every screen already renders. The app bar's left slot carries context only; `PageShell`'s legacy `title`/`headerAction` props were **retired with the app-bar build (2026-07-28)** — `PageShell` now takes a `topBar` slot instead. No surface may ever double-header: one pattern, codified so a future page can't drift into both.
...
- **More** — **Games hub** (decided 2026-07-22: the bonus-games directory lives HERE — More → Games; games are surfaced contextually everywhere they matter via Home action cards, spectator pitch, and deadline nudges, so the hub is the directory, not the front door), Account (below), **How to play** (the /welcome content permanently findable + a mini-FAQ), how scoring works, and the **legal pages** when Phase 3 lands them. Theme moved to the top-nav app bar (shipped 2026-07-28); sign-out moves into Account when that page ships (today the More page holds display name, profile link, scoring link and sign-out; Games hub, Account and How to play are unbuilt).

**Predict hub — hero + journey map (re-cut decided 2026-07-22; supersedes the flat checklist — BUILT 2026-07-28: hero with weighted completion/countdown/champion/Continue, numbered stepper with tie-aware A–F chips, step eyebrows and stage-boundary CTAs on every prediction screen).** Pre-tournament this is the app's main page and must read like its centrepiece, not a settings list. Top to bottom:
...
**Post-lock: the Predict tab morphs into "My entry" (decided 2026-07-22 — FIRST CUT 2026-07-28: the hub title flips to My entry, the stepper becomes view-links, Review stays reachable for the auto-submission outcome and Jokers stays live; the points/rank hero and the consensus page remain).** At the lock instant the tab relabels (same nav slot, same icon — tabs are config) and the hub re-purposes from journey to identity: the hero flips from completion-and-countdown to the locked entry — champion flag + name, total points, overall rank. The stepper becomes **view-links into the user's picks**: Groups (read-only, scored match cards as results land), Bracket (your picks; the against-reality view lives in Matches → Bracket), Review summary + Share, and **"How everyone called it →"** (the post-lock consensus page — own section below). **Jokers stays fully live** — the one still-actionable stage (place/move until each match's kickoff), carrying the same unplayed-jokers nudge as the Matches tab. Nothing is removed; the tab never goes dead. Division of labour with the Matches tab per its spec: My entry = what you said; Matches = what's happening.

**Spectator state — no-entry after lock (decided 2026-07-22 — FIRST CUT 2026-07-28: lock explainer with the Games-hub pitch and a browse-the-tournament path; the richer surfaces remain).** A user who signs up (or never completed an entry) after `lock_at` is a first-class citizen, not a dead end. Rules: **the Original Predictor is locked and view-only for them; bonus games are fully open to them** — every bonus game (KO Predictor, Last Man Standing, Predictor Cup; sweepstake later) has its own entry window and deadlines, so a latecomer is a spectator in the main game and a full player in the bonus games; the bonus games are precisely the latecomer's pitch. Surfaces:
...
- **Member row (collapsed):** rank | movement arrow (accent up / red down / muted dash) | avatar initials + name (truncating, min-width: 0) | champion-pick flag (18×12) | latest points | total points. Current user's row: chip background, accent avatar, YOU chip, auto-scrolled into view on long lists. **Movement column renders only once `rank_history` has data (2026-07-22 audit)** — a full column of placeholder dashes is pure noise on the most-compared screen and squeezes the name cell; the grid track collapses entirely until the first snapshot exists (applies to `LeaderboardRow` everywhere it's used). Reserve space for features when they exist, not before.
...
- **Submit zone (amended 2026-07-22 audit)**: **sticky** at the viewport bottom (gradient fade to `--bg` above it) — the activation CTA is never hunted for at the end of a long checklist. When blocked, the zone shows an **enabled secondary action that routes**: "Fix N items → [first blocker's title]", deep-linking to the first incomplete screen (the checklist rows keep their per-row "Fix ›" links). *Supersedes the disabled "Fix 1 item to submit" button — a disabled control carrying an instruction is banned app-wide: the most important line on the page must never sit on the least legible, least interactive element (and `--tx3` on `--mut` fails AA — §3).* When clean, full-width accent "Submit entry" → confirm Modal → submitted state
...
**Order rule (2026-07-22 audit — binding):** the stat strip is always the first content element; the Today card second. **Share affordances never render above the user's own numbers** — sharing is downstream of seeing you're 2nd, not upstream of knowing anything (the built layout led with a full-width Share button, suppressing both the check-in loop and the share rate). The during-phase Share entry point moves out of the content flow entirely: a ShareIcon **header action** in the top-nav app bar — persistent, contextual, never pushing the stats down.

1. **Stat strip** — four segments in one card: total Points | Points Today (accent) | overall rank + movement arrow | best-league position. Each segment taps through to its source screen — **with visible affordance (2026-07-22 audit):** a quiet `›` on the segment label plus an `:active` press state (background → `--chip`, 120ms). Four buttons must not render as static stats; hidden interactivity is zero-discovery navigation.
2. **Today card** — cyan border while any match is live (live matches appear *inside* this card; Home has no separate live strip). One row per fixture today: live rows get pulsing dot (reduced-motion: static) + score + minute (from the live-data feed — see §6 → Live data & refresh); upcoming rows show kickoff time; every row shows the user's prediction ("You said 2–1") and chevrons into the match centre. **Team-name truncation rule (2026-07-22 audit):** fixture rows never ellipsis country names (the built `8ch` cap clipped most of Europe — "Netherl…"); full names by default, and where two names + a live score genuinely can't fit at 360px, fall back to FIFA trigrams (`NED 2–1 SUI`) — a deliberate format, never a mid-word ellipsis on the most-viewed card of the tournament.
...
### Account page (decided 2026-07-22 — More → Account; the private half of the profile/account split — NOT YET BUILT)
...
- **Vertical rhythm (2026-07-22 audit):** tight within groups, open between them — title block ~4px internal, the steps card ~20px above / ~8px below — so the primary CTA visually binds to the card it pays off. Never a uniform gap between every element (the built 14px-everywhere flattened all grouping).
...
### Matches tab (5th nav slot — expansion into the during-tournament command centre decided 2026-07-22; all four sub-views shipped in v1 form 2026-07-28, deferred slices noted per sub-view)
The answer to "what's happening, and how am I doing against it". A **segmented control at the top** switches four sub-views (aria-pressed toggle chips in a labelled group, same reasoning as the fixture filter — not a tablist):
...
- **Tables** — all six groups rendered with the Live table component (Predicted / Live switcher + the "You" comparison column — that component's designed home is HERE). Live values from entered results; predicted from the user's entry; footer status line per the component spec.
  - **Built v1 (2026-07-28):** all six groups + the best-thirds ranking rendered from REAL results only, through the standard Group table / Best-third table components — the same pure domain as the predictor (`resolveGroupTies` / `rankThirdPlacedTeams`) fed authoritative scores. Real-world ties the criteria can't break yet display as shared positions with an explanatory note (never an invented order); the best-thirds table appears once any group game has been played (an all-tied ranking of untouched teams is noise). **Deferred:** the Live table component itself (Predicted/Live switcher, "You" column, cyan live dot) stays Phase 3 — when it lands it replaces the standard tables here.
- **Bracket ("as it stands")** — the projection view. Once group games are underway, the R16 auto-populates from **live standings** (the same pure domain — `resolveGroupTies` → `rankThirdPlacedTeams` → `resolveRoundOf16` — fed real results instead of predictions), displayed against the user's predicted R16 with **per-slot agree/differ marks** (accent tick = your predicted team currently holds that slot; muted diff = someone else does). Clearly labelled provisional ("As it stands after MD2") in cyan-family language until each group completes; projections harden into real fixtures as groups finish; once the KO starts, it shows real results vs the user's picks through to the final (adjacent to the H2H bracket-health concept — one visual language between them).
  - **Built v1 (2026-07-28):** the real knockout tree from server-owned data — resolved teams where the server has populated them, slot descriptions ("Winner Group A", "Best third", "Winner QF-1") while a slot is open, and the authoritative score / winner / decided-by line once a tie settles; every tie taps through to the match centre. **Deferred:** the mid-groups live-standings projection with per-slot agree/differ marks — it needs the tournament-context engine's phase language ("As it stands after MD2") and lands with it.
- **Stats** — the awards race: (1) **Group-stage goals** — running real total (sum of entered results) vs the user's derived predicted total, with the tiered-points bands visible ("You said 89 · currently 41 after 18 games"); works from day one. (2) **Top scorers** — live top-5 vs the user's golden boot pick highlighted in its position (or "outside the top 5 · 1 goal"). **Feed-dependent:** scorer data doesn't exist in manual result entry, so this card renders only once the live-data feed (§6) supplies it — hidden entirely before that, never a broken placeholder. Stats also carries a quiet link to **"How everyone called it"** (the consensus page under My entry).
  - **Built v1 (2026-07-28):** the group-stage goals race (real running total; "You said N" appears once the entry's group scores are complete), result-derived records (biggest win, highest-scoring match, goals per match), top-scoring teams (top 5 by real goals — the buildable substitute while scorer data doesn't exist), and a tournament facts card (dates, host cities, matches played). Top scorers stays hidden per the feed rule above. **Deferred:** the tiered-points band visual and the consensus-page link (that page is itself unbuilt).
...
### How everyone called it (post-lock consensus page — decided 2026-07-22; lives under My entry, pointed to from Matches → Stats — NOT YET BUILT, and its Matches → Stats / My entry entry points are themselves unbuilt)
...
### Admin panel (protected ops console — core sections SHIPPED 2026-07-27; remaining sections designed, deferred)

**Build status (synced 2026-07-27):** the protected shell and the two operationally critical sections are live — `/admin` → `/admin/results` (result confirm/correct/clear with review step, revision history, regulation/extra-time/penalty handling, and actual third-place qualification-boundary resolution) and `/admin/users`, behind `RequireAdmin` capability checks on server-owned Auth `app_metadata` (PRs #110-follow-ons through #120, #126). The interim SQL mechanism in `docs/ops-result-entry.md` is superseded for result entry. The sections below marked "designed, deferred" (Overview, Leagues, Fixtures, Connections, Feature flags, Audit log) remain unbuilt.
The operator's cockpit for running a live tournament. Not a public surface — a protected route behind the server-side admin grant (`docs/ops-admin-bootstrap.md`; RLS / SECURITY DEFINER checks are the real gate, the hidden nav is cosmetic). One shell, a left rail of sections, each a focused tool. Mobile-usable (result entry must work from a phone at a match) but density-first, not marketing-polished. Every mutating action is audit-logged (below) and, where it recomputes scores, shows the impact before commit. *(The shipped result workspace records immutable revisions; the full cross-section audit-log surface remains deferred.)*
...
**Prose specs for the remaining sections** (designed, deferred — except Users, whose first cut shipped):
- **Users** *(first cut SHIPPED as `/admin/users`; the fuller spec below remains the target)* — search/browse profiles; see role, entry status, league memberships, join date; grant/revoke admin (mirrors the bootstrap SQL, audit-logged); moderation actions on display names that slipped past the filter. Never exposes passwords or auth internals — reads the same `profiles` the app does.
...
Presentational shell + forms (previewable in `/dev/components`): `AuthScreen` — app title over a centred card on `--bg`. **Log in** (email, password, "Forgot password?" link) / **Sign up** (display name 1–40, email, password) / **Reset request** (`/auth/reset` — neutral "if an account exists" confirmation, enumeration-safe) / **Update password** (`/auth/update-password` — grace window while the recovery session resolves, success → Home, expired-link fallback). Errors surface via `Alert` in friendly copy only (`friendlyAuthError` — a raw Supabase message never reaches the user); validation mirrors the server (`validateSignUp`). Turnstile widget renders above the submit button on both forms when enabled; widget remounts after a server error (single-use tokens). Session restore shows a neutral splash — a refresh never flashes the logged-out screens.

**Amendments (2026-07-22 audit):**
- **Submit is never disabled-until-valid.** The button stays enabled; validation *speaks* on submit — missing fields → the existing per-field errors; missing Turnstile token → a visible line ("Still verifying you're human — give it a second, or reload the page"). Silent `return` inside a submit handler is banned: a tap that does literally nothing is a dead end the user cannot diagnose. *(Supersedes "submit gates on a token" as a disabled-button mechanic — the gate stays, but it gates the call and explains itself, never the button.)*
- **Turnstile load failure is a designed state.** If the Cloudflare script errors or is blocked (ad-blockers, corporate networks, flaky mobile), the form says so and offers a reload — never a permanently dead CTA. The script-load rejection (`loadScript().catch`) must surface, not vanish into a null token.
- **Value prop above the sign-up form (interim, until the Landing page ships).** The auth route is the de-facto front door for all cold traffic today, and its entire pitch was a 13px `--tx2` tagline. Until the real landing page becomes the logged-out root: the /welcome three-step card (already presentational + token-clean) renders above the sign-up card, and the tagline is promoted to 15px `--tx`. Removed the day the Landing page (§6) ships.
- **One source of truth per field rule:** the password field's placeholder is "Password"; the *hint* carries the rule ("At least 6 characters.") — never both saying the same thing (placeholder-as-instruction vanishes on focus anyway).
