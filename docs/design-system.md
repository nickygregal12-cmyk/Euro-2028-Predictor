...
- **Connections** — external integrations and their health: SMTP/Resend send status, the (future) live-score API feed, Supabase project pointer (dev vs production — the split is a Phase 2 exit gate). Read-only health first; any credential lives in the dashboard/env, never surfaced here. The live-score API, when it exists, is prefill-only — admin always confirms a result (the manual-entry design is deliberate).
