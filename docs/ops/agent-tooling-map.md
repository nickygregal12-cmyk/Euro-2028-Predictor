...
| How do I give one prompt and let GPT coordinate Claude implementation plus independent Ox review? | **Predictor Conductor** in OpenCode | persistent multi-model orchestration |
...
| Persistent private multi-model Conductor | [`persistent-cloud-conductor.md`](persistent-cloud-conductor.md) |
...
| OpenCode / Ox Alpha | OpenCode CLI provisioned; **no OpenRouter key is committed and no model request is made until the operator supplies a private developer credential and launches it** |
| Predictor Conductor | tracked GPT/Claude/Ox agent definitions + private-cloud installer; **no VM is created and no service/model request runs by configuration alone** |
