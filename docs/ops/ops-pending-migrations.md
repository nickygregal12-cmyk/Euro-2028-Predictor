...
## Current state — repository 200, Production 198, Development 198 (18 August 2026)

| Environment | Contract | Evidence | Status |
| --- | ---: | --- | --- |
| Repository candidate | **200** | 200 canonical migrations through `20260818050000_ai_odds_collection_cadence.sql`; contracts 199-200 are private AI Lab operational contracts — canonical betting evidence and the paid odds cadence — and 198 canonical migrations through `20260818030000_cup_knockout_reservation.sql`; contracts 191-192 give a weekly-season standings row a player address, position over time and a season-long rivalry, contracts 193-194 close `CUP-003` and `CUP-004`, contracts 195-196 give the action centre its Championship and consequence generators, contract 197 adds the cross-competition football calendar, and contract 198 closes `CUP-006`. | EIGHT CONTRACTS AHEAD OF PRODUCTION; DEVELOPMENT NINE BEHIND |
...
| Production Supabase | **198** | `vkfnsqdyhvtwyqkisxhk`; owner-dispatched rollout [32126890172](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/32126890172) succeeded at 10:28:59Z from exact `main` `3751dc1`, gated by its own sequence check on a fresh encrypted backup and on exact-head rehearsal [32126447952](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/32126447952) at 10:27:16Z, with the operator typing the confirmation phrase naming the boundary. **This row read 190 until 18 August 2026 because no record-only follow-up reached `main` after the promotion — the same `DOC-001` drift the Development row above carried, on the other environment.** Independent read-only verification at 11:05Z names `20260818030000 cup_knockout_reservation` beside a total of 198: 1 auth user, 1 profile, 3 entries, 1 league, 578 season fixtures, 226 `ai.bets` of which 177 pass `ai.valid_bets` and 0 have a `ai.bet_results` row, nine current models at `auto-20260817T0754Z` with eight challengers, 744 of 20,000 paid credits used this month, and 11 cron jobs of which exactly one is `ai-odds-window-heartbeat`. | TWO CONTRACTS BEHIND REPOSITORY; PAID COLLECTION ON |
