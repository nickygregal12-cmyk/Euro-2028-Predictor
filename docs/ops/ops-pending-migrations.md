...
- **104:** The Predictor Championship split table, derived across both phases. ADR 0025 decision 2 requires the continuing standings to come from the initial AND split fixtures "so points genuinely carry forward rather than being copied into a new starting total". `predictor_internal.cup_split_group_tables` attributes both kinds of fixture to the entrant's **split** group before aggregating, which is the carry-forward itself; no column stores a starting total, so correcting a league-phase result after the split moves the continuing table. `cup_final_group_tables` is untouched and still answers on the initial roster alone.

Contracts 64–103 are all applied to development; contract 104 is not yet. None is authorised for production merely to remove the intentional contract gap.
