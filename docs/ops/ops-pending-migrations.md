...
The repository and hosted Development are both at **contract 131**. Contracts 126–131 were applied on 7 August 2026 by guarded fast-lane evidence run `31186564948`, pinned to exact repository head `50b01154e5a6cfc2ebc1b2312eaceb42e8a39b57`. Before any hosted write the lane proved the target was Development project `iouzoutneyjpugbbtdem`, hard-refused Production, required exactly the six canonical pending versions `20260806170000` through `20260806220000`, passed the repository additive-migration checker, and took the ADR 0024 lightweight snapshot. `supabase db push` then applied all six; workflow parity and an independent Supabase ledger read both ended at `20260806220000_period_standings_display_names`. Evidence artifact `development-fast-lane-126-131-evidence` is retained for seven days as artifact `8997049916`. **Production is also hosted at contract 131 after guarded continuation run `31216257649`; repository, Development and Production are aligned and no migration promotion is pending.**

**The record of that rollout did not reach `main` on its own.** The follow-up workflow wrote the correct contract-120 record, pushed it to its automation branch and then failed, because the repository forbids GitHub Actions from opening pull requests — so `main` went on stating 115 while the truth sat on a branch. The record has since been landed by hand and the workflow now announces a refused pull request through a job summary and an issue instead of dying silently.
...
> **Contract 133 boundary (8 August 2026):** Contract 133 follows Contract 132 and adds only bounded player reads for season Predictor Championship instances. Development must apply 132 first and then 133; Production promotion remains a separate explicit operation.
