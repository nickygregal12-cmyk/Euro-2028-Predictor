...
The repository is at **contract 107**; development is at **106**, applied by fast-lane run 30984799464, and trails by one until the next fast-lane run.

| Environment | Contract | Evidence | Status |
| --- | ---: | --- | --- |
| Repository `main` | **107** | Contract 106 closes DATA-009 so a corrected result still rederives after a competition completes, through `20260805020000_terminal_aware_bonus_rederive.sql`; contract 107 is the Last Man Standing restart driver — complete, succeed, re-enter, copy nothing else — through `20260805030000_lms_restart_driver.sql` | MERGED, AWAITING DEVELOPMENT ROLLOUT |
| Development Supabase `iouzoutneyjpugbbtdem` | **106** | Applied 5 August 2026 by fast-lane run 30984799464 on `aa46fee`. Corroborated read-only: the ledger holds 106 rows through `20260805020000`. Both Bonus Games rederive functions now resolve through `current_public_competition_id` and **neither** retains a `predictor_internal.live_competition_id` call, so a result corrected after a competition completes rederives instead of silently returning (DATA-009). Neither is reachable by `anon`, `authenticated` or `service_role`. **Zero** competitions are completed, so the change is behaviour-neutral today and lands ahead of the restart driver that will make it reachable | VERIFIED; ONE CONTRACT BEHIND THE REPOSITORY |
...
Two operational consequences, and the first is already live — contracts 98–106 landed after the declaration moved to 97, so previews are printing the "unavailable" line right now. Development is rolled out to 106, so moving the declaration to 106 is the correct next owner action:
...
Contracts 64–106 are applied to development; contract 107 is not yet. None is authorised for production merely to remove the intentional contract gap.
...
3. The Netlify `dev`, branch-deploy and deploy-preview declarations moved 86 → 97 on 4 August 2026, owner-reported; read the corroboration note above before relying on it. They now trail both development and the repository, which are level at **105**, by eight. The precondition that they move only **after** development is rolled out is satisfied — run 30968263589 brought development to 105 — so moving them to **105** is unblocked and is the next owner action here.
