...
## Current state — repository 189, both hosted databases at 188 (13 August 2026)

| Environment | Contract | Evidence | Status |
| --- | ---: | --- | --- |
| Repository candidate | **189** | 189 canonical migrations through `20260813215920_ai_quarantined_evidence_reads.sql`, which takes quarantined forecasts and the bets advised from them out of every admin AI evidence read. Additive; function replacement plus one private view. | ONE AHEAD OF BOTH |
...
| Production Supabase | **188** | `vkfnsqdyhvtwyqkisxhk`. Guarded rollout run [31739706612](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/31739706612) from exact `main` `1510a6e3` at 20:13 UTC, gated on fresh backup [31730225719](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/31730225719) and rehearsal [31739287772](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/31739287772). Independent postflight at 20:14:11Z **names** the same three rows. Protected counts unchanged, 14 cron jobs before and after, Euro `hidden`, zero provider calls. Paid collection was disabled at 2026-08-13T15:09Z under `KNOWN_IDENTITY_PIPELINE_CORRUPTION` and **RE-ENABLED at 2026-08-13T21:24:37Z** after the repair, the alias audit and a clean regeneration; allowance 20,000/18,000. | LEVEL; PAID COLLECTION ON |

**Contract 189 is pending on both environments, and is the only thing pending.** It is one additive migration, `20260813215920_ai_quarantined_evidence_reads.sql`: one private view (`ai.valid_bets`) and eight `create or replace` function replacements, no table, column, trigger, constraint, policy or grant moved and no row written. It exists because contract 188's quarantine was enforced on three reads and declared on all of them — measured against the **installed** Production definitions, four prediction reads still selected from `ai.predictions` and three betting reads still counted `ai.bets`/`ai.bet_results`, the publication gate joining nothing at all. It is masked on Production only by `ai.prediction_results` holding zero rows, and the earliest quarantined kickoff is 2026-08-14T19:00Z, so the boundary is time-bound rather than theoretical.
