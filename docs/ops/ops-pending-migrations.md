...
**NOT additive, and therefore NOT fast-lane eligible.** `check-migration-additive.mjs` refuses it by name: `drop function`. Narrowing `get_league_preview` from five returned columns to two cannot be done with `create or replace` — PostgreSQL answers `cannot change return type of existing function (SQLSTATE 42P13)` — so the function is dropped and recreated, and the checker is right to call that destructive: a narrowed return type removes a guarantee a caller may rely on. **Contract 152 goes through `stage-c1-development-rollout.yml`, with its backup and restore rehearsal, not the fast lane.** It also carries one `alter table ... drop constraint` immediately followed by its replacement, which the checker reports as third-category: no row is at risk and the character-length range only widens.

**The first draft of this migration was wrong in two ways and neither was visible by reading it.** Database parity rebuilt every committed migration on a disposable local database and stopped at contract 152 with the 42P13 above — the failure this environment could not produce, since it has no Docker daemon. Investigating it surfaced the second, worse error: the draft rebuilt `join_league` from the **19 July** original rather than from `20260803070000_c1b_game_catalogue_memberships.sql`, which silently dropped the game-membership gate (*"Join this league game before joining its private league"*) and reverted the pinned empty `search_path` — while its own comment claimed the body was verbatim. Both are corrected; the record is here rather than only in the diff, because deleting a membership rule under cover of adding a rate limit is exactly what this repository's rules exist to catch.
