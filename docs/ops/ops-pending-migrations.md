...
| --- | ---: | --- | --- |
| Repository candidate | **96** | Contracts 94–95 add season standings and the bounded co-member leaderboard read; contract 96 adds server-only provider response custody and decoder evidence through `20260804243000_provider_ingestion_custody.sql` | EXACT GATES REQUIRED BEFORE MERGE |
| Development Supabase `iouzoutneyjpugbbtdem` | **93** | Applied 4 August 2026 by fast-lane run 30920330240 on `2c2fe85`; evidence artifact `development-fast-lane-evidence` (ID 8896836977) retained. Corroborated read-only, structurally and behaviourally: the ledger holds 93 rows through `20260804213000`; `season_fixtures` carries `replay_fixture_id` and its chain-walk trigger; `process_due_season_matchweek_scores` has no `EXECUTE` for `anon`, `authenticated` or `service_role`; `cron.job` holds `season-settle-due-matchweek-scores` on `30 * * * *` as `username = postgres`. Inside a rolled-back transaction the settler returned `settled` for a league season, refused a tournament with `not_a_league_season` and wrote no rows. Contract 91's earlier probes — `is_settlement_score('{}')` returning false rather than null, a scoreless `completed` fixture refusing `completed_without_result`, and a completed + void + carried card reporting a denominator of 1 — were verified on run 30916033941 | VERIFIED; CONTRACT 96 PENDING |
