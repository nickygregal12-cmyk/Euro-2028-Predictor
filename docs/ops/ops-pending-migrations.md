...
> **Repository candidate — contract 204, the league-scoped quarantine count (18 August 2026):** `20260818090000_ai_dashboard_league_scoped_quarantine.sql` redefines `public.admin_ai_dashboard` and `public.admin_ai_betting_dashboard` at their existing signatures so `quarantined_predictions` answers for the league asked about, and adds `ai.quarantined_predictions` as the custody view they read it through. **Additive** — two functions redefined, one view created, no relation altered, no row touched, no grant widened, no threshold moved; asked with no league both still return the lab-wide count. **FOUND BY THE 198-TO-203 REHEARSAL, NOT BY CI.** Suite 236 asserted the dashboard reports one excluded forecast for its one seeded league and passed every run, because the database CI builds holds nothing but the fixture and the lab-wide count and the league count are the same number. Against a disposable copy of Production carrying 37 real quarantined forecasts it answered 38. The test was right and had been passing vacuously; it now seeds a second league so it cannot. **Applied to Development by fast-lane run [32196292139](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/32196292139); not applied to Production by this record.**

> **Repository candidate — contract 203, the Bet Builder currency rule (18 August 2026):** `20260818080000_ai_bet_builder_current_decision.sql` redefines `public.admin_ai_bet_builder_candidates` and `public.admin_ai_bet_builder_books` at their existing signatures. **Additive** — no relation created or altered, no row touched, no threshold moved, and the allow-list is unchanged because neither function is new. **Applied to no hosted environment by this record.**
>
> **Its visible consequence:** a superseded BET can no longer be returned as an actionable leg, and the venue picker's leg count becomes the number of legs selecting it returns. The nine-request browser fan-out that re-derived the same rule is deleted, so the Bet Builder is one request rather than ten.

> **Repository candidate — contract 202, forecast horizons (18 August 2026):** `20260818070000_ai_forecast_horizons.sql` widens the `ai.predictions.horizon` CHECK to admit `t72`, `t120` and `t168`. **Additive** — one CHECK constraint replaced by a strictly wider one; no relation created, no row rewritten, no function redefined, no threshold moved. The migration asserts the row count is unchanged across the widening. **Applied to no hosted environment by this record.**
>
> **Why it exists.** `predict.py` scored 52 upcoming fixtures on 18 August after that morning's import and wrote nothing, because the per-model uniqueness key is `(model_id, fixture_id, horizon)` and `scheduled` covered everything beyond forty-eight hours. Predictions stay immutable; a later, better forecast is now a new row rather than a discarded one.
...
> **Contract 205 repository candidate — `CUP-007`, the bracket read survives a split (19 August 2026):** `20260819100000_cup_bracket_seed_initial_phase.sql` pins contract 193's `your_seed` lookup to the initial membership. It is a one-line semantic correction to a read that raised an exception for every entrant in a split Championship; the signature, volatility, grants, entrancy boundary and Penalty Number scoping are all carried across unchanged. It claims **no** Development or Production rollout. The `stage <> 'group'` predicate contract 193 shares with nothing else is a separate defect and is deliberately **not** addressed here.
...
| --- | ---: | --- | --- |
| Repository candidate | **205** | 205 canonical migrations through `20260819100000_cup_bracket_seed_initial_phase.sql`. **Contract 205 is a corrective read fix, not a new capability.** Contract 193's `qualification.your_seed` is a scalar subquery over `bonus_cup_members` filtered by competition and user only; contract 102 keyed that table `(competition_id, user_id, phase_kind)` so one entrant may hold both an `initial` and a `split` membership, and contract 124's split transition inserts the second without deleting the first. From the moment a competition splits the subquery matched two rows and raised `more than one row returned by a subquery used as an expression`, failing the **whole** read — format, qualification, my_tie, penalty_number, my_ties, bracket and champion — for every entrant in it. Reproduced on a scratch PostgreSQL 16 against the real key shape before the fix was written. The lookup is now pinned to the initial membership, which is the only phase that can hold a seed at all: `bonus_cup_members_split_metadata_empty` requires a split row to carry `seed`, `group_position` and `qualified_as` as NULL. Nothing else in contract 193 changes — the two sibling lookups were checked and are already safe (`qualifiers` is an aggregate, `you_qualified` is an `exists`), and contract 193's own guard block is re-run against the replaced definition with one assertion added, so the defect cannot return through a later restated body. No hosted rollout is claimed. | ONE CONTRACT AHEAD OF DEVELOPMENT; SEVEN AHEAD OF PRODUCTION |
| Development Supabase `iouzoutneyjpugbbtdem` | **205** | Contract 205 applied 19 August 2026 from exact `main` `88576cf` by the **guarded** lane, run [32246603342](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/32246603342), on top of contract 204 applied by fast-lane run [32196292139](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/32196292139). **The lane was switched, and the reason is recorded rather than left to inference:** the ADR 0024 fast lane was dispatched twice and both runs hung on *Take the lightweight pre-apply snapshot* — runs [32244708088](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/32244708088) and [32245758861](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/32245758861), both cancelled. Read-only SQL against the same project worked throughout, so the CLI dump was hanging rather than the database being unreachable; both runs proved 205 additive before the snapshot and neither reached the apply, so Development was untouched at 204 each time. The guarded lane carries no dump step, names its boundary by filename, proves the source contract against the live database and dry runs — **stricter about what is applied, not looser** — and completed in 18 seconds. It gives up the snapshot, which is acceptable for Development because ADR 0024 makes the deterministic seed the recovery authority there; it would not be for Production. Independent read-only verification names `20260819100000 cup_bracket_seed_initial_phase` beside a total of 205. **Contract 205 is the first in this range to touch a player-facing read** — `public.get_season_cup_bracket(uuid)` — so that grant was measured: executable by `authenticated`, **not** by `anon`. Contract 204's AI Lab custody survives 205 landing on it: `ai.quarantined_predictions` readable by neither browser role, schema `ai` still zero grants to either. Every player-owned count unchanged (24 auth users, 24 profiles, 34 entries, 64 season predictions, 756 match predictions, 8 matchweek scores, 20 entrants, 0 bonus score events, 5 leagues, 39 members, 1 Cup launch, 112 Cup fixtures, 578 season fixtures); cron still 14 jobs; paid odds collection still **disabled** with `ai.api_usage` at zero; both publication gates still **false**. | SEVEN CONTRACTS AHEAD OF PRODUCTION; LEVEL WITH REPOSITORY; PAID COLLECTION OFF |
| Production Supabase | **205** | `vkfnsqdyhvtwyqkisxhk`; guarded rollout [32253892640](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/32253892640) from exact `main` `8971245`, gated on encrypted backup [32229916242](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/32229916242) and exact-head rehearsal [32252751621](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/32252751621). **Seven migrations, proven additive as an exact set.** The rehearsal dumped Production, restored a disposable copy, applied 199–205 to it and ran **all seven behavioural pgTAP suites green against restored Production data** — including 236 and 237, which failed seven assertions on the previous attempt until their absolute counts were made baseline-relative. **THE ROLLOUT RUN IS RECORDED AS FAILED AND THAT IS NOT A CONTRADICTION:** the apply succeeded and the run then exited non-zero on its own postflight measurement, which carried one closing parenthesis too many and so reported a syntax error sixty lines from the mistake. **Production is correctly migrated and the workflow's own verification never ran.** The parenthesis is fixed in the same change as this row, the fixed statement executes end to end, and a guard now refuses an unbalanced `jsonb_build_object` in any promotion workflow. Independent read-only verification names `20260819100000 cup_bracket_seed_initial_phase` beside a total of 205. Contract 204's custody is live: both AI Lab dashboards read the excluded-forecast count through `ai.quarantined_predictions` and both scope it by league, **neither** reaches `ai.predictions` directly, the view is readable by **no** browser role and schema `ai` holds **zero** grants to either. Contract 205's `get_season_cup_bracket(uuid)` is executable by `authenticated`, **not** `anon`. AI Lab history intact: 230 bets, 317 forecasts, 37 quarantined, 87 settled, 9 current models — all matching the pre-rollout measurement. `ai.valid_bets` reads **79 where it read 181**, which is contract 199 reclassifying evidence to one canonical row per fixture/market pair **without deleting or rewriting a base row** — the rehearsal predicted the same 79 over 79 pairs before Production saw it. Player-owned counts unchanged (1 auth user, 1 profile, 3 entries, 36 match predictions, 1 league, 1 member, 578 season fixtures); cron 11 jobs; Euro publication still `hidden`; both publication gates still **false**; paid collection still on at 770/20000 credits with 19230 remaining and **no credit spent by this rollout**. | LEVEL WITH REPOSITORY AND DEVELOPMENT; AI LAB STILL PRIVATE |

**There are no pending hosted migrations: repository, Development and Production are level at Contract 205.** Contracts 199 to 205 reached Development first and Production second, in that order, which the Production workflows refuse to invert. The sentence this replaced said contracts 199 to 204 were applied to Development only, which was true until the 198-to-205 rollout and is corrected rather than deleted, because the ordering it recorded is the reason the table above is trustworthy. This paragraph described the 190 boundary and was left behind when the table above moved twice in one day; it is corrected here rather than deleted, because the sequence it recorded is the reason the table is trustworthy. Contract 190 was the single additive `20260814005000_ai_actionable_bet_evidence.sql` boundary, which keeps historical bet rows immutable while excluding synthetic aggregate/reference books from actionable betting evidence and refusing future unbettable aggregate writes. Development reached 198 through the ADR 0024 fast lane and Production followed it, in that order, which the Production workflows now refuse to invert. Contracts 199 to 202 have since taken the same route in the same order — Development first, through fast-lane run [32141009659](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/32141009659) — and contract 203 is a repository candidate only.
