...
The repository is at **contract 96**; development is at 95 until the next rollout.

| Environment | Contract | Evidence | Status |
| --- | ---: | --- | --- |
| Repository `main` | **96** | Contract 96 corrects a refusal-order drift between the Cup tie settlement's two implementations, through `20260804243000_cup_tie_settlement_refusal_order.sql` | MERGED; DEVELOPMENT NOT YET APPLIED |
| Development Supabase `iouzoutneyjpugbbtdem` | **95** | Applied 4 August 2026 by fast-lane run 30923985137 on `21c9645`. Corroborated read-only: the ledger holds 95 rows through `20260804233000`; `predictor_internal.season_standings` exists; `public.get_season_leaderboard` is executable by `authenticated` and **not** by `service_role`, which is the deliberate asymmetry contract 95 documents | VERIFIED AND ALIGNED |
...
Contracts 64–95 are applied to development; contract 96 is merged and not yet applied. None is authorised for production merely to remove the intentional contract gap.
