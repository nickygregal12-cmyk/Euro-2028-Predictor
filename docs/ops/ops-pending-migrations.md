...
> **Contract 185 repository candidate — private AI Lab and odds custody (12 August 2026):** `20260812070000_ai_lab_operational_loop.sql` implements [ADR 0029](../adr/0029-private-ai-lab-and-odds-custody.md). It is a private analytical schema, admin-only bounded reads and service-only writes; it cannot write platform fixture/result/scoring truth. The paid key stays in the existing Edge Function, the archived request URL is credential-free and collection is installed **disabled** behind a monthly budget. The migration is structurally non-additive because it consolidates the supplied six-stage scaffold and removes constraints/columns created earlier in the same transaction; it therefore requires the guarded Development migration workflow, not the fast lane. **Applied to no hosted environment by this record. Production rollout is not authorised.**
>
...
## The 178 to 184 Production boundary was prepared, refused by its own guard, and abandoned — 12 August 2026 (forty-third entry)

**No Production mutation occurred. Production remains at 178.** This entry exists because a Production backup was taken and a rehearsal was dispatched, and evidence of an attempt that stopped is worth more than silence about it.

**What was prepared.** The owner authorised a Production promotion on 12 August 2026. A one-shot pair, `production-178-to-184-rehearsal.yml` and `production-178-to-184-rollout.yml`, was derived from the proven 174 → 178 pair — deliberately, because the 158 → 171 rehearsal once died on `supabase: command not found` after being derived from an older pair that had silently lost its CLI setup steps. It merged as #748 with Workflow lint, CI and CodeQL green, and the range selection was simulated locally against the committed tree to resolve to exactly six files.

**What stopped it, and it is the guard working rather than failing.** Between the pair being written and the rehearsal being dispatched, contract **185** (`20260812070000_ai_lab_operational_loop.sql`) merged. The rehearsal refused at its fourth step — *"Prove the repository still holds exactly the approved contract 184"* — because the repository by then held 185. Had that assertion not existed, the dry run would have found a seventh pending migration and promoted something nobody approved.

*(The visible error in the log is `supabase: command not found`, which is a red herring worth naming: it comes from the `if: always()` cleanup step, which calls the CLI that the skipped setup step never installed. The real failure is the assertion above it, and its own message names the count.)*

**Why the boundary was not simply extended to 185.** Contract 185 is **destructive** — `check-migration-additive.mjs` refuses it on `alter table ai.publication_gate drop column` — and it writes data at migration level, seeding `ai.publication_gate`, `ai.bookmakers` and `ai.markets`. The whole justification for promoting six contracts as one boundary is that every one of them is additive, so the checker runs as a **GATE** rather than a report. Extending to 185 would have forced that softening, which this document and `CLAUDE.md` both say should stop a rollout rather than be worked around. Contract 185 is also not on Development, so promoting it would have put it on Production first.

**The owner's decision, taken on those facts, was to stop.** Production stays at **178**, now seven behind the repository at 185, and Development at 184 is one behind for the same reason — contract 185 is on neither.

**What a future attempt needs, so it does not start from scratch.** The pair is removed here rather than left dispatchable, because a one-shot pinned to an abandoned boundary is a live workflow nobody approved; it is recoverable from git history at #748, which is exactly how it was built from the deleted 174 → 178 pair. A future attempt must additionally decide what the "repository holds exactly contract N" assertion should say when the repository tip has moved past the boundary — every prior Production boundary rolled out at the tip and never faced the question. Backup run `31617219376` succeeded against Production and was never used; a fresh one should be taken rather than that one reused.
