...
The repository candidate is **contract 134**. Hosted Development and Production are both independently verified at **contract 132**, ending at `20260807210812_provider_initial_fixture_approval`; Contract 133 (`20260808003000_private_season_cup_player_reads.sql`) and Contract 134 (`20260809030000_rate_limit_events_client_revoke.sql`) are therefore the two pending migrations and remain repository-only until the guarded Development rollout is applied and verified. Contract 134 is additive and privileges-only — two `revoke` statements, no relation, function, policy, trigger or rule — so it is fast-lane eligible under ADR 0024. Production follows only through the separately controlled Production process.
