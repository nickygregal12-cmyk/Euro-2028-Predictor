...
>
>
> **Current hosted boundary — contract 185 on both Supabase projects (12 August 2026):** Development and Production both name `20260812070000_ai_lab_operational_loop` as their latest migration and no schema migration is pending. Production `provider-poll` is ACTIVE at version 14. Paid odds collection is enabled only on Production at 500 monthly / 450 soft cap; Development is disabled. Both were at zero API usage, dispatch and raw-response rows when verified, and no live provider poll was used as a migration smoke test. Exact evidence lives in the two machine records linked below.
>
> **Contracts 186 and 187 repository candidates — the season Championship can finish (12 August 2026):** `20260812080000_cup_group_stage_span.sql` and `20260812090000_season_cup_qualification_driver.sql` close `CUP-002` and its prerequisite. **Both additive** — `check-migration-additive.mjs` accepts both, reporting one structural item it carries rather than hides: contract 186 drops and immediately re-creates its own immutability trigger, on the table the same migration creates. **Applied to no hosted environment by this record.**
>
> **Three things to know before promoting, each of which surfaces as a confusing failure rather than as a message about itself.** First, contract 187 patches `admin_finalise_predictor_cup_groups` **in place**, in contract 60's idiom, for contract 184's reason: no migration holds that function's current text, and restating it from contract 47 is how contract 184's first draft reverted contract 60 entirely. It fails closed on all six anchors, **counts** the two identical window lookups and the four table reads rather than merely finding them — a `position(...) > 0` check is satisfied by one occurrence, and replacing one of the two identical lookups would leave the playoff and the first knockout round reading different windows — and proves the round trip: reversing all six replacements must reproduce what was read, byte for byte. `create or replace function` preserves grants, so it touches none, and the migration re-asserts that the driver has not become browser-reachable. Second, contract 187 also patches **both season launchers** to set `bonus_competitions.draw_completed_at`, which neither has ever done: the tournament's `admin_draw_predictor_cup` sets it and the season launchers do not, so the qualification gate had been refusing every season Championship on its FIRST check. It refuses a Championship carrying `draw_required = false` rather than writing anyway, because `bonus_competitions_draw_shape` forbids the pair; measured, every `predictor_cup` row carries `draw_required = true`, so this refuses nothing that exists. Third, contract 186's backfill reads the launch plan out of `bonus_competition_audit` under **two different keys** — `leagueRounds` and `roundsNeeded` — handled explicitly rather than coalesced, and it reports how many drawn Championships it could NOT place, because those are the ones `CUP-002` will refuse. On a database rebuilt from every committed migration it places zero and refuses zero, since no season Championship has been launched anywhere.
...
| --- | ---: | --- | --- |
| Repository candidate | **189** | 189 canonical migrations through `20260813215920_ai_quarantined_evidence_reads.sql`, which takes quarantined forecasts and the bets advised from them out of every admin AI evidence read. Additive; function replacement plus one private view. | LEVEL WITH BOTH |
| Development Supabase `iouzoutneyjpugbbtdem` | **189** | Fast-lane run [31751114401](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/31751114401) from exact `main` `e2edf66`, success at 22:57 UTC. Confirmed by a query that **names** `20260813215920 ai_quarantined_evidence_reads`. A rolled-back hosted lifecycle probe proved the quarantine is dynamic: two forecasts with results and bets, both counted by all seven reads, one invalidated afterwards, every figure 2 → 1 on the next call and exposure 6.000 → 1.000. Paid collection disabled; zero API usage, predictions and bets. | LEVEL; PAID COLLECTION OFF |
| Production Supabase | **189** | `vkfnsqdyhvtwyqkisxhk`. Guarded rollout run [31752340686](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/31752340686) from exact `main` `e2edf66` at 23:05 UTC, gated on a FRESH backup [31751115908](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/31751115908) and rehearsal [31751938850](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/31751938850) — this boundary's own backup, not the 185-to-188 one. Independent postflight **names** the ledger row. Upcoming forecasts 92 → 55 with the 37 invalidated ids gone **by id**; open bets 49 → 14 and exposure 92.615 → 23.639. Underlying counts unchanged, 14 cron jobs and the four `ai-odds-*` schedules unchanged, Euro `hidden`, zero provider calls, paid collection deliberately left **ON**. | LEVEL; PAID COLLECTION ON |

**Contract 189 is applied to both environments and nothing is pending.** It is one additive migration, `20260813215920_ai_quarantined_evidence_reads.sql`: one private view (`ai.valid_bets`) and eight `create or replace` function replacements, no table, column, trigger, constraint, policy or grant moved and no row written. It exists because contract 188's quarantine was enforced on three reads and declared on all of them — measured against the **installed** Production definitions, four prediction reads still selected from `ai.predictions` and three betting reads still counted `ai.bets`/`ai.bet_results`, the publication gate joining nothing at all. It was masked only by `ai.prediction_results` holding zero rows, and the earliest quarantined kickoff is 2026-08-14T19:00Z, so it was applied inside that window rather than after it.
