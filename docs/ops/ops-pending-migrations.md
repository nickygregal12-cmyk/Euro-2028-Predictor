...
Two operational consequences, and the first is already live — contracts 98–107 landed after the declaration moved to 97, so previews are printing the "unavailable" line right now. Development is rolled out to 107, so moving the declaration to 107 is the correct next owner action:
...
Contracts 64–107 are all applied to development, with nothing pending. None is authorised for production merely to remove the intentional contract gap.
