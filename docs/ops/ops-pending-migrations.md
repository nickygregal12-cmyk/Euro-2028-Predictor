...
| --- | --- |
| below the repository contract | `Netlify deploy-preview database contract is <n> and the application requires 97: hosted database preview unavailable until the development rollout applies it.` |
...
Two operational consequences follow from the declaration now sitting level with the repository:

1. The next contract puts the non-production declaration behind again, and previews will print the "unavailable" line until Netlify is updated after that rollout. That is the normal cycle, not a fault.
...
3. **Done, owner-reported 4 August 2026:** the Netlify `dev`, branch-deploy and deploy-preview declarations moved to 97 after the development rollout was verified. Read the corroboration note above before relying on it, and move them again after the next development rollout.
