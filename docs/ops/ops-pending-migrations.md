...
## Current state — repository 203, Production 198, Development 202 (18 August 2026)

| Environment | Contract | Evidence | Status |
| --- | ---: | --- | --- |
| Repository candidate | **203** | 203 canonical migrations through `20260818080000_ai_bet_builder_current_decision.sql`. Contracts 199-203 are private AI Lab operational contracts — canonical betting evidence, the paid odds collection cadence, three bounded admin reads, and forecast horizons that let a fixture be re-forecast as results arrive. Below them, contracts 191-192 give a weekly-season standings row a player address, position over time and a season-long rivalry, contracts 193-194 close `CUP-003` and `CUP-004`, contracts 195-196 give the action centre its Championship and consequence generators, contract 197 adds the cross-competition football calendar, and contract 198 closes `CUP-006`. | ONE CONTRACT AHEAD OF DEVELOPMENT; FIVE AHEAD OF PRODUCTION |
| Development Supabase `iouzoutneyjpugbbtdem` | **202** | Contracts 199 to 202 applied through the ADR 0024 fast lane, run [32141009659](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/32141009659), from exact `main` `42f778f`, on top of contracts 191 to 198 applied by run [32119101708](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/32119101708). Independent read-only verification names `20260818070000 ai_forecast_horizons` beside a total of 202, and measures the properties the four contracts exist for rather than only their presence: every player-owned count unchanged across the apply (24 auth users, 24 profiles, 34 entries, 64 season predictions, 756 match predictions, 8 matchweek scores, 20 entrants, 5 leagues, 39 members, 1 Cup launch, 112 Cup fixtures, 578 season fixtures); Euro 2028 still `hidden`; paid odds collection still **disabled** with `ai.api_usage` at zero, so none of this cost a credit; the three new admin reads and the three new `ai` views authenticated-only with **nothing** reachable anonymously; every collection cadence tier strictly inside the matching price-freshness limit at zero exceptions; all eleven refusal codes carrying an explanation; `actual_result` nullable only behind the void-settlement branch; and `predictions_fixture_idx` present. **This row previously read 190, and before that 189 while the database already held 190 — contract 190 reached Development by a route this repository does not record, which remains unexplained rather than backfilled.** | ONE CONTRACT BEHIND REPOSITORY; PAID COLLECTION OFF |
