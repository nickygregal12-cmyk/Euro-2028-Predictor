...
## Current state — repository 205, Production 198, Development 204 (19 August 2026)
...
| Development Supabase `iouzoutneyjpugbbtdem` | **204** | Contract 204 applied through the ADR 0024 fast lane, run [32196292139](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/32196292139), from exact `main` `e896178`, on top of contract 203 applied by run [32174280080](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/32174280080), contracts 199 to 202 by run [32141009659](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/32141009659) and contracts 191 to 198 by run [32119101708](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/32119101708). Independent read-only verification names `20260818090000 ai_dashboard_league_scoped_quarantine` beside a total of 204, and measures the property contract 204 exists for rather than only its presence: **both** `admin_ai_dashboard` and `admin_ai_betting_dashboard` read the excluded-forecast count through `ai.quarantined_predictions` and **both** scope it with `p_league is null or q.league = p_league`, so a league-scoped fall is explained by a league-scoped figure. **Neither reaches `ai.predictions` directly** once line comments are stripped — contract 204 did not buy its scoping by breaking the custody rule suite 236 asserts, which is what made the first attempt at this fix wrong. The new view is present and readable by **no** browser role: `has_table_privilege` false for `anon` and `authenticated` alike, and schema `ai` still carries zero table grants to either. Every player-owned count unchanged across the apply (24 auth users, 24 profiles, 34 entries, 64 season predictions, 756 match predictions, 8 matchweek scores, 20 entrants, 0 bonus score events, 5 leagues, 39 members, 1 Cup launch, 112 Cup fixtures, 578 season fixtures); cron still 14 jobs; paid odds collection still **disabled** with `ai.api_usage` at zero, so this cost no credit; both publication gates still **false**. | ONE CONTRACT AHEAD OF PRODUCTION; LEVEL WITH REPOSITORY; PAID COLLECTION OFF |
