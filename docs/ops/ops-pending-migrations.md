...
## Current state — both hosted databases at 188 (13 August 2026)

| Environment | Contract | Evidence | Status |
| --- | ---: | --- | --- |
| Repository candidate | **188** | 188 canonical migrations through `20260813100000_ai_lab_multi_model_evidence.sql`. | LEVEL WITH BOTH |
...
| Production Supabase `vkfnsqdyhvtwyqkisxhk` | **188** | Guarded rollout run [31739706612](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/31739706612) from exact `main` `1510a6e3` at 20:13 UTC, gated on fresh backup [31730225719](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/31730225719) and rehearsal [31739287772](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/31739287772). Independent postflight at 20:14:11Z **names** the same three rows. Protected counts unchanged, 14 cron jobs before and after, Euro `hidden`, zero provider calls. **Paid collection remains deliberately DISABLED** since 2026-08-13T15:09Z, reason `KNOWN_IDENTITY_PIPELINE_CORRUPTION`. | LEVEL; PAID COLLECTION OFF |

**The repair is installed and uncalled.** `ai.provider_identity_repairs`, `ai.prediction_invalidations` and `ai.recommendations` all hold zero rows on Production after the apply. Contract 188 deliberately does not repair on arrival — a migration that did would make the repair unauditable — so running `ai.repair_provider_identity` is a separate operator action against a Production that now holds the authority.
