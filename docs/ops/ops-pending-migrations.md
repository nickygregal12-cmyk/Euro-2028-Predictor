...
| --- | ---: | --- | --- |
| Repository candidate | **190** | 190 canonical migrations through `20260814005000_ai_actionable_bet_evidence.sql`; Contract 190 makes actionable bet evidence real-bookmaker-only while retaining historical aggregate rows in the base tables. | LEVEL WITH PRODUCTION; DEVELOPMENT ONE CONTRACT BEHIND |
| Development Supabase `iouzoutneyjpugbbtdem` | **189** | Hosted Contract 189 remains the Development boundary. No Contract-190 Development rollout is claimed by this Production close-out. | ONE CONTRACT BEHIND REPOSITORY; PAID COLLECTION OFF |
