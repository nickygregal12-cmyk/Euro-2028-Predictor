...
## Current state — repository 196, Production 190, Development 189 (18 August 2026)

| Environment | Contract | Evidence | Status |
| --- | ---: | --- | --- |
| Repository candidate | **196** | 196 canonical migrations through `20260818010000_game_consequence_actions.sql`; contracts 191-192 give a weekly-season standings row a player address, position over time and a season-long rivalry, contracts 193-194 close `CUP-003` and `CUP-004`, and contracts 195-196 give the action centre its Championship and consequence generators. | SIX CONTRACTS AHEAD OF PRODUCTION; DEVELOPMENT SEVEN BEHIND |
| Development Supabase `iouzoutneyjpugbbtdem` | **189** | Hosted Contract 189 remains the Development boundary. No Development rollout of contracts 190 to 196 is claimed by this record. | SEVEN CONTRACTS BEHIND REPOSITORY; PAID COLLECTION OFF |
| Production Supabase | **190** | `vkfnsqdyhvtwyqkisxhk`; guarded rollout [31807942945](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/31807942945) from exact `main` `c54d5b2`, gated on encrypted backup [31785056556](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/31785056556) and fresh exact-head rehearsal [31807550563](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/31807550563). Independent postflight names `20260814005000 ai_actionable_bet_evidence`; 49 historical bets retained, 0 actionable valid bets at verification, 0 synthetic actionable evidence, provider usage unchanged at 10 and all four paid-odds schedules still active. | SIX CONTRACTS BEHIND REPOSITORY; PAID COLLECTION ON |

**Contracts 191 to 196 are applied nowhere; contract 190 is applied to Production; Development remains at Contract 189.** All six are repository candidates only — seven pending migrations now separate Development from the repository, and six separate Production from it. **Contract 190 is applied to Production; Development remains at Contract 189.** Production Contract 190 is the single additive `20260814005000_ai_actionable_bet_evidence.sql` boundary: it keeps historical bet rows immutable while excluding synthetic aggregate/reference books from actionable betting evidence and refusing future unbettable aggregate writes. Development has not been promoted to 190 in this close-out, so the repository and Production are level at 190 while Development is one contract behind. That is an explicit hosted-state difference, not a hidden pending Production migration.
