...
- **103:** A competition can happen more than once (ADR 0025 decision 1, prerequisite). `unique (tournament_id, game_key)` was an *availability* key doing an *instance* row's job, which is why a restart was unrepresentable rather than merely unimplemented. It becomes a partial unique index over **live** instances, and `bonus_competitions` gains series lineage plus a `completion_reason`. Nothing in it can create a second instance — that is contract 105, after contract 104 moves the tournament+game readers onto the live-instance resolver.
