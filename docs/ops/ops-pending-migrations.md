...
| Production Supabase | **198** | `vkfnsqdyhvtwyqkisxhk`; guarded rollout [32126890172](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/32126890172) from exact `main` `3751dc1`, gated on encrypted backup [32121831688](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/32121831688) and fresh exact-head rehearsal [32126447952](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/32126447952). **Eight migrations, not one** — the boundary was proven as an exact set, each additive, and the dry run diffed against that set. **ADR 0024 ordering enforced on a Production workflow for the first time**: both workflows refuse unless Development is at or beyond the target, and Development reached 198 first through fast-lane run [32119101708](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/32119101708). Independent postflight names `20260818030000 cup_knockout_reservation` beside a total of 198. Every human-owned count byte-identical across the apply; cron fingerprint unchanged; Euro publication still `hidden`; paid collection still on. Five new reads authenticated-only with none anonymous; the two generators and `cup_knockout_rounds` executable by nobody; contracts 102, 194, 196 and 198 all present at once on the installed Championship functions. | LEVEL WITH REPOSITORY; PAID COLLECTION ON |

**Contracts 191 to 198 are applied to both hosted environments, and contracts 199 to 204 are applied to Development only.** This paragraph described the 190 boundary and was left behind when the table above moved twice in one day; it is corrected here rather than deleted, because the sequence it recorded is the reason the table is trustworthy. Contract 190 was the single additive `20260814005000_ai_actionable_bet_evidence.sql` boundary, which keeps historical bet rows immutable while excluding synthetic aggregate/reference books from actionable betting evidence and refusing future unbettable aggregate writes. Development reached 198 through the ADR 0024 fast lane and Production followed it, in that order, which the Production workflows now refuse to invert. Contracts 199 to 202 have since taken the same route in the same order — Development first, through fast-lane run [32141009659](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/32141009659) — and contract 203 is a repository candidate only.
