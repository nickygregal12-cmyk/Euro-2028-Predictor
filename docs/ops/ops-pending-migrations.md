...
## Current state — repository 192, Production 190, Development 189 (17 August 2026)

| Environment | Contract | Evidence | Status |
| --- | ---: | --- | --- |
| Repository candidate | **192** | 192 canonical migrations through `20260817130000_season_rank_history_and_rivalry.sql`; contract 191 gives a weekly-season standings row a season-scoped player address, and contract 192 adds position over time and one season-long rivalry over it. | TWO CONTRACTS AHEAD OF PRODUCTION; DEVELOPMENT THREE BEHIND |
| Development Supabase `iouzoutneyjpugbbtdem` | **189** | Hosted Contract 189 remains the Development boundary. No Development rollout of contracts 190, 191 or 192 is claimed by this record. | THREE CONTRACTS BEHIND REPOSITORY; PAID COLLECTION OFF |
| Production Supabase | **190** | `vkfnsqdyhvtwyqkisxhk`; guarded rollout [31807942945](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/31807942945) from exact `main` `c54d5b2`, gated on encrypted backup [31785056556](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/31785056556) and fresh exact-head rehearsal [31807550563](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/31807550563). Independent postflight names `20260814005000 ai_actionable_bet_evidence`; 49 historical bets retained, 0 actionable valid bets at verification, 0 synthetic actionable evidence, provider usage unchanged at 10 and all four paid-odds schedules still active. | TWO CONTRACTS BEHIND REPOSITORY; PAID COLLECTION ON |

**Contracts 191 and 192 are applied nowhere; contract 190 is applied to Production; Development remains at Contract 189.** Both are repository candidates only — three pending migrations now separate Development from the repository, and two separate Production from it. **Contract 190 is applied to Production; Development remains at Contract 189.** Production Contract 190 is the single additive `20260814005000_ai_actionable_bet_evidence.sql` boundary: it keeps historical bet rows immutable while excluding synthetic aggregate/reference books from actionable betting evidence and refusing future unbettable aggregate writes. Development has not been promoted to 190 in this close-out, so the repository and Production are level at 190 while Development is one contract behind. That is an explicit hosted-state difference, not a hidden pending Production migration.
