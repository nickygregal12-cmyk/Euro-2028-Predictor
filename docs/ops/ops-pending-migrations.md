...
The repository is at **contract 106**; development is at **105**, applied by fast-lane run 30968263589, and trails by one until the next fast-lane run.
...
| Development Supabase `iouzoutneyjpugbbtdem` | **105** | Applied 5 August 2026 by fast-lane run 30968263589 on `03613eb`, carrying contracts 104 and 105 together. Corroborated read-only: the ledger holds 105 rows through `20260805010000`. `cup_split_group_tables`, `assert_bonus_cup_member_split_parent`, `current_public_competition_id` and `live_competition_id` all exist in `predictor_internal` and **none** is present in `public`; `anon`, `authenticated` and `service_role` hold no EXECUTE on the split authority or its ancestry guard. Both split integrity triggers are attached. Nothing has used the new stage yet — **zero** split groups, split members, split fixtures, ancestry violations and completed competitions — so every existing reader still resolves the same live public row, and the ancestry rule was enforceable the moment it landed rather than needing a backfill | VERIFIED; ONE CONTRACT BEHIND THE REPOSITORY |
...
Two operational consequences, and the first is already live — contracts 98–105 landed after the declaration moved to 97, so previews are printing the "unavailable" line right now. Development is rolled out to 105, so moving the declaration to 105 is the correct next owner action:
...
Contracts 64–105 are applied to development; contract 106 is not yet. None is authorised for production merely to remove the intentional contract gap.
