...
| Netlify `euro28predictor` non-production contexts | **65 declared / 79 Development** | `dev`, branch-deploy and deploy-preview point to the development Supabase project; `EURO28_DEPLOYED_DB_CONTRACT` is stale but informational under ADR 0024 | ACTIVE SITE; DECLARATION REQUIRES ALIGNMENT |
...
1. Align Netlify `dev`, `branch-deploy` and `deploy-preview` `EURO28_DEPLOYED_DB_CONTRACT` declarations to the verified Development contract, then run exact-origin non-production smoke.
2. Keep the Development machine record and this inventory aligned after every later migration rollout.
