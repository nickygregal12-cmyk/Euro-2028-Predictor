...
| --- | ---: | --- | --- |
| Repository `main` | **101** | Contract 101 makes the Cup split a distinct persisted stage, through `20260804293000_cup_split_stage_persistence.sql` | MERGED, AWAITING DEVELOPMENT ROLLOUT |
...
- **101:** The Cup split becomes a distinct persisted stage (`stage = 'split'`), the thing contract 79 deliberately stopped short of. `bonus_cup_groups` gains `phase_kind` and `parent_group_id`, split membership lands in its own `bonus_cup_split_members` rather than widening `bonus_cup_members`' key, and `matchday` keeps counting past the league phase. The tournament draw was proven byte-identical across contracts 100 and 101.
