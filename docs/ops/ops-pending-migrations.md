...
## Current state — repository 204, Production 198, Development 203 (18 August 2026)
...
| Development Supabase `iouzoutneyjpugbbtdem` | **203** | Contract 203 applied through the ADR 0024 fast lane, run [32174280080](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/32174280080), from exact `main` `6288420`, on top of contracts 199 to 202 applied by run [32141009659](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/32141009659) and contracts 191 to 198 by run [32119101708](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/32119101708). Independent read-only verification names `20260818080000 ai_bet_builder_current_decision` beside a total of 203, and measures the property contract 203 exists for rather than only its presence: **both** `admin_ai_bet_builder_candidates` and `admin_ai_bet_builder_books` take their rows from `ai.current_fixture_recommendations`, so a superseded BET cannot be returned as an actionable leg. Every player-owned count unchanged across the apply (24 auth users, 24 profiles, 34 entries, 64 season predictions, 756 match predictions, 8 matchweek scores, 20 entrants, 0 bonus score events, 5 leagues, 39 members, 1 Cup launch, 112 Cup fixtures, 578 season fixtures); cron still 14 jobs; paid odds collection still **disabled** with `ai.api_usage` at zero, so this cost no credit; both bet-builder reads authenticated-only with **no** anonymous execute, the three `ai` views readable by no browser role, and schema `ai` unreachable from `anon` and `authenticated` alike. **This row previously read 190, and before that 189 while the database already held 190 — contract 190 reached Development by a route this repository does not record, which remains unexplained rather than backfilled.** | ONE CONTRACT BEHIND REPOSITORY; PAID COLLECTION OFF |
