# Provider-ingestion custody — contract 86

Contract 86 adds a server-only evidence boundary for football-provider responses. It deliberately does not make provider data authoritative competition state.
...
After merge, apply the additive migration to Development through `.github/workflows/development-fast-lane-rollout.yml`, verify the ledger at 86, then align non-production declarations. Production remains at contract 63 and has no fast lane.
