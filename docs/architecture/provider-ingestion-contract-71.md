# Provider ingestion contract 71
...
Contract 71 adds the minimum safe custody boundary for external football fixture and result providers. It follows contract 70's independent domestic Main Predictor scoring authority. Provider evidence remains separate from application fixture, result and scoring authority.
...
Provider custody was originally prepared while contracts 68, 69 and 70 were being claimed by concurrent work. Contract 68 became the season-fixture container, contract 69 became the season card and contract 70 became Main Predictor scoring with TypeScript/PostgreSQL parity. Contract 71 is rebuilt from the exact merged contract-70 head and preserves all three migrations unchanged.
...
A first hosted Development poll requires a separate guarded rollout after contracts 70 and 71 are hosted, the named key and provider secret are configured, and unauthorised access is proved against the hosted function. Production remains prohibited.
