...
**Status:** Executable C1/C1b after-state contract  
**Date:** 3 August 2026  
...
This document classifies every assertion in the seven Stage C executable suites and the supporting account-deletion before-state suite. The Stage C suites now describe the contract-66 C1b after-state; the shared C2 boundary remains frozen.
...
- **`C1`** — belongs to the competition-season foundation or its C1b game-catalogue and membership boundary, and proves the approved contract-66 after-state.
...
- **`shared-before-state`** — freezes current auth ownership and deletion behaviour. C1/C1b must keep it unchanged so C2 retains an exact starting point.
...
| --- | --- | --- |
| `stageCRelationCoverage.test.ts` | `C1` | keeps parser positive controls at the contract-81 schema boundary |
...
| `stageCTriggerBindingCoverage.test.ts` | `C1` | keeps the parser positive control at the contract-81 trigger boundary |
...
| `stageCTournamentIdCompatibility.test.ts` | `C1` | keeps the parser positive control at the contract-81 boundary |
...
| `accountDeletionSemantics.test.ts` | `shared-before-state` | names the references that cascade with identity deletion |
...
The two pgTAP suites and the executable relation/function/trigger inventories prove the approved C1/C1b after-state. Reference-scope protections remain hostile-write guards, while lock assertions prove fail-closed missing entry timing, append-only observed lock evidence, inclusive fixture and entry boundaries, and non-reopening reschedules.

The account-deletion suite is different: every one of its assertions remains a shared before-state through C1b. Any edit to its expected foreign-key matrix, profile dependency state or ownership consequences belongs to C2 and is blocked by issue #272.
