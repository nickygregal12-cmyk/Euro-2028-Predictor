# Provider ingestion contract 72
...
Contract 72 adds the minimum safe custody boundary for external football fixture and result providers. It follows contract 70's independent domestic Main Predictor scoring authority and contract 71's LMS survival-resolution parity. Provider evidence remains separate from application fixture, result and scoring authority.
...
Provider custody was originally prepared while contracts 68, 69, 70 and 71 were being claimed by concurrent work. Contract 68 became the season-fixture container, contract 69 became the season card, contract 70 became Main Predictor scoring parity and contract 71 became LMS survival-resolution parity. Contract 72 is rebuilt from the exact merged contract-71 head and preserves all four migrations unchanged.

The decoder's internal format label may retain its original reviewed draft version. That label identifies the normalized evidence format, not the database migration number.

## Explicit non-authority

Contract 72 does not:
...
A first hosted Development poll requires a separate guarded rollout after contract 72 is hosted, the named key and provider secret are configured, and unauthorised access is proved against the hosted function. Production remains prohibited.
