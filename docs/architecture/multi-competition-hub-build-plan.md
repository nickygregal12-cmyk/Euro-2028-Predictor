...
> **Contract 208 stage effect:** the bracket read states the caller's own outcome and stops offering a split fixture as a knockout tie. No hub build-stage scope changes.

> **Contract 209 stage effect:** the phase read answers about a split entrant's current group rather than their pre-split one. No hub build-stage scope changes.
...
> **Contract 211 stage effect:** none on any hub surface either. It makes contract 210's coverage affordable by moving the long reach onto a separate hourly tier instead of the ten-minute live one. No read, no shape and no build-stage scope changes.
