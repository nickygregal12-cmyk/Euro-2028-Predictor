...
| `provider_poll_targets.touch_provider_poll_target` | `predictor_internal.touch_provider_poll_target` | stamp `updated_at` when a poll target changes (contract 114); cadence and enablement are operational settings, and the only way to tell later whether a quiet target was disabled deliberately or drifted is the date somebody last touched it |
