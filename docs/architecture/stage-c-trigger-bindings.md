...
| `season_fixtures.season_fixtures_mark_round_window_stale_ins` | `predictor_internal.mark_round_window_stale` | contract 123; a fixture added changes the round's play span, so its window is out of date. Writes one timestamp and evaluates nothing — it cannot refuse an insert, which is the property that keeps ingestion reliable |
| `season_fixtures.season_fixtures_mark_round_window_stale_upd` | `predictor_internal.mark_round_window_stale` | contract 123; scoped to `kickoff_at` and `competition_round_id`, because a score or status write does not move a span. Marks BOTH sides when a fixture moves between rounds. Marking is not re-deriving: contract 117's import is untouched and the guarded refresh decides separately whether the new span can be taken |
| `season_fixtures.season_fixtures_mark_round_window_stale_del` | `predictor_internal.mark_round_window_stale` | contract 123; a fixture removed shrinks or empties the span, and a round with none left has no window at all |
