...
- the Contract 120 season play-matchweek read: the season Match Predictor surface has been production code since UI-04 and reachable only from `/dev/season-predictor`, because `get_season_matchweek_card` takes the matchweek as an argument and nothing could tell the caller what to pass — `competition_rounds` is revoked from the browser. Contract 120 exposes `predictor_internal.next_eligible_league_round`, which contract 109 already uses for the Last Man Standing successor calendar, and adds no scheduling logic of its own. It is the last database gap between that finished surface and a registered route;
