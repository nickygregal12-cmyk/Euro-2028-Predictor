...
- the Contract 110 season Predictor Championship round calendar. Until it, `bonus_cup_fixtures.window_id` was `NOT NULL` and nothing in the repository created a window for a season competition, so no season Championship fixture could be persisted in either phase — which is why its phase-transition driver could not be built.
