...
- the Contract 120 season play-context read: the season Match Predictor surface has been production code since UI-04 and reachable only from `/dev/season-predictor`, because `get_season_matchweek_card` takes the matchweek as an argument and nothing could tell the caller what to pass — `competition_rounds` is revoked from the browser. It also could not tell which season two URL slugs meant: `competitions.slug` is revoked from the browser too, and two seasons share `season_key = '2026-27'`. Contract 120 answers both in one read — resolving on `(competitions.slug, tournaments.season_key)`, a real composite key — and exposes `predictor_internal.next_eligible_league_round`, which contract 109 already uses for the Last Man Standing successor calendar, adding no scheduling logic of its own. It was the last database gap between that finished surface and a registered route, and **the route is now registered** at `/competitions/:competitionSlug/:seasonSlug/main-predictor` behind `VITE_UI_SEASON_MATCH_PREDICTOR`. The flag stays off until contract 120 reaches a hosted environment: the route depends on a read no hosted database has;
