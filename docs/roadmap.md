...
3. Complete the PR #317 migration and recovery review, then merge. The review found and fixed two defects: an unqualified PostgREST embed that the migration's composite keys made ambiguous, and an entry lock made permanent by consulting lock events. Both are on the branch and both are recorded in [`quality/current-status.md`](quality/current-status.md). Outstanding: a green browser suite, and reconciling the branch with `main` — a test merge applies cleanly with typecheck, lint and the full Vitest suite green.
