...
- [ ] **Scoring engine completion** (§2 group positions / §3 knockout progression / §4 awards → `score_events`) — **the single most important remaining Phase 2 correctness item; blocks the Phase 3 dress rehearsal.** The engine (`20260720130000_add_scoring.sql`) scores §1 group matches in SQL today; §2/§3/§4 are deferred to a TS-domain invocation because they need *derived actuals* (group order via tie-breaks, resolved bracket) that only the unit-tested TS domain produces — reimplementing them in SQL would drift (rule 6). They score 0 on current data (no full group complete, no KO result), so §1-only is byte-identical to the full pipeline *for now* — but this **becomes urgent the moment a full group's results exist** (§2 turns non-zero), and again on KO results (§3) and awards (§4). Mechanism is still TBD (the TS pipeline invoked server-side vs. SQL against stored derived actuals — the latter rejected as drift-prone). The seed's pure pipeline (`scripts/seed-dev/scoreEntries.ts`) is the reference implementation and equality with it is the acceptance test (`tests/scripts/scoreEntries.test.ts`). *(Tracked in `build-todo.md` under Phase 2.)*
...
### Phase 2 exit gates
- [ ] **Separate production Supabase project** — fresh migrations, real data; dev project keeps the test mess; Netlify env vars switch to prod project. **Applying the full ordered migration set is part of this — see `docs/ops-pending-migrations.md` (the consolidated pending-migrations checklist).**
- [x] Auth hardening complete (pure-code items, SMTP, Turnstile, password reset; email-verification decided OFF for now)
- [ ] **Single-tester entry-flow test**: ONE trusted person, run against a test script Nicky defines (sign up → full entry → join league → specific friction checkpoints), findings reported back and triaged before Phase 3. This is a friction check, not a launch — the real all-hands test is the Phase 3 dress rehearsal. **Script written: `docs/test-script.md`** (silent-observation tasks + follow-up questions + note-taking shorthand); the run + triage remain the gate. **Prerequisite: the pending migrations (`docs/ops-pending-migrations.md`) must be applied to the dev DB first — otherwise leagues/scoring aren't live and the friction test isn't meaningful.**
