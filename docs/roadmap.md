...
### Phase 2 exit gates
- [ ] **Separate production Supabase project** — *in flight (repo side ready 2026-07-21).* Fresh migrations, real data; dev project keeps the test mess; Netlify env vars switch to prod project. **The full manual runbook is `docs/ops-prod-cutover.md`** (ordered checklist Nicky executes: Resend key rotate → create prod project → apply all migrations → verify → `seed.sql` + `prod-baseline.sql` → Auth config → admin bootstrap → Netlify env swap + smoke test → revert dev Turnstile secret). Repo pieces landed: the dev-ref-pinned auto-login/seed guards, `supabase/prod-baseline.sql`, and the prod applied-state tracking + verification query in `docs/ops-pending-migrations.md`. Remaining = Nicky running the runbook. **Dev is fully applied; a fresh prod project needs the whole set from scratch.**
