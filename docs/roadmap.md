...
7. **B7 — Predictor Cup (staged B7a–B7c):**
   ...
   - **B7b — group-stage scoring (delivered; production-released at contract 55):** derives everything at read time from confirmed results — per-window prediction points (raw 5/3/0, regulation only, Original entry for group fixtures and the shared store for knockout fixtures), settled head-to-head results with walkover/void, and ranked group tables (§5.2 steps 1–3, 6–8; the mini head-to-head steps 4–5 apply at the B7c qualification gate);
   - **B7c — qualification and knockouts (delivered, development only):** contract 56 adds the audited service-role qualification gate (full §5.2 tables including the mini head-to-head, automatic places, per-game wildcard ranking, banded seeding, playoff with byes and same-group avoidance, fixed canonical bracket), parity-laned Penalty Numbers (home/better seed odd, away even; per-kickoff lock, optimistic versions), the audited round settle (points → Extra-Time Accuracy → Penalty Number → walkover, elimination, next-round creation), the champion and the read-derived Golden Predictor, with the bracket/penalty/honours surfaces at `/games/cup`.
