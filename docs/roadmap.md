...
| 18 | Predictor Championship backend completion (`CUP-001`–`CUP-005`) | **`CUP-005` closed as contract 182 and `CUP-001` as contract 184, both 12 August 2026; `CUP-002`–`CUP-004` remain unbuilt and are unblocked rather than blocked.** `CUP-001`'s table is published in [`predictor-cup-rules.md`](predictor-cup-rules.md) § 6.2 as arithmetic covering sizes 3 to 20, reproducing the tournament's own two rows exactly. **What `CUP-002` must still do:** move the finalisation gate off `win.sequence between 1 and 3` — the tournament's three matchdays, against a season group stage of thirty-eight — and create the knockout windows a season has none of, which the same function already refuses on by name. **Re-measured 12 August 2026 against contract 184, and the "precisely specified" description above was incomplete.** Three facts, read off the installed catalogue rather than the migrations. **(1)** `admin_finalise_predictor_cup_groups` hard-codes the tournament's shape in three places, not one: the settle gate `win.sequence between 1 and 3`, the knockout-window existence check `win.sequence between 4 and 3 + v_needed`, and the two lookups that read the playoff and first-round windows as `sequence = 4`. **(2)** `predictor_internal.generate_season_cup_windows(competition_id, after, rounds)` appends `rounds` windows continuing from `max(sequence)`, so it is already a general appender and can create knockout windows unchanged — that half is cheaper than it looks. **(3) The blocker is that the group-stage span is stored NOWHERE.** `launch_season_cup` computes `leagueRounds` from `select_season_cup_format`, passes it to the window generator and discards it; no column on any `bonus_*` relation holds it, and there is no launch-record table. So nothing can answer "where does this competition's group stage end", which every one of the three hard-coded places needs. **Deriving it after the fact is ambiguous rather than merely awkward**: contract 124's split writes further fixtures into later windows, so counting windows or counting fixtures both stop being the group-stage span once a competition has split. **`CUP-002` therefore has a prerequisite the description above omits: persist the group-stage span at launch** — one additive column written by `launch_season_cup` and `launch_season_cup_groups`, backfilled for the competitions that already exist — before any gate can read it. Only then are the three hard-coded places safely replaceable. ADR 0028 §§ 6–8 removed the owner blocker from all four. `CUP-005` is done and was smaller than the register implied: measured on the installed catalogue, `settle_season_cup_tie` is reached by **nothing**, so the second authority was potential rather than actual, and contract 182 makes that permanent with a guard that fires on the conjunction ADR 0028 forbids. **What remains, in dependency order:** `CUP-001` must extend `predictor-cup-rules.md` § 6.2 — which covers group sizes **3 and 4 only** — to every size from 5 to 20, reconciled against § 6.1's `round(entrants × 2 ÷ 3)` target and § 7.1–7.3's seeding, bracket arithmetic and avoidance rules; only then can `CUP-002` build the driver, which must also create knockout windows rather than assuming tournament sequences 4+ exist, because `admin_finalise_predictor_cup_groups` still gates on exactly three settled windows and a season group stage runs to thirty-eight. `CUP-003` (Penalty Number read) and `CUP-004` (walkover/withdrawal, with no invented score or prediction points) are independent of `CUP-001` and could be taken first |
...
| 20 | Private model and paper-betting laboratory | **Implemented in the repository as contract 185; hosted rollout pending.** Nine leagues share one fixture/prediction/grade/value/settlement/CLV lifecycle. Free Football-Data.co.uk history and twice-weekly fixture prices are the broad coverage lane; the paid Odds API runs through the secret-safe Edge boundary with budget evidence and collection disabled by default. SportDB.dev remains behind its capability/licensing audit. This is not the player-facing Innovation Lab and has no platform result or scoring authority |
