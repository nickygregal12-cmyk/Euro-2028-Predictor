...
- [~] **Auth hardening** (one combined build) — pure-code items + SMTP + Turnstile frontend done; password reset ready to build, Turnstile go-live (dashboard) + the email-verification decision remain:
  ...
  - [~] Cloudflare Turnstile on sign up / log in — **frontend done, dormant** (Option A: Supabase built-in CAPTCHA; widget on both forms with `data-action="turnstile-spin-v2"`, token threaded to auth calls, gated on `VITE_TURNSTILE_SITE_KEY`). **To go live, Nicky: (1) set the site key in Netlify env, (2) Supabase Auth → CAPTCHA = Turnstile + paste the secret — the two must move together.** Dev auto-login uses `VITE_TURNSTILE_DEV_TOKEN` (CF test token).
...
- [ ] **Admin result-entry page** (moved from Phase 2; **must exist before the dress rehearsal**) — protected route, result entry with scoring-impact preview before confirm, correction flow. Deferred from Phase 2 deliberately: build it once against the final feature set (including what the bonus games need from results) rather than accreting features onto an early version. Result correction is already safe via the delete-and-rederive recompute, and Phase 2 uses `docs/ops-result-entry.md` (SQL entry) in the meantime.
