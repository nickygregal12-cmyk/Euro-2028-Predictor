...
- [ ] Minimal admin result-entry page + correction flow (scoring-impact preview before confirm; no double-counting on re-entry) — **recompute foundation in place**: entering/correcting a result (via SQL for now) already cascades to fresh, non-double-counted scores through the result trigger; the admin UI + scoring-impact preview are what's left.
