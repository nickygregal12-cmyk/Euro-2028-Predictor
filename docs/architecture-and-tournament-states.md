...
Each competition independently reports one of: `not_open | registration_open | entered | registration_closed | waiting_for_draw | waiting_for_round | action_required | round_live | round_awaiting_confirmation | qualified | survived | eliminated | champion | complete`. These render as Games-hub cards and Home action cards.

**Precedence (canonical — ADR-0010 decision 3, 28 July 2026).** When several states are simultaneously true, `resolveCompetitionStatus` reports the highest of, in order: `complete` → `champion` → `eliminated` → the non-entrant registration view (`not_open` / `registration_open` / `registration_closed`) → `action_required` → `round_live` → `round_awaiting_confirmation` → `waiting_for_draw` → `qualified` / `survived` → `waiting_for_round` → `entered`. This mirrors the §6 Home action queue so the Games hub and Home can never disagree about what a user should do next. A window settles only when its settle instant has passed **and** every fixture is officially confirmed — `full_time_unconfirmed` never settles, scores, eliminates or progresses anything. Original-entry states include the **spectator** (`no_valid_entry` post-lock): browse everything, join leagues as no-entry member, view public post-lock entries, enter bonus games with open registration; never in Original standings (per the decided spectator state).
...
All official result mutations pass through one admin-only `confirm_match_result` operation: validate admin → **acquire the per-tournament advisory lock** (audit follow-up item 5) → validate fixture + result (+ ET/pens data where relevant) → store confirmed score → derive real winner → recompute Original → recompute KO Predictor → resolve affected Cup fixtures → resolve affected LMS selections → update rank history → update real tables + bracket progression → append audit entry → return a scoring-impact summary. Corrections use the same operation with old/new recorded. **Nothing ever awards points from a live-feed update.** (The Original Predictor slice of this is **shipped**: browser-authorised `admin_confirm_match_result` / `admin_correct_match_result` / `admin_clear_match_result` wrap the authoritative lifecycle with the advisory lock, serialized recompute and immutable revisions — migrations `20260723183000` and `20260727075922`. The multi-competition fan-out — KO Predictor, Cup, LMS recompute — is the Phase 4 platform-generalisation and remains future work.)
