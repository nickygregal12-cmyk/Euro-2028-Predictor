...
- **The purchased Euro domain is a retained tournament property, not the universal account brand.** Recorded by [ADR 0026](0026-public-site-separation-shared-accounts-and-euro-2028-acquisition.md) on 6 August 2026: `euro28predictor.com` serves the Euro 2028 site (`SITE-004`), while the weekly platform waits for the umbrella-brand domain this record's trigger produces (`SITE-003`). **The brand decision remains deferred and this does not pre-empt it** — it decides only that a tournament domain does not become the year-round product's address by default, which is what would otherwise happen through inaction. The transactional sender identity moves to the umbrella brand at the same time (`SITE-007`), through the checklist in [`../auth-plan.md`](../auth-plan.md) § 5. The rename procedure below — Supabase redirect allow-list first, verification, then removal of old entries — is unchanged and now runs against two live origins rather than one replacing another.
