...
| [0020](0020-football-prediction-hub-product-model.md) §79 | "the round its new kickoff falls in" | **Accepted with an undefined term.** The ADR never says what *falls in* means, and the two readings behave differently: a round bounding the time it is played leaves gaps, so a midweek reschedule refuses; rounds tiling the calendar always resolve but require choosing where a boundary sits. Contract 113 implements the first because it invents nothing. **Widening to the second is an open owner decision** — it is lock-critical by the ADR's own §201 |
