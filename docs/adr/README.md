...
| [0012](0012-season-predictor-rules.md) | Season Predictor rules | Accepted direction — partially implemented: rules, storage, recurring processing, scoring, cumulative standings backend and the Contract-114 bounded card read/writes are merged, and Contract 120 supplies the season's play context — resolving which season a URL means, which no browser read could do, and which matchweek the card opens at by exposing the next-eligible-round authority rather than adding a second one; the phone card surface exists behind its route flag (UI-04) and the complete season surfaces remain |
...
Contract 118 applies ADR 0022's generalisation to the games hub read: `get_bonus_games` reads neutral window fixture facts rather than the tournament relation, on the shape contract 98 used for the Cup RPCs.
