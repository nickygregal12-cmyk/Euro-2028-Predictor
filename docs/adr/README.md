...
| [0020](0020-football-prediction-hub-product-model.md) §Ingestion | Provider ingestion and notifications | Accepted — **the identity map is merged at Contract 112, the round window at Contract 113 and the outbound poll dispatch at Contract 115 and the automatic kickoff revision at Contract 117 and the rescheduled-fixture lock at Contract 118** — the database can now call the deployed Edge Function on a schedule, which it previously could not do at all; the automatic fixture import and administrative reassignment the ADR decides on remain unbuilt |
