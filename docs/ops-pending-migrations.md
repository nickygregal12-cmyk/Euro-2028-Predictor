...
| --- | ---: | --- | --- |
| Repository | 42 | 42 canonical files through `20260727182300` | none in development |
| Development Supabase `iouzoutneyjpugbbtdem` | 42 | exactly 42 canonical versions through `20260727182300` | verification only |
| Netlify `dev`, `branch-deploy`, `deploy-preview` | 42 | development Supabase | none |
| Production Supabase `vkfnsqdyhvtwyqkisxhk` | 38 | exactly 38 canonical versions through `20260727080159` | migrations 39–42 deferred to a later approved milestone |
| Netlify `production` | 38 | production Supabase; verified milestone deploy is locked | no change approved |

Production is a controlled future-tournament target, not an active tournament. Contracts 39–42 are intentionally development-only. Do not update the production database, production Netlify contract declaration or locked production release without explicit owner approval and the full milestone gate.

## Migration 37–42 hosted history
...
The repository migration filenames for 39–42 match the exact canonical versions recorded by development Supabase. Do not renumber or reapply them under another timestamp.
...
- development Supabase records `20260727182300_bounded_read_models` as its latest migration;
