...
| `UI-F07` | Player & League Insights: comparison, profiles, rank movement and percentile, Rival Watch, post-lock predictions, reusable points explanation | **Partial** — consensus, one-matchweek head-to-head, the rival-gap line, **rank percentile** and the **reusable points explanation** are delivered. Percentile is arithmetic over the server's rank and field size and needed no history; movement does need history and stays blocked. League-wide named predictions (`MIG-UI-01`), profiles (`MIG-UI-02`) and movement (`MIG-UI-03`) are backend-blocked |
...
| `UI-F11` | Matches & Match Centre: dedicated journey, consequences, league comparison, consensus, movement | **Partial** — the addressable route is delivered, every fixture list links into it, and it now composes: football and the player's own side in the main column, the anonymous per-fixture consensus beside it in four distinct states. League comparison and movement need `MIG-UI-01`/`MIG-UI-03`, and an exact fixture read is `MIG-UI-11` |
...
| `UI-F13` | Create / Join: one wizard, universal join code | **Partial** — `/leagues` now offers both. Create is the four-step journey over `create_game_league`, offering exactly what that function accepts (Match Predictor in a competition the player already plays) and explaining the other two games rather than rendering forms that fail on submit. Join needed no wizard: `join_league` resolves the game from the invite code itself, so a code is enough. The UNIVERSAL resolver — a code for any container, including private LMS and Championship — remains `MIG-UI-07`, and the two private-game containers remain `MIG-UI-05`/`MIG-UI-06` |
...
## 14. Visual baselines outstanding

Eight gallery sections are declared in `e2e/visual-gallery.spec.ts` with no baseline images yet: `desktop-navigation-rail`, `lms-form-guide-panel`, `fixture-consensus-panel`, the three onboarding sections, `create-a-league-journey` and `game-rules-disclosure`. A baseline is only meaningful when rendered on the machine that compares it, and that machine is the GitHub runner.

The remaining action is one dispatch of **Visual contracts** (`.github/workflows/visual-contracts.yml`) on this branch with `update_baselines: true` and `commit_baselines: true`. It renders the images on `ubuntu-latest` and pushes them to the branch; the workflow refuses to write to the default branch by design.

Until then the section names sit in `AWAITING_BASELINE` in the same spec. That list is checked by `visualContractHarness`, which fails if a name in it already has baselines on disk — so the exemption expires with the dispatch that satisfies it, and cannot become the way new sections are added.
