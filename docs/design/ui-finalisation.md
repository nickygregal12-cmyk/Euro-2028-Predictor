...
**Follow is stored and read, and `/competitions` now offers it.** Contract 157 (`MIG-UI-10`) added the persistence on 11 August 2026 and the shell consumed it the same day. **The discovery surface did not, until 12 August, and it said so in its own copy**: `ExploreCompetitionsPage` carried a docblock and a note at the foot of the page stating that following could not be persisted and that a Follow control would therefore be a dead one. That was true when written and stale from the moment contract 157 landed — so the platform was offering follow and unfollow in Account while telling a player on another surface that the preference was unstorable, which is worse than a missing control. The control now sits BESIDE each card rather than inside it, because the card is a link and a button nested in a link is neither valid nor operable as either; it is absent rather than disabled when the preference read did not land, since a control that cannot know whether the player already follows would have to guess its own label; and it writes a follow and never a membership, which `exploreFollow.test.tsx` asserts mechanically. `followed` and `favourite` are booleans whenever the preference read landed, and `'unknown'` now means only that it did not — never that the player declined. Follow and game membership are **unioned**, not intersected: a player who follows a competition and has joined no game in it is in `mine` with an empty `joinedGames`, which is the state that was previously invisible.
...
| ID | Item | State |
| --- | --- | --- |
| `UI-F01` | Responsive shell: permanent global desktop rail with no competition-tree expansion, real multi-column composition, compact mobile competition switcher, mobile preserved | **Delivered, including adoption breadth, 12 August 2026.** `SideRail`, `Workspace`, rail-aware `PageShell`, `CompetitionSwitcher`, bounded competition shortcuts and `/competitions` were the mechanism; the remaining gap was which surfaces used it. Contextual panels now reach the competition Matches section (contract 141's club form, in a `panel` layout that stacks the same eight facts per club rather than shrinking an eight-column table into 320px), one player's season (contract 176's Prediction DNA), `/profile` (contract 161's season history beside the current competitions), `/leagues` (the explainer the navigation label deliberately does not carry, plus contract 165's organiser reads — presentation of a distinction that already exists, which closes none of `MIG-UI-20`: contract 179 landed that row's READ on the same day and the page it names is still unbuilt, as is the `PPLAY-001` correction to which read `/leagues` rebuilds private play from) and `/admin/season` (contract 172's operational health, which is platform-wide rather than season-scoped and so sits beside the season work rather than in it). Four single-column surfaces gained a reading measure — More, How the games work, Account and the private-play creation journey — because the shell is 1440px and a stack of link rows or radio labels across all of it is the stretched-phone composition this document names. **A measure is the right answer where a panel is the wrong one**, and the creation journey is the clearest case: a wizard has one live question, so a rail beside it can only duplicate that question or summarise steps already left behind, which is why that surface got a 640px column and an inline running summary instead of a second column; the season Leagues section went the other way to full width, because its Matchweek tab is a comparison matrix that was scrolling inside 820px with 600px empty beside it. **Two viewport media queries became container-measured `auto-fit` grids in the process**: a `@media (width >= 900px)` inside a 320px contextual column lays three tracks in a 320px box on a 1440px screen and overflows the page |
...
| `UI-F05` | Match Predictor: flagship flow, completion/lock/Joker polish, desktop insights panel | **Partial** — desktop panel delivered; the route stays behind `VITE_UI_SEASON_MATCH_PREDICTOR` until its migration reaches a hosted environment. **Offline drafting landed 12 August 2026** (`INNOV-020`, contract 177): a strip that appears only when work is outstanding, never a synchronisation dashboard, and a Joker/confirm defect was fixed with it — see `UI-F23` |
| `UI-F06` | Football Insights: form, H2H, table/goals context, enrichment slots | **Delivered on the data that exists, 12 August 2026.** Contract 141's form and club head-to-head render in the Match Centre; the form guide beside the Last Man Standing pick list ranks by form because that game asks for one club; the competition Matches section carries club form beside the fixtures and contract 160's table below them. **The gap this pass closed was the flagship**: the Match Predictor's contextual panel is labelled "Match insights" and held only contract 130's consensus, which is withheld until the matchweek locks — so the column beside the prediction card was empty at exactly the moment a player is deciding what to enter, and full afterwards. It now carries the matchweek's form first and the consensus after it, from the same contract 141 read the other three surfaces use, one request per season visit. It recommends nothing: no suggested score, no favourite, no probability, no arrow and no control, asserted negatively in `matchweekForm.test.tsx`. **The row's "table context" clause was closed on 12 August 2026**, and the reason it had stayed open was structural rather than missing data: contract 160's table was read inside `SeasonLeagueTable`, so the only surface that could see a club's position was the one already showing the whole table. The read is now the route's, through `useCompetitionTable`, and three consumers share one answer — the Table segment, the Match Centre's fixture header and the Match Predictor's form panel — because three components each running their own effect would be three requests for one answer and three chances for two surfaces on the same screen to disagree about a club's position while one was still in flight. A fixture states each club's position, points, matches played and goal difference, and does so **both or neither**: one club's rank beside a blank is a comparison a reader will complete incorrectly, so a club the table does not contain suppresses the pair rather than rendering half of it. Nothing is recomputed — the position is the server's, which `seasonLeagueTable.test.tsx` already required of the table segment and which now travels with it to the two surfaces that borrow a single row. **Enrichment beyond this stays conditional on stored and served provider data plus entitlement**, and an unimplemented source gets no card and no placeholder statistic |
...
| `UI-F09` | LMS: selection, status, survival context | **Delivered for the public competition** — the pick, the round, the deadline, the survival verdict from the settlement authority, and a desktop form guide beside the pick list derived from contract 141. **This row said other-player information "waits on a read that does not exist" until 12 August 2026, and that was stale**: contract 164 landed it on 11 August, behind the round's own lock, and `SeasonLmsPage` has rendered it since the same day through `SeasonLmsField`. What genuinely remains is a private container's own page (`MIG-UI-20`) — and since contract 179 landed `get_private_competition_workspace` on 12 August, that is now a surface rather than a read |
...
| `UI-F11` | Matches & Match Centre: dedicated journey, consequences, league comparison, consensus, movement | **Delivered** — the route is genuinely self-contained: contract 148 resolves the fixture by its own id, so the `?on=` day, the three-week window search and the "that match is not in this window" state are gone. The page composes three deliberately distinct answers: **You** (the player's prediction and points) in the main column, **Your leagues** (contract 149's named predictions after the matchweek's lock, with contract 150's movement) and **Everyone** (contract 130's anonymous consensus with its minimum cohort) beside it. A named league prediction is never presented as a consensus. **Extended 11 August 2026 by the Innovation Lab pass** (`UI-F22`): the page now also carries the What-If projection for a live fixture and, inside the consensus panel, how unusual the player's own pick was. **From 12 August 2026 the projection is the server's** (contract 175) rather than browser arithmetic |
| `UI-F12` | Leagues: private-play workspace, Table / Matchweek / Members | **Delivered for the Match Predictor league** — global `/leagues` lists all private play, and a league now opens into the accepted workspace. **Table** is contract 128's ranking with the rival and leader gaps; **Matchweek** is contract 149, as a desktop comparison matrix that scrolls inside its own container and a purpose-built phone layout with the player's own row first in every fixture — never the matrix shrunk; **Members** is the same rows ordered by name, with a route into each player's season. Contract 150's movement sits above the tabs and only from a settled matchweek — **above them deliberately, because it is the only place the caller's own standing survives a tab change**: Table ranks the league, Matchweek and Members do not. That line was corrected on 12 August 2026 for a § 4A violation it had carried since it was written: `presentLeagueMovement` has always computed the field size and the component printed the bare ordinal, so a player read "7th → 4th" without being told 4th of what. It now prints `7th → 4th of 12`. An unsettled matchweek still renders no line at all rather than a position derived from totals that are still changing. Which league is open and which tab lives in the URL, so Back from a player's season returns to it. Private LMS and Championship containers are created and joined since 11 August 2026 (contracts 153–155); a private COMPETITION still has no workspace of its own and lands in the `/leagues` list |
| `UI-F13` | Create / Join: one wizard, universal join code | **Delivered** — one creation journey covering all three weekly games, dispatching to `create_game_league`, `create_private_season_lms` or `create_private_season_cup` and showing only the selected game's fields; and one code entry point over `resolve_invite_code`, on both `/join/:code` and the code sheet, with the server's `joinWith` deciding which join is called. The refusal screens for the other two games are gone with the reason for them. **Its desktop composition was finished on 12 August 2026, and the answer was not a contextual panel**: a wizard is a sequence with one live question, and a 320px rail beside it either duplicates that question or holds a summary of steps the player has just left. The journey instead takes a reading measure — a form at 1440px whose radio labels run the full shell is harder to scan than the same form on a phone — and gained the thing a multi-step form actually loses, a running summary of what has been chosen so far, so a player at the naming step can still see which game they picked without going back for it |
...
| `UI-F18` | Final design pass: spacing, typography, hover/focus, motion, skeletons, all states, both themes, density | **Materially complete, 12 August 2026, and re-measured rather than re-asserted.** The sweep was re-run by the same method over 126 width/theme/route combinations and found five things the first run had not reported — one real horizontal overflow and four undersized controls. All five are fixed and the sweep is now clean apart from the score stepper's recorded 44×36 decision. See § 13, which records the findings and the one technique that is new: expanding a POINTER TARGET past its visible glyph, marked `data-tap="expanded"` in the markup so it is greppable and so a sweep can tell a deliberate widening from a small control |
| `UI-F19` | Full signed-in acceptance: phone + desktop, light + dark | **Executed as far as the tooling genuinely permits, 12 August 2026, and the hosted half is UNAVAILABLE rather than passed or failed.** What ran: the full Vitest suite, the seven-width two-theme measured sweep over every anonymous and harness-rendered surface, and the browser and visual suites in CI on the pull request. What could not run is the part that needs a real session on the hosted environment, and the Team-SSO boundary there is a genuine gate rather than an obstacle to work around — so no signed-in hosted journey is claimed. The distinction matters and is kept: an item nobody could execute is not an item that passed. See § 16 for the surface-by-surface record of which is which |
| `UI-F20` | Public acquisition landing page | **Delivered for both variants, 12 August 2026.** ADR 0026's deployment seam landed on 11 August, so the weekly landing page has a tournament-led sibling: `EuroLandingPage` leads with Euro Predictor, groups Last Man Standing and the Predictor Championship beneath it as Bonus Games and names domestic Match Predictor as played on the Hub, keyed on contract 143's publication state and closed by default. **The acquisition polish this row was written for is now done.** The Hub's two product previews became one deterministic four-frame SCRIPT carrying the weekly loop — a deadline arrives, you predict, it locks, the football happens, the table moves — because a single still shows what the product looks like and says nothing about what it does. Every clause of the direction's list for a simulated preview is a test rather than a comment: fixed local data, no clock and no randomness, no session and no write, paused by `IntersectionObserver` and by a hidden document, a full stop under `prefers-reduced-motion` with every frame still reachable by hand, a valid static first frame before any effect runs, and no control inside the device — asserted by proving nothing focusable exists anywhere in it. The step controls sit outside the chrome and name the preview. **The Euro hero gained its second column and it is the LIFECYCLE rather than a picture**, because that deployment has no draw, no teams and no fixtures and a preview would be the placeholder § 8 forbids; it fails closed, so an unreadable publication state reads "Not open yet" |
| `UI-F22` | Innovation Lab UI pass: signature live, retention and distribution surfaces promoted from [`../product/innovation-lab.md`](../product/innovation-lab.md) | **Delivered for twelve `INNOV-*` rows, 11 August 2026, and honestly short of the register.** On production routes: the What-If projection and the community-divergence line in the Match Centre (`INNOV-001`, `INNOV-011`); Prediction DNA on a player's season (`INNOV-002`); per-matchweek league side honours (`INNOV-012`); closest misses under a settled card (`INNOV-014`); the matchday briefing on Home and `/play` (`INNOV-016`); the submission receipt (`INNOV-017`); the Matchday TV screen (`INNOV-006`); the share entry points (`INNOV-007`); the app badge and view transitions (`INNOV-023`, `INNOV-024`). `INNOV-013` is recorded as superseded — the historic-fixture experience was already the Match Centre's. **Twelve rows did not ship and none is disguised as a control**: an analyst with no server, a public league page with no public read, a challenge with no persistence, a confidence input that saves nothing, a wallet pass nobody can sign, a calendar feed that does not exist, a passkey the auth stack cannot issue. Offline drafting is deferred rather than blocked, for the reason in the register. **The pass changed no database, no hosted environment and no authority**; every derived figure comes from `domain/season/scoring`. **Extended 12 August 2026 by the contracts 174–178 consumption** — see `UI-F23` |
| `UI-F23` | Contracts 174–178 consumed: the server What-If projection, server Prediction DNA, offline drafting, the shadow-scoring report and provider calendar changes | **Delivered, 12 August 2026, and it removed more than it added.** `database.types.ts` was regenerated against hosted Development at contract 178 through the repository's own read-only workflow, which unblocked all five. **`INNOV-001`** now reads contract 175 rather than projecting in the browser: the fold over the league's revealed predictions, the client-side ranking and the banked-points read are deleted, each branch shows the whole matchweek post-Joker, and the basis is the server's word rather than inferred from a score being present. **`INNOV-002`** now reads contract 176, which closed a defect the browser derivation carried — the exact-score rate had a different denominator from the tendency shares — and the panel is three groups with three stated denominators, obeying the server's `sufficient` flag by withholding the signature and the share rather than by hiding the counts. **`INNOV-020`** is built: a versioned local draft store scoped by schema, account, season and matchweek, and a reconciliation honouring every per-item outcome separately, where only `accepted` removes a draft and no state in the model means submitted. **Contract 178** appears on `/admin/season` with no control at all and with "never checked" kept distinct from "verified". **Contract 174** appears there as a second queue that is decided rather than acknowledged. Two defects were found while wiring it and fixed: the save controller closed over the matchweek from first render, so `setJoker` and `confirmCard` targeted the matchweek the player arrived at rather than the one on screen; and the season Match Predictor route resolved its matchweek below its own early returns |
| `UI-F24` | Private-play lifecycle: `/leagues` discovery for private Last Man Standing and Championship containers, and `MIG-UI-20`'s private workspace | **Server half delivered as contract 179, 12 August 2026; the surface is NOT delivered and is not claimed.** `get_my_private_competitions` answers for the caller with no competition, game, season or player argument, and `get_private_competition_workspace` opens one container by its own id. **The defect this closes is still live in the product**: `/leagues` rebuilds private play through `get_my_game_leagues`, which reads `public.leagues`, and a private container is a `bonus_competitions` row — so a create or join that fully succeeded still disappears. The frontend work is (a) consume the discovery read for bonus-game containers rather than pretending they are league rows, (b) make the post-join "it is in the list below" copy follow an authoritative reread rather than an RPC result, and (c) give an organiser a way back to an unlaunched Championship, which contract 179's readiness object supplies the server facts for. **Contract 180** additionally means a Championship-only entrant now reaches the matchweek card at all, so the surface must stop assuming a Match Predictor membership implies the card and vice versa. **Contract 184** changes no surface: it makes Championship qualification correct at every group size, which a bracket UI would eventually have depended on. **Contract 186** and **contract 187** change no surface either, and together they create the thing that UI would render: a season Championship can now qualify, seed and bracket, so a bracket exists to show. The READ of it is `CUP-003` and is not built, so nothing consumable arrives with them. **Contract 183** lets the favourite-club picker drop its two-read join (`MIG-UI-16`) and lets a leaderboard show the caller's immediate neighbours without paging to them (`MIG-UI-18`); neither is consumed yet. **Contract 182** changes no surface at all — it pins which function ranks a Championship group. **Contract 181** gives a full league a server refusal an invite surface can render — `league_member_limit_reached` at a hundred members — rather than a failure with no explanation. **Contract 188** changes no player-facing surface: its one presentation change is the Bet Builder inside the administrator-only AI Lab, which is Hub-only and absent from the Euro deployment. **Contract 189** changes no player-facing surface either: it narrows eight administrator-only AI Lab reads so a quarantined forecast and the bets advised from it stop counting as evidence, which is Hub-only and absent from the Euro deployment. **Never render a failed read as an empty private-play list** — that is how this became invisible |
| `UI-F21` | Two deployments from one commit: variant-selected brand, navigation, landing, metadata, sitemap and route visibility | **Delivered for brand, navigation, landing, metadata and the signed-in top level.** `VITE_SITE_VARIANT` and one typed `SiteConfiguration` (`src/app/site/`), failing closed to the Hub. The document head, sitemap and `robots.txt` are generated per deployment; an unconfigured origin emits none of them rather than falling back to the other site's domain. **The signed-in half landed 11 August 2026** — `/`, `/play`, `/matches` and `/leagues` resolve through a typed variant table rather than serving the domestic Hub under Euro labels, the domestic weekly tree and the rail's competition list are withdrawn from the Euro build, first sign-in no longer runs domestic onboarding there, `/auth/signup` is gated on contract 143's publication state at the route, and domestic Match Predictor is no longer offered as a Euro Bonus Game. **Route visibility in the OTHER direction is built and not turned on:** the deployment gate refusing the tournament's player routes exists and is proven, and both deployments still serve them, because withdrawing them from the weekly app deletes the `euro-2028-baseline` browser evidence rather than moving it. That flip is an owner decision plus a browser-suite move. **The Euro tournament's own journeys are not wired on the Euro build either** — they stay parked for January 2028 behind `tests/app/parkedEuroBoundary.test.ts`, so what the Euro destinations show today is a truthful closed state rather than the tournament |

Contract 186 adds no player-facing UI either: its multi-model evidence, calibration and decision log are admin-only, and the one new read is a bounded competition-admin RPC.

Contract 185 adds no player-facing UI: ADR 0029 keeps its modelling, paper-betting and paid-odds evidence private and admin-only.
...
- **A league prediction list can say how much of the league it is showing.**
  ...
  the league leader could be absent from the league's own table. **Consumed**, and
  this paragraph said otherwise until 12 August 2026. Two things about the stale
  claim are worth keeping. It named the wrong blocker: these reads are typed
  `Returns: Json`, so their fields are decoded by hand and the generated types
  never constrained them — regenerating was never the bar. And by the time the
  claim was read, `seasonLeaguePredictionsModel` decoded both fields and
  `leagueMatchweekModel` and `sideHonoursModel` rendered them through
  `src/features/shared/listCoverage.ts`. Measured in the source rather than
...
- **A second administrator queue exists and nothing shows it.** Contract 174
  ...
  **Consumed 12 August 2026.** `/admin/season` carries it as a second queue,
  deliberately separate from contract 138's: those are ACKNOWLEDGED and these are
  DECIDED, and one control doing both would be the ambiguity the staging exists
  to remove. The proposal is shown beside what we hold field by field, warnings
  and blockers by name and never as a count, and no raw provider payload reaches
  the browser. `decidable` is rendered rather than recomputed, so approval is
  offered exactly where the server permits it; the control names its consequence
  ("Add this fixture", "Void this fixture") rather than saying "approve"; and a
  refusal is shown as the server wrote it, which matters most for the one that
  says the fixture now has a result.
- **Four reads and a write exist for surfaces nobody has built.** Contract 175
  ...
  reconciliation surface has to put in front of the player. **All four are
  consumed, 12 August 2026.** What did NOT change: contract 178 gains no
  scheduled caller, so the administrator surface it feeds says "never checked"
  rather than "verified", and that distinction is the whole of what makes the
  panel honest.
- **An administrator can see whether reminders are working.** Contract 172 adds
  ...
  user id, action key, display name or provider error text. **Consumed 12
  August 2026** as `/admin/season`'s contextual panel, and this paragraph said
  otherwise until then. The panel leads with the three JOBS rather than the
  counts, deliberately: "no reminders are scheduled" and "the scheduler is not
  running" are the same number read from the ledger, which is exactly how
  contracts 162, 163 and 170 each shipped a generator with no caller and
  nobody noticed for three contracts. `jobs: null` and `jobs: []` are kept
  apart by type — "I could not look" and "there is nothing scheduled" are
  different operational facts, and the reassuring reading of the first is the
  mistake contract 172 exists to stop. A missing job is named rather than
  counted, because a count of two out of three tells an operator that
  something is wrong and not what to restart. It carries **no control**: every
  driver is `service_role`-only, no sender exists, and a button would either
  do nothing or be the first step toward the brand decision being taken by
  accident. **Nothing sends**: no provider is chosen, the dry run is on by
  default and `SITE-007` still blocks the sender on the brand decision — which
  the panel states in words, so a queue of pending reminders reads as the
  expected state rather than as a backlog.
- **A Championship table now says what it was ranked over.** Contract 169 corrects the
  ...
  competition's kind. It adds no surface and changes no layout. **The label IS
  rendered**, and this paragraph's reason for saying otherwise was wrong twice
  over: `get_season_cup_phase` is typed `Returns: Json`, so `table_source` was
  never constrained by the generated types, and by 12 August 2026 `seasonCup.ts`
  decoded it and `cupPhaseModel`/`SeasonCupPhasePage` printed it. Corrected
  against the source rather than deleted, because the mistaken reason — "the
  generated types are the blocker" — is one this repository has now made twice.
...
### The second sweep, 12 August 2026 — five things the first one did not report

The same method was re-run over **126 width/theme/route combinations**, adding the public landing page to the surfaces the first sweep covered. It found five things, and the useful lesson is that a sweep is only as good as the run: three of these were present when the first sweep reported "no undersized control on a shipping season surface", and that sentence was therefore too strong.

- **One horizontal overflow**, 15px at 390px in both themes, in the component gallery. A flex item's default `min-width: auto` is its content's min-content width, so a panel holding a component with a wide minimum could not shrink below it. The gallery is not shipping UI — but an overflowing measuring instrument reports its own layout as a product defect on every future run, so it is fixed rather than annotated.
- **The Alert and Toast dismiss controls at 24×24**, on every surface that renders one. 24×24 meets WCAG 2.2 AA's minimum and misses this repository's own stricter `--tap-min` of 44px.
- **The landing page's header and footer links at 22px tall.** A text link is only as tall as its line box, and the public acquisition page is exactly where a mis-tapped "Create account" costs a visitor.
- **`JokerButton` at 30px.** `MatchCard` is the parked Euro card — the weekly season uses `ClubMatchCard` — so this is not a live weekly defect, and it was corrected anyway on the precedent this section set for the tournament Match Centre's topbar.
- **Both skip links a pixel under the minimum**, from padding and a line box rather than from a decision. Corrected together, because a restated rule that drifts from its original is worse than no restatement.

**The dismiss controls introduced a technique worth recording, because it will be reached for again.** The obvious fix — grow the box — is the wrong one there: an Alert's height is its message and a Toast is `align-items: center`, so a 44px control would add twenty pixels to every one of them and make a dismiss the tallest thing in a block whose point is the sentence. The POINTER TARGET is expanded instead, with a 44×44 pseudo-element centred on the 24px glyph, which participates in hit testing and takes no space in the flex row. That is the property the 44px rule exists to protect; the layout and the visual baseline are unchanged. `data-tap="expanded"` marks it in the markup so the sweep can tell a deliberately widened target from a small one, and so every instance is greppable — the attribute is the audit trail.

**After: no horizontal overflow at any width in either theme on any surface, and the only sub-40px controls left are the score stepper's 44×36 `+`/`−`**, which is the standing decision recorded in `ScoreInput.module.css` and which still stands.
...
## 16. `UI-F19` — what was executed, and what could not be

The acceptance row asks for the signed-in product exercised on phone and
desktop, in both themes, across the product's states. This section records the
12 August 2026 execution **honestly split into three**, because "we ran it and
it passed", "we ran it and it failed" and "the tooling cannot run it" are three
different facts and only the first two are results.

### Executed, and green

| What | How |
| --- | --- |
| Every domain, model and component assertion | Full Vitest suite |
| Every anonymous and harness-rendered surface, at 390 / 768 / 1023 / 1024 / 1280 / 1440 / 1800, in both themes | The § 13 measured sweep, 126 combinations, reading the live document rather than judging by eye |
| Horizontal overflow, tap targets and the 1024px rail handover | Same sweep. Clean after the five fixes § 13 records |
| The signed-in journeys a seeded local database can drive | Browser E2E in CI on the pull request, which is where Playwright runs — the development container has no `.env.local` |
| Rendered appearance of every declared gallery section, both themes | Visual contracts in CI on the pull request |

### Not executed, and NOT claimed either way

A true hosted manual journey — signing in to the Development deployment as a
real player and walking Hub, Play, Matches, the three games, a private league,
create and join, the player profile, Account and admin on a phone and a desktop
— **did not run**. The Team-SSO boundary on that environment is a genuine gate
rather than an obstacle to work around, and this session neither weakened it nor
attempted to.

**That is recorded as unavailable. It is not recorded as passed, and it is not
recorded as failed.** The distinction is the whole point of this section: an
item nobody could execute tells you nothing about the product, and filing it
under either result would put a false fact in the record to make a table look
complete.

### What acceptance found and fixed in the same branch

- The five § 13 findings: one horizontal overflow and four undersized controls.
- Two surfaces claiming a capability the platform had: `/competitions` telling a
  player that following could not be stored while Account stored it, and this
  document telling a reader that Last Man Standing other-player information
  needed a read that in fact landed on 11 August.
