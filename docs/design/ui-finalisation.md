...
| `UI-F08` | Games brought to equal presentation quality | **Partial** — every card carries joined state, the game's own outstanding action and deadline, a direct action label and honest refusals with no dead controls, and joined games now sort before available ones. What remains is per-game rules/help: `docs/scoring-rules.md` is the preserved Euro configuration and explicitly not a platform default, so a season game card has no rules surface to link to |
