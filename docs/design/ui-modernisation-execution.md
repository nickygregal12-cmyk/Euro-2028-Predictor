...
| Adopt the foundations component by component | **Part done.** The type scale reached every stylesheet (UI-06 → UI-09) and the seven neutral tokens were repointed at the target ramp (UI-11), so `--fs-*` is consumed by 64 files and the ramp is the single source of neutral truth. What has *not* been adopted is measurable and small: the motion tokens are used by 2 files, `--numeric` by 5, and the stacking scale by none — 8 hardcoded `z-index` values remain across 6 production stylesheets, alongside roughly 44 raw pixel font-sizes in 11 files |
| Close the offline/unavailable/conflict gallery states | Outstanding |
