...
| Thin competition shell and the phone-first season Match Predictor, behind a route-level flag | **Surface done, data path outstanding** — the page, shell, state machine, save/conflict recovery, lock-explanation layer, fail-closed flag and telemetry category are production code under `src/features/season/`, proven at `/dev/season-predictor`. No browser role can read a season fixture or prediction yet (`get_season_leaderboard` is the only exposed season RPC), so the gateway is fixture-backed and the production route registration waits for the bounded RPCs rather than shipping a route that can only say "unavailable" |
