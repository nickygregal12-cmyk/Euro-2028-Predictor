...
| Thin competition shell and the phone-first season Match Predictor, behind a route-level flag | **Both halves built; the production route is not registered.** The page, shell, state machine, save/conflict recovery, lock-explanation layer, fail-closed flag and telemetry category are production code under `src/features/season/`, and contract 114 supplied the bounded RPCs the gateway needed — they are applied to development. What remains is the route registration itself: route table, titles, redirects, axe matrix, MSW scenarios and the flag flip. The surface is still reachable only at `/dev/season-predictor` |
