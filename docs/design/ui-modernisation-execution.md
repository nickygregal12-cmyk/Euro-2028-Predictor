...
| Close the offline/unavailable/conflict gallery states | **Done** (UI-16). Each has a section of its own rendering the treatment §9.1/§9.2 requires, and `tests/design-system/galleryStateCoverage.test.ts` now relates the harness to the state matrix — the control that was missing, and the reason three states could stay absent while seventy sections made the gallery look complete. It also makes the remaining gap visible rather than closed: **refreshing**, **stale** and the **blocking error** page, each listed there with the reason it is not rendered yet |
