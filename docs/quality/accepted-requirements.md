...
| `EURO-001` | Euro 2028 is completely hidden from the weekly platform until an owner-approved publication state | `EURO-002` | Weekly platform surfaces contain no Euro reference while the state is hidden | Accepted — **currently violated**: Euro 2028 is absent from the weekly Hub catalogue, but the weekly/public surface still exposes Euro through landing/metadata and no server-owned publication-state route guard exists yet |
| `EURO-002` | Server-owned publication states: hidden, prelaunch, registration-open, live, completed, archived | — | Persisted state with one authority, and a transition record | **Implemented in repository Contract 135 candidate** — default hidden, bounded read, owner-only adjacent transitions and append-only transition history; hosted rollout not claimed |
