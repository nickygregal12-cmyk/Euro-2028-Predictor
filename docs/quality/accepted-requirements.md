...
| `EURO-001` | Euro 2028 is completely hidden from the weekly platform until an owner-approved publication state | `EURO-002` | Weekly platform surfaces contain no Euro reference while the state is hidden | Accepted — **currently violated**: Euro 2028 is absent from the weekly Hub catalogue and its tournament routes are no longer registered at all, but the weekly/public surface still exposes Euro through `LandingPage`'s Euro band and three `index.html` meta descriptions, five mounted weekly routes (`/profile`, `/profile/:playerId`, `/account`, `/h2h/:rivalId`, `/league/:id`) still answer for the Euro tournament and only for it, and no server-owned publication-state route guard exists yet. The Euro dataset is at least no longer fetched on every domestic page load — it mounts behind `TournamentJourney`, wrapping exactly the routes that read it |
| `EURO-002` | Server-owned publication states: hidden, prelaunch, registration-open, live, completed, archived | — | Persisted state with one authority, and a transition record | **Implemented in the repository Contract 143 candidate** — default `hidden`, bounded `euro_publication_state()` read, owner-only adjacent transitions through `admin_transition_euro_publication_state` and append-only transition history. Hosted rollout is not claimed; the row leaves this register once Development holds it |
