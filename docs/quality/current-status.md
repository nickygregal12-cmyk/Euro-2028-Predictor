...
| Repository contract | 48 canonical migrations through `20260728150000_bonus_games_platform.sql` |
...
| Development Supabase | `iouzoutneyjpugbbtdem` — contract 48 applied 28 July 2026 with canonical history (48 versions through `20260728150000_bonus_games_platform`), verified deny-all bonus-table posture and unchanged baseline data |
...
| Netlify contexts | `dev`, `branch-deploy` and `deploy-preview` still declare 47 and require an owner `EURO28_DEPLOYED_DB_CONTRACT=48` update before the next non-production deploy; `production` declares 44 and uses production Supabase |
...
Production is a controlled future-tournament target, not an active Euro 2028 service. Its database and application are aligned and locked at contract 44. Contracts 45–48 are development-only and must not be promoted to production without a later approved milestone gate.

## Executive verdicts

| Area | Verdict |
| --- | --- |
| Contract alignment | **Intentionally split.** Repository and development Supabase are at contract 48; non-production Netlify still declares 47 pending an owner environment update; production database and application are aligned and locked at contract 44. |
...
1. The Bonus Games platform foundation is delivered under ADR-0010: B1 pure domain (`src/domain/competitions/`, 25 unit tests) and B2 deny-all schema (contract 48, pgTAP `103_bonus_games_platform.sql`, Database parity green, applied to development Supabase with verified posture and advisors). Game surfaces (B3+) remain gated behind the Stage 4 core experience per `docs/roadmap.md` Stage 5.
2. Owner follow-up: set `EURO28_DEPLOYED_DB_CONTRACT=48` on the non-production Netlify contexts before the next non-production deploy.
