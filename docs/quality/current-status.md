...
| Repository contract | **103** — 103 canonical migrations through `20260804333000_competition_instance_lineage.sql`. Development Supabase is hosted at **103**, applied 4 August 2026 by fast-lane run 30959460638 on `6dc7ae3`, which carried contracts 98–103 in one batch; production remains at **63**. Development and the repository are level. That batch had briefly had no lane at all: the additive gate refused contract 103 for `drop trigger`, the fast lane derives its own pending list so one refusal refused all six, and the documented fallback is pinned to contract 65. The gate now treats a drop as structural only when the very next statement re-creates the same trigger on the same table, and the apply log corroborates that this removed nothing — `trigger "prepare_competition_lineage" ... does not exist, skipping`. Contract 87 is the first in this run to remove a database guarantee rather than only add one — it drops `bonus_lms_selections`' cycle-blind club key and replaces it with one scoped to `used_cycle` — and its rollout did carry the `structural` line the widened additive gate now emits, naming `drop constraint` rather than passing it in silence. Non-production Netlify contexts declare `EURO28_DEPLOYED_DB_CONTRACT=97` as of 4 August 2026, which now trails both the repository and development — the ordinary cycle, and previews will say so until the declaration is moved to 103 now that the rollout has landed — **owner-reported, with no repository-side read path to confirm it** ([`../ops/ops-pending-migrations.md`](../ops/ops-pending-migrations.md) records which build-log line corroborates which value) — while production keeps its 63 declaration and the fatal contract gate. |
| Target design authority | **Added 4 August 2026.** [`../design/README.md`](../design/README.md) indexes the *Hub Architecture and Modernisation Plan* rev 1.5 and its landing-page prototype — what the product should look like when finished. Presentation and delivery only: it changes no scoring, lock, membership, settlement or visibility rule. **Its baseline is contract 93**, so Appendix D.2's reconciliation list predates contracts 94–103 and must be checked against this document before being treated as outstanding. The prototype's light theme inherited dark-tuned semantic colours and failed WCAG AA at 1.3–2.2:1; corrected and guarded by `tests/design/landingPrototypeContract.test.ts` |
