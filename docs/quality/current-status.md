...
| Current development position | Stage B implementation complete across merged foundation/Home work and the validated draft stack for Matches, Match Centre, entry lock and temporal-state retirement; remaining integration to clean `main` is not complete |
| Next engineering stage | Stage C competition-season schema, only after the remaining Stage B stack is integrated and verified on clean `main` |
...
| Context engine | **Foundation and Home are merged; the remaining Stage B consumers are implemented and internally green in draft PRs #216, #222, #223 and #224.** The stack is not yet integrated to clean `main`. |
| Stage B exit | **Implementation complete, integration pending.** The shared engine now owns Home, Matches, Match Centre and entry-lock decisions in the validated stack, and `MatchTemporalState` is retired there. |
| Preview evidence | **Externally blocked.** Exact Netlify PR-preview identity remains unavailable, so preview HTTP/browser smoke stops before application code loads; core CI, database parity and authenticated local-browser journeys are green on the final Stage B heads. |
...
The ADR stack, planning hierarchy, domain-path controls and current implementation authorities are established. The active documentation slice reconciles live status and roadmap wording with the completed Stage B implementation. Brand-clearance work remains governed by ADR 0017 and must not be inferred complete from engineering progress.
