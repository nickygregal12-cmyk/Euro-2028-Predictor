...
| Next executable issue | **Contract 102 represents the Cup split stage and phase-aware membership; the split creator, scheduler and combined standings read remain later Cup drivers.** The remaining ADR 0025 lifecycle work is the LMS restart, which first needs `bonus_competitions`' availability/instance split. Contracts 98–102 all await a development fast-lane run. No provider credential, provider request, Edge Function deployment, C2 work or production write is authorised by repository inclusion. |
