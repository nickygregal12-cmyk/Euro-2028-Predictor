...
| Repository contract | **68** — 68 canonical migrations through `20260803193000_season_fixtures.sql`. Development Supabase is hosted at **67** (contract 66 applied 3 August 2026 through the ADR 0024 fast lane, run 30837677979; contract 67 the same day, run 30840592967); production remains at **63**. The repository therefore leads development by one additive migration until the next fast-lane run. Non-production Netlify contexts carry `EURO28_DEPLOYED_DB_CONTRACT` from the last alignment and report any gap as informational rather than failing the build (ADR 0024); production keeps the fatal check |
