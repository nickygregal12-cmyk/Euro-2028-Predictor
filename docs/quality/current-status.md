...
| LMS restart lifecycle — **prerequisite landed at contract 102** | ADR 0025 decision 1 named its own prerequisite as the larger half of the work, and this is it. **The conflation, named:** one `bonus_competitions` row answered two questions — *availability* (`tournament_id`, `game_key`, `availability_status`, `published`) and *instance* (`registration_*`, `draw_*`, `completed_at`) — and `unique (tournament_id, game_key)` was the availability key doing the instance row's job. That is why `restart_all_reentered` was **unrepresentable rather than unimplemented**: a season offers Last Man Standing once, so it could only ever run it once. Contract 102 **reconciles** the two rather than splitting the table, which is the second option the ADR allows and by far the smaller change: the availability pair stops being unique across all history and becomes unique across the **live** instance only. Splitting availability into its own relation was considered and rejected for now — it would move two columns out from under 27 functions that read them alongside instance columns, for no behaviour anyone has asked for. The conflation is resolved by making the KEY honest, not by moving columns. Also added: `series_id` / `series_sequence` / `predecessor_competition_id` (backfilled so every existing row is instance 1 of a series that is itself), and `completion_reason` in `('won','no_winner_restarted','abandoned')` so a restarted competition is afterwards distinguishable from a won one. A chain cannot wander between series — the predecessor reference is composite on `(series_id, id)`. **Why this is safe to land alone, verified rather than assumed:** ~20 functions do a single-row lookup on `(tournament_id, game_key)`. Relaxing the key makes that assumption false — but only once a second instance exists, and nothing in contract 102 can create one. Read-only checks confirm **zero completed competitions in development (10 rows) and production (3 rows)**, so the partial index permits nothing the constraint forbade and is equivalent today. **Sequence, deliberate:** shape (102), then callers onto `predictor_internal.live_competition_id` (103), then the restart driver (104). Landing the driver first would make all ~20 lookups arbitrary-row reads at the exact moment the first competition restarted — the failure already found three times this run. Evidence: `supabase/tests/153_competition_instance_lineage.sql`, 18 assertions, five mutants killed at 6, 6, 2, 2 and 1 |
| Next executable issue | **Contract 103:** move the ~20 single-row `(tournament_id, game_key)` lookups onto `predictor_internal.live_competition_id`, which is behaviour-identical while only one instance can exist. **Then contract 104:** the restart lifecycle function itself — idempotent, advisory-locked, completing the old row as `no_winner_restarted`, creating the successor, re-entering every previous entrant, copying no selections, cycles, projections or windows, and recording a `public_wipeout_restart` audit event. Settlement continues only to derive and report. Separately, the Cup split-stage standings await whichever contract-101 split shape lands. Contracts 98–102 all await a development fast-lane run. Every contract still merges only with exact-head CI, Database parity, Browser E2E, hosted-inventory and protected Netlify preview evidence. No provider credential, provider request, Edge Function deployment, C2 work or production write is authorised by repository inclusion. |
