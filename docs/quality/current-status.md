...
| Repository contract | **96 candidate** — 96 canonical migrations through `20260804243000_provider_ingestion_custody.sql`. Contracts 94–95 supply season standings and the bounded co-member leaderboard read; contract 96 adds server-only provider-response custody and strict decoder evidence. Development Supabase is hosted at **93**, applied 4 August 2026 by fast-lane run 30920330240 on `2c2fe85`, whose postflight reported `Development is at contract 93.` Production remains at **63**. Repository inclusion does not deploy `provider-poll`, configure a provider credential or call a provider. |
