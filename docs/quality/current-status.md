...
| Repository contract | **65** — 65 canonical migrations through `20260730235602_stage_c1_competition_season_foundation.sql`. Development Supabase is hosted at **65** (applied and postflight-verified 2–3 August 2026); production remains at **63**. Non-production Netlify contexts (Development, branch deploys, deploy previews) are aligned to **65** with `VITE_SUPABASE_URL` verified per context (three non-production → development ref, production → production ref); an exact-origin deploy-preview smoke passed on PR #369 against a contract-65 preview |
