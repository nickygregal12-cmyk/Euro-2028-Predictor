...
| Repository contract | **86** — 86 canonical migrations through `20260804143000_provider_ingestion_custody.sql`. Development Supabase is hosted at **84**, applied 4 August 2026 by fast-lane run 30899305992 on `a9daf64`, whose postflight reported `Development is at contract 84.`; production remains at **63**. Contracts 85 and 86 are additive pending migrations and may be applied together by the next fast-lane run after contract 86 merges. Repository inclusion does not deploy `provider-poll`, configure a provider credential or call a provider. Non-production Netlify contexts still carry their separately managed declaration, while production keeps the fatal contract gate. |
...
| Next executable issue | **Finish provider-ingestion custody as contract 86.** Merge only after exact CI, Database parity, Browser E2E, hosted-inventory and Netlify preview gates pass. Then apply the two pending additive migrations (85 and 86) to Development through `.github/workflows/development-fast-lane-rollout.yml`, require postflight `Development is at contract 86.`, align non-production declarations, and keep production at 63. |
...
| --- | --- | --- |
| Development Supabase `iouzoutneyjpugbbtdem` | healthy Postgres 17. Development Supabase is hosted at **84**, applied 4 August 2026 by ADR-0024 fast-lane run 30899305992, whose postflight reported `Development is at contract 84.` Contracts 85 and 86 are not applied; `provider-poll` is not deployed. | After contract 86 merges, dispatch `.github/workflows/development-fast-lane-rollout.yml` from exact `main`; require it to identify both pending migrations, prove both additive, take the lightweight snapshot, push them and report `Development is at contract 86.` |
...
**Known follow-up.** `SEED_REVIEWED_AT_CONTRACT` is 86 and explicitly reviewed against contracts 85 and 86, including the provider-custody relation, service-role RPC and browser-access boundaries.
