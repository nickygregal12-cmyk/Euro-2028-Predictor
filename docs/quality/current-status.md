...
| Stage C1 | **Migration drafted and proven on disposable infrastructure (PR #317); not merged.** Two defects were found and fixed on the branch on 1 August 2026 — see the PostgREST embed and entry-lock rows below. The authenticated browser suite went 37 failures → 6 → the fixes are now under CI. The branch is behind `main`; a test merge applies cleanly with typecheck, lint and the full Vitest suite green. Remaining before merge: a green browser suite on the reconciled head. No hosted write exists or is authorised. |
| Entry lock reversibility | **Defect found and resolved on the PR #317 branch, 1 August 2026.** A draft of the migration treated the existence of a `competition_lock_events` row as a lock, independently of `tournaments.lock_at`. Because `record_tournament_lock_transition` writes that row the first time a deadline passes, the table is append-only, and `prevent_lock_event_mutation` refuses both `delete` and `update`, a tournament whose `lock_at` had *ever* been in the past could never accept another prediction — reproduced at contract 65 with `lock_at` 30 days ahead still failing `23514`. It was also the cause of six authenticated browser failures. **Owner decision: `lock_at` and `kickoff_at` remain the sole enforcement authorities, and lock events are evidence only.** That matches the approved scope — issue #303 authorises lock-transition *evidence*, and [`stage-c1-schema-overlay.md`](../architecture/stage-c1-schema-overlay.md) dispositions the table as an observed transition with no browser authority. Ten lookups were removed from the three enforcement functions; all four evidence inserts remain. Three assertions in `032_stage_c_lock_before_state.sql` that encoded the reversed behaviour were rewritten to assert the decision. |
