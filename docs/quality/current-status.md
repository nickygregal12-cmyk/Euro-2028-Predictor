...
| Repository contract | **86** — 86 canonical migrations through `20260804143000_lms_season_selection.sql`. Development Supabase is hosted at **85**, applied 4 August 2026 by fast-lane run 30901081053 on `701fe62`, whose postflight reported `Development is at contract 85.`; production remains at **63**. Development trails the repository by one additive migration until the next fast-lane run. Non-production Netlify contexts still carry their separately managed `EURO28_DEPLOYED_DB_CONTRACT` declaration, while production keeps the fatal contract gate. |
