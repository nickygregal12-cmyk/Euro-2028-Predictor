...
| Repository contract | **70** — 70 canonical migrations through `20260803213000_season_scoring.sql`. Development Supabase is hosted at **69** (contract 66 applied 3 August 2026 through the ADR 0024 fast lane, run 30837677979; contract 67 run 30840592967 and contract 68 run 30843028463 and contract 69 run 30845804899, all the same day); production remains at **63**. The repository therefore leads development by one additive migration until the next fast-lane run. Non-production Netlify contexts carry `EURO28_DEPLOYED_DB_CONTRACT` from the last alignment and report any gap as informational rather than failing the build (ADR 0024); production keeps the fatal check |
...
| Season/Cup SQL parity | **Main Predictor scoring landed at contract 70** (`predictor_internal.season_fixture_points`, `season_matchweek_points`), held in step with `src/domain/season/scoring.ts` by `seasonScoringParity.test.ts` and proven against a real database by `122_season_scoring.sql`. LMS and Cup parity remain absent and required.**Previously:** The season domain modules have no PostgreSQL counterpart, so no parity suite exists for season scoring, LMS or the Cup. ADR 0012 requires season scoring parity and ADR 0022 (as corrected) records the Cup case. This gap closes only when the corresponding SQL lands |
