...
| Repository contract | **67** — 67 canonical migrations through `20260803180000_matchweek_lock_scope.sql`. Development Supabase is hosted at **66** (contract 66 applied 3 August 2026 through the ADR 0024 fast lane, run 30837677979); production remains at **63**. The repository therefore leads development by one additive migration until the next fast-lane run. Non-production Netlify contexts carry `EURO28_DEPLOYED_DB_CONTRACT` from the last alignment and report any gap as informational rather than failing the build (ADR 0024); production keeps the fatal check |
