...
| Repository contract | **69** — 69 canonical migrations through `20260803203000_season_predictions.sql`. Development Supabase is hosted at **68** (contract 66 applied 3 August 2026 through the ADR 0024 fast lane, run 30837677979; contract 67 run 30840592967 and contract 68 run 30843028463, all the same day); production remains at **63**. The repository therefore leads development by one additive migration until the next fast-lane run. Non-production Netlify contexts carry `EURO28_DEPLOYED_DB_CONTRACT` from the last alignment and report any gap as informational rather than failing the build (ADR 0024); production keeps the fatal check |
