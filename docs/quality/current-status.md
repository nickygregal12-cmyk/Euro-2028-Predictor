...
| Next executable issue | **Contracts 94–97 are merged, rolled out and Netlify-aligned; nothing from that batch is outstanding.** The next buildable item needing no decision is the ADR 0022 rescoping of `predictor_internal.cup_*` to competition-season scope, behaviour-preserving, changing no qualification, seeding, bye, playoff-pairing or Penalty Number rule. Four items are **blocked on an owner decision, not on engineering**: the LMS `restart_all_reentered` endgame (what "a fresh competition" means), the Cup split-stage persistence shape (`bonus_cup_fixtures_group_shape`), the two known tournament-path defects (the `entry_automatic_submission_outcomes` three-valued-logic hole and REL-001), and the Appendix D.2 post-lock-reveal question against contract 95's deliberate co-member gate. Every contract still merges only with exact-head CI, Database parity, Browser E2E, hosted-inventory and protected Netlify preview evidence, and reaches Development only through the ADR-0024 fast lane with the required confirmation phrase and postflight proof. No provider credential, provider request, Edge Function deployment, C2 work or production write is authorised by repository inclusion. |
