...
| Next executable issue | **All four blocking questions were decided by the owner on 4 August 2026 and are recorded in ADR 0025; nothing is waiting on a decision.** The work queue is now ordinary engineering, in this order: the two tournament-path defects (the `entry_automatic_submission_outcomes` CHECK, precondition audit clean; then REL-001's advisory lock), the Cup split-stage persistence, the Euro post-lock reveal gate removal, and last the LMS restart lifecycle — last because it needs `bonus_competitions`' availability/instance split first, which is the largest single piece outstanding. Contract 98 also awaits a development fast-lane run. Every contract still merges only with exact-head CI, Database parity, Browser E2E, hosted-inventory and protected Netlify preview evidence, and reaches Development only through the ADR-0024 fast lane with the required confirmation phrase and postflight proof. No provider credential, provider request, Edge Function deployment, C2 work or production write is authorised by repository inclusion. |
