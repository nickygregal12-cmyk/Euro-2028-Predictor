...
**What is still true of all three:** no surface renders any of it. A season has scores and a Last Man Standing has survivors, and no player can see either. Contract 94 gives `standings.ts` its SQL counterpart, so the ranking now exists server-side; what is still missing is the bounded read that would let a browser ask for it.
