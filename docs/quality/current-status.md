...
| LMS restart lifecycle — **prerequisite represented at contract 103** | ADR 0025 decision 1 requires a new competition row rather than wiping the completed one. Contract 103 makes that representable without yet creating it: `(tournament_id, game_key)` is unique only across the live public instance while each series has at most one live row, allowing independent private series to coexist; every row carries public/private scope plus season/game-scoped predecessor lineage; `completion_reason` distinguishes a win, restart and abandonment; and one revoked `predictor_internal.live_competition_id` function defines the live public instance. Hosted evidence found no completed competition in either project, so the partial key is equivalent to the replaced total key today. |
