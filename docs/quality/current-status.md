...
| Next executable issue | **Remaining ADR 0025 work:** the Cup split-stage persistence and its cross-phase standings, then the LMS restart lifecycle — last because it needs `bonus_competitions`' availability/instance split first, which is the largest single piece outstanding. Contracts 98–101 all await a development fast-lane run. Every contract still merges only with exact-head CI, Database parity, Browser E2E, hosted-inventory and protected Netlify preview evidence, and reaches Development only through the ADR-0024 fast lane with the required confirmation phrase and postflight proof. No provider credential, provider request, Edge Function deployment, C2 work or production write is authorised by repository inclusion. |
