...
| Repository contract | **107** — 107 canonical migrations through `20260805030000_lms_restart_driver.sql`. Contract 107 is the ADR 0025 decision 1 restart driver: a wiped-out Last Man Standing competition completes as `no_winner_restarted` and a successor is created in the same series, re-entering every entrant and copying no selections, cycles, projections or windows. It is idempotent under a series-scoped advisory lock, and window generation for the successor is deliberately deferred. Contract 106 closes DATA-009: the two Bonus Games rederive functions now resolve through the terminal-aware reader, so a result corrected after a competition completes still rederives instead of silently doing nothing. Contract 104 separates live-only operational callers from terminal-aware current reads; contract 105 enforces one-parent split ancestry and derives the continuing Championship table from settled initial and split fixtures, so carried points are never copied. Development Supabase is hosted at **106**, applied 5 August 2026 by fast-lane run 30984799464 on `aa46fee`; production remains at **63**. Development trails the repository by one until contract 107 is rolled out. Non-production Netlify contexts are at **97** — **read directly from the Netlify project on 5 August 2026, the first independent confirmation rather than an owner report**: `dev`, `branch-deploy` and `deploy-preview` each declare 97 and `production` declares 63, exactly as reported. Nine behind development and the repository alike, and they may now move to the verified development level **106**. |
...
| Next executable issue | **Contract 104:** move the ~10 tournament+game lookups onto `predictor_internal.live_competition_id`, behaviour-identical while only one instance can exist. **Then contract 105:** the restart lifecycle function itself — idempotent, advisory-locked, completing the old row as `no_winner_restarted`, creating the successor, re-entering every previous entrant, copying no selections, cycles, projections or windows, and recording a `public_wipeout_restart` audit event; settlement continues only to derive and report. Contract 102's Cup split creator, scheduler and combined standings read remain later Cup drivers, and the parked cross-phase standings derivation must be rewritten against its phase-aware membership. Contracts 98–103 all await a development fast-lane run. Every contract still merges only with exact-head CI, Database parity, Browser E2E, hosted-inventory and protected Netlify preview evidence. No provider credential, provider request, Edge Function deployment, C2 work or production write is authorised by repository inclusion. |
