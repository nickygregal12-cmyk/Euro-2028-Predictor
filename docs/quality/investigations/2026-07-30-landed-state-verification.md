...
| C2 — `multi-competition-hub-build-plan.md` describes itself as the engineering workstream of the programme plan | `NOT LANDED` | The file does not exist on `origin/main`. It exists from `origin/agent/reframe-platform-forward-docs` (PR #202) upward. On `origin/docs/add-programme-plan` its header does read "# Multi-competition hub — engineering workstream", "**Status:** Child engineering plan within the product programme", "**Parent programme:** [`programme-plan.md`](programme-plan.md)". | The self-description exists and is correctly worded — but only on unmerged branches. Nothing on `origin/main`. |
