...
- the Contract 114 provider poll dispatch: `pg_net` installed, the `net` schema revoked from every browser and service role, and the deployed `provider-poll` Edge Function driven from `pg_cron` at each target's declared cadence. It records no poll target and imports no fixture, so it is behaviour-neutral until an operator supplies two vault secrets and a target;
- the Contract 113 round play window on `competition_rounds`, derived from the fixtures a round is played over, held disjoint per competition season by a trigger whose bounds are inclusive at both ends — so windows that merely touch are refused, and an ambiguous reassignment destination cannot arise from stored data;
