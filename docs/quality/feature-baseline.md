...
- the Contract 120 season play-matchweek read: `get_season_matchweek_card` takes the matchweek as an argument and nothing told the caller what to pass, while `competition_rounds` is revoked from the browser — so the season Match Predictor surface could not be registered as a production route, and a client could not derive the answer without putting a second opinion beside the database's. It exposes `predictor_internal.next_eligible_league_round`, contract 109's authority, which orders by the derived lock instant rather than by ordinal so a rescheduled season resolves to the round that actually locks next. It adds no scheduling logic, returns nothing personal, and distinguishes a season past its last lock (null matchweek) from an unknown or tournament-shaped competition (refused);
