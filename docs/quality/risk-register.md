...
| `TEST-003` | No rule covers drawn geometry, and Stage 12 draws a bracket | **Open, recorded 19 August 2026.** `playwright.vnext.config.ts` records the right technique on the Stage 10 chart — the browser reads plotted coordinates out of the rendered SVG, because a chart drawn upside down is invisible to any assertion that cannot see geometry — but it is a comment about one component rather than a stated expectation. A knockout bracket is harder: ordering, connectors, byes and walkovers are all wrong in ways that render successfully. Closure is the precedent stated as the lane's rule for anything positioned by computed geometry, before the bracket is drawn. Stage 12 is in progress. |
