...
| `ING-002` | The provider status vocabulary is measured for four tokens and guessed for eight | **Partly closed at contract 209.** SportMonks `10` is now measured from retained payloads. SportMonks `14`–`21` were seeded by contract 135 from published documentation rather than from a payload, and the evidence now contradicts one of them: `10` is what this provider actually sends for a postponement, while `14` is mapped `postponed` on no measurement at all. | Measure each of `14`, `15`, `16`, `17`, `18`, `20` and `21` against a real payload before relying on it, or remove it so it fails closed to `unknown`. Contract 209 deliberately removed none: replacing one guess with another is not an improvement, and an `unknown` token is recorded in `provider_status_observations` where it can be measured. |
...
| `ING-005` | Prediction deadlines are enforced per fixture but published per matchweek | **Open; predates contract 209 and is not made worse by it. DIRECTION DECIDED 20 August 2026, and the frontend half is already in place.** Contract 119 made ENFORCEMENT per fixture for a rescheduled match; `get_season_matchweek_card` still publishes the matchweek instant. A rescheduled or postponed fixture therefore reads as locked on the Match Predictor while the trigger would accept the write — the surface is stricter than the rule, so nothing illegal is possible, but a player is told they cannot do something they can. **The direction is to PUBLISH per-fixture editability rather than to reverse contract 119**, and the reason is that the alternative is not available: reversing 119 would re-close the window ADR 0020's 5 August amendment deliberately opened, on the exact case it was written about. **The frontend cannot close this on its own, and that is the load-bearing fact.** Contract 119 chose the NARROW reading — only a fixture with a row in `predictor_internal.season_fixture_revisions` gets its own lock — so "has this moved?" is a stored fact with provenance that no browser can see. A surface that inferred it by comparing a kickoff against the round's minimum would implement the WIDE reading the owner rejected, and an ordinary Friday-to-Monday matchweek would become predictable in stages, with Monday's game open after Saturday's results. **What the frontend already has is the shape to receive it.** `PredictorFixture.editable` is carried per fixture today and its own docblock says why: *"it is the card's `editable` today, because the main predictor's lock scope is the whole matchweek, and it is carried per fixture anyway so a future per-match lock policy needs no new field and no new component."* `tests/vnext/predictorLockTruth.test.ts` pins the current behaviour to that field and names this finding, so the change lands in one mapper line rather than being hunted for. | **Closure is a contract that publishes per-fixture editability on `get_season_matchweek_card`, plus its hosted rollout.** Not taken in this batch for two reasons, both external: the contract number this needs was not free while the provider-cadence work was open — it took 210 and 211, which have since merged — and a published field is worth nothing until every environment that serves the read has it — so the mapper must treat the field's ABSENCE as today's answer, which keeps the frontend correct on both sides of the rollout. Reversing contract 119 instead is refused above. |
| `ING-006` | A postponement can be up to 22 hours stale, and the staleness spans the prediction lock | **Closed in the repository at contracts 210 and 211; awaiting rollout to both hosted environments.** The measurement stands: the enabled SportMonks target polled `cadence_minutes = 1440` with a live window opening `live_lead_minutes = 15` before kickoff, while the Main Predictor matchweek locks at the earliest kickoff of the round with every caller passing a buffer of **0** — so a postponement announced in the day before a deadline was applied only after predictions had locked. Masked entirely by `ING-001`, which meant nothing was observed at all. | **Contract 210 fixed the rule and contract 211 fixed the instrument, and BOTH corrections are worth reading because each replaced an answer that looked right.** The row originally proposed raising `cadence_minutes` to `360`; that was withdrawn as the worse fix, because it spends four times the credit every day of the year and still leaves hours of silence immediately before a lock. Contract 210 instead widened `live_lead_minutes` to `720` — correct coverage, wrong dial. `provider_target_is_live` is tournament-level, so on a real 72-hour Premier League matchweek those windows chain into one block: **42 hours open and 252 polls**, against contract 146's published budget of about **58 requests on a matchday**. Contract 211 splits the question in two: `deadline_cadence_minutes` (60) inside a watch opening `deadline_lead_minutes` (720) before an unresolved kickoff and closing AT it, with `live_lead_minutes` returned to `15` for the in-play question it was chosen for. **The watch stands open 36 hours for 36 polls; the matchweek falls from 252 to 77 while covering more of it.** A postponement announced in the twelve hours before a deadline is seen within the hour. pgTAP `256` asserts the deadline relationship, `257` asserts the cost as well as the correctness — a fix that has to be watched for cost is one that eventually gets turned off. Widening the eight-day fixture window remains separate and is still NOT proposed. |
