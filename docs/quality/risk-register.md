...
| `ING-006` | A postponement can be up to 22 hours stale, and the staleness spans the prediction lock | **Closed in the repository at contract 210; awaiting rollout to both hosted environments.** The measurement stands and is worth keeping: the enabled SportMonks target polls `cadence_minutes = 1440` with a live window opening `live_lead_minutes = 15` before kickoff, so for a 22 August 14:00 kickoff the last routine look before it was 21 August ~15:40 and the next was 13:45 on match day — while the Main Predictor matchweek locks at the earliest kickoff of the round, with every caller passing a buffer of **0**. Players could be locked into a match the provider had already called off. This was masked entirely by `ING-001`, which meant nothing was observed at all. | **`20260820070000_provider_live_window_covers_the_deadline.sql` raises `live_lead_minutes` to `720` — the maximum `provider_poll_targets_live_window_bounded` allows — on every enabled target and as the column default, so the deadline falls inside a ten-minute polling window instead of behind up to a day of silence. THE FIX PROPOSED IN THIS ROW ORIGINALLY WAS THE WORSE ONE AND IS WITHDRAWN: raising `cadence_minutes` to `360` would spend four times the provider credit every day of the year, including weeks with no fixture near, to shrink a gap that only exists in the hours before a lock — and would still leave up to six hours of silence immediately before it, which is precisely where the gap does its damage. `cadence_minutes` is therefore left at `1440`. pgTAP suite `256` asserts the relationship rather than the number: that the window is open AT the matchweek lock, that the old 15-minute lead left it shut an hour before, that a six-hour idle cadence would not have opened it either, and that the window still closes once every fixture has a score. Rollout to Development and Production is pending and carries its own gates. Widening the eight-day fixture window remains a separate question and is still NOT proposed. |
