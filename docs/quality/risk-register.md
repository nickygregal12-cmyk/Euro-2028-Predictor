...
| `OPS-014` | The Euro 2028 production deployment is 401 commits and 28 contracts behind the database it queries | **Opened 27 August 2026, measured directly.** `https://euro28predictor.com/release.json`, read behind the site-password gate, reports `commit 07f59af6fd0acb4992c16155d634a7223fc318a6`, `applicationContract 190` and `hostedContract 190`, against Supabase project `vkfnsqdyhvtwyqkisxhk`. That commit is *Add online Graphify and OmniRoute agent tooling (#844)*, dated 18 August 2026 and an ancestor of `main`; `git rev-list --count 07f59af6..origin/main` is **401**. The database that deployment queries is the same production project the Hub uses, independently verified at **contract 218** on 24 August and recorded in `config/production-hosted-contract.json`. So a shipped client built against contract 190 is talking to a contract-218 database. **`applicationContract` and `hostedContract` are both build-time values** — the first is `config/deployment-contract.json` at the built commit, the second is Netlify's `EURO28_DEPLOYED_DB_CONTRACT` variable (`vite.config.ts`, `createReleaseMetadata`). Both being 190 means the variable is stale too, so `validate-deployment-contract.mjs` would pass on the next Euro build while still declaring the wrong number. This is the same family as `OPS-011`. **WHAT IT IS NOT.** It is not a claim that the Euro product is broken. Its tournament routes sit behind `TournamentJourney` plus the contract-143 publication gate, and `euro_publication_state` is `hidden`, so almost nothing there is reachable; the four served destinations render `EuroDestinationPage`, whose only read is `euro_publication_state()` at contract 143 — well below 190. No defect is asserted, and none was looked for. **WHAT MAKES IT WORTH TRACKING.** Two things. (1) The final completion contract requires Euro Production state to be reported truthfully, and until this was read nobody knew it. (2) `netlify.toml` is shared by both sites and the Euro deployment PREDATES the fourteen `VITE_UI_FOOTBALL_HUB_*` flags — so the next redeploy of the Euro site from current `main`, for any reason, applies all fourteen at once as a side effect of an unrelated change. That is an unrehearsed cutover of a second product, and it should be a decision rather than a consequence. **Closure:** an owner decides whether the Euro site is redeployed now, deliberately and with its own verification, or deliberately left where it is with that fact recorded. Either is defensible; the current state is neither, because nothing recorded it. |
