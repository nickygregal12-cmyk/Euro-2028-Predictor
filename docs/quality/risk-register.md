...
**Development candidate:** PR #193 plus stacked PR #196, contract 63; development Supabase verified at 63, non-production Netlify alignment remains owner verification.  
**Recovery:** fresh encrypted production backup, disposable restore and contract-60 backup verifier passed.

Current code, executable tests and verified hosted evidence override older classifications. Production is the controlled final target, not an active tournament service. The development-63/production-60 split is intentional and must remain recorded until promotion.

## Current contract movement

| ID | Current position |
| --- | --- |
| `OPS-006` | **Controlled split, not accidental divergence.** Repository and development Supabase are at 63; non-production Netlify contract-63 alignment requires owner verification; production remains aligned and locked at 60. |
| `POSTLOCK-001` | **Resolved in development by PR #193/#196.** Bounded post-lock consensus and the richer locked My Entry state are implemented; production promotion remains pending approval. |
...
| `PRIV-001` | **Resolved in development by contract 63 on PR #196.** Tournament-wide consensus is suppressed below ten submitted entries; the caller counts, the public response is an explicit success state and browser roles cannot execute the unsuppressed helper. |
...
| `MIG-001` | **Resolved by PR #196.** The pull-request guard fetches `origin/main`, checks every added migration against main's highest timestamp, enforces strict internal ordering and passes when no migrations are added. |
| `DATA-003` | **Resolved and hosted.** Same-tournament/reference guards are present in both hosted environments. |
| `DOC-001` | **Resolved and actively maintained.** The contract-63 development split is recorded here and in `current-status.md`. |
...
| `TEST-001` | **Reduced.** Candidate-63 consensus/privacy/final standings now have unit, pgTAP, desktop/phone and axe coverage; manual accessibility and later full-volume/rollback rehearsals remain. |

## Critical

| ID | Finding | Current status | Required closure |
| --- | --- | --- | --- |
| `OPS-006` | Uncontrolled production contract divergence | **Resolved for production; controlled development split active** | Do not promote or deploy contract 63 to production without backup, approval and exact verification. |
...
| `POSTLOCK-001` | Locked entries lacked a crowd/trends experience | **Resolved in development** | Contract 61 exposes bounded submitted-entry aggregates; contract 63 suppresses tournament-wide output below ten entries. Production promotion remains. |
...
| `MIG-001` | Concurrent branches can add stale/colliding migration timestamps | **Resolved by [PR #196](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/pull/196). Implementation: [`check-migration-timestamps.mjs`](../../scripts/check-migration-timestamps.mjs), including fresh `origin/main` fetch and strict per-file/order checks. Validation: [`migrationTimestampGuard.test.ts`](../../tests/scripts/migrationTimestampGuard.test.ts) proves no-addition, collision, multiple-valid and duplicate-order cases; exact-head CI [30468772275](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/30468772275) passed.** |
| `PRIV-001` | Tournament-wide prediction consensus had no minimum cohort | **Resolved in development by contract 63 migration [`20260729154931_prediction_consensus_minimum_cohort.sql`](../../supabase/migrations/20260729154931_prediction_consensus_minimum_cohort.sql). Threshold 10 includes the caller; suppression is a successful state. Validation: [`117_prediction_consensus.sql`](../../supabase/tests/117_prediction_consensus.sql), [`119_prediction_consensus_minimum_cohort.sql`](../../supabase/tests/119_prediction_consensus_minimum_cohort.sql), response-model tests, CI [30468772275](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/30468772275), Database parity [30468772538](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/30468772538) and authenticated Browser E2E [30468772387](https://github.com/nickygregal12-cmyk/Euro-2028-Predictor/actions/runs/30468772387). Development Supabase was owner-verified at 63 on 29 July 2026; production remains 60.** |
