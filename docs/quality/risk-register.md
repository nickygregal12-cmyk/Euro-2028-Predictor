...
| CSP allows inline styles | **Confirmed** | `netlify.toml` — `style-src 'self' 'unsafe-inline'`. Still true of the ENFORCED policy; the tightened one now runs beside it in report-only mode. See `SEC-002` |
...
dispatched URL can ever match. Fixed by contract 209.
...
unchanged kickoff. Seeded as `postponed` by contract 209, with that evidence
...
approving a contract 174 proposal. Fixed by contract 209, which applies and
...
per-fixture lock read the elapsed instant. Fixed by contract 209.
...
contract 209: the presenter refuses to format an instant that is only a memory,
...
**Before contract 209**, with the committed pre-209 resolvers installed, both
...
**After contract 209**, with nothing else changed, the same response resolves,
...
**What was real and what was stood in for.** Every function contract 209 writes
...
score-matching CHECK constraints. The functions contract 209 does not touch were
...
CI, where `supabase/tests/255_provider_fixture_lifecycle.sql` drives the same
...
| --- | --- | --- | --- |
| `ING-001` | Provider ingestion resolved no season for nine days, so nothing was imported at all | **Repository-fixed at contract 209 and replayed against the real archived response; hosted Development still carries the outage.** Both arms of `resolve_provider_response_season` were broken — an equality lookup against a composite season key, and a URL compared against a path still holding contract 146's `{{date:+N}}` placeholders. 13 consecutive responses consumed `unresolved_season`. | Apply contract 209 to Development, then confirm the next consumption records outcome `applied` and that `season_fixture_live_state` gains rows again. **A backlog does not replay itself:** `provider_response_consumption` is keyed by processing id, so the 13 already-consumed responses stay consumed and the recovery arrives with the next poll rather than from history. |
| `ING-002` | The provider status vocabulary is measured for four tokens and guessed for eight | **Closed for SportMonks in the repository at contract 209 and contract 213; awaiting rollout to both hosted environments. One residual is named below and is deliberately still open.** Contract 209 measured `10` from retained payloads. Contract 213 removed the seven that were seeded by contract 135 from published documentation and never observed: `14`, `15`, `16`, `17`, `18`, `20`, `21`. (`19` was never seeded, which is why it is not listed as dropped.) **The evidence check came first and could have stopped the removal:** read-only against both hosted environments on 20 August 2026, Development's 440 retained SportMonks responses (2026-08-05 to 2026-08-19) contain `state_id` values `1, 2, 3, 5, 10, 22` and its `provider_status_observations` holds 24 rows, all token `22`; Production's 21 responses (2026-08-10 to 2026-08-19) contain `1, 5, 10` and its single observation is token `10`. Not one of `14`–`21` appears on either environment. | **Removed, not remapped, and the distinction is the whole point.** Contract 209 deliberately remapped none, because replacing one guess with another is not an improvement. Removal IS the improvement: an unmapped token resolves to `unknown` through `provider_status_kind`, an `unknown` is recorded in `provider_status_observations` once per response with the number of fixtures it blocked, and nothing downstream acts on a fixture from a status the system has never seen. For `15` (`cancelled`) and `16`–`18` (`abandoned`) that matters most — acting on either would take a fixture away from a player on no evidence this provider ever says those words. After contract 213 exactly one SportMonks token resolves `postponed` and it is the measured one, and NO SportMonks token resolves `cancelled` or `abandoned` at all. pgTAP `259` asserts the behaviour rather than the row count. **What would be needed to re-add any one of them:** a retained payload in which that token appears, plus the observation row the appearance writes, plus a reading of what the fixture actually did across consecutive polls — the same standard `10` was measured to at contract 209, and no lower one. **RESIDUAL, RECORDED RATHER THAN ACTED ON:** token `4` ("break", mapped `in_play`) did not appear in either environment's retained payloads either. It is NOT dropped, for evidential reasons rather than convenience — the retained window is two weeks of pre-season and opening fixtures, thin for a token that only occurs during a stoppage, and `4` sits between `2` and `3`, both observed, in a contiguous in-play run. Dropping it would also change how a LIVE match reads, which contract 213 is not for. Measuring `4` against a payload from a stopped match is the next thing owed here. |
| `ING-007` | The legacy production fixture surfaces had no representation of an abnormal fixture status | **Closed at contract 209, repository-side.** `fixtureListModel` carried `played` and nothing else, so a postponed, abandoned or voided fixture rendered as an ordinary row at a kickoff that would not happen — on the generation of the UI production serves today, not the flagged vNext one. | Regression-tested in `tests/features/season/fixtureListModel.test.ts`, including that the three abnormal states produce three different accessible sentences, that none of them says "kick-off", and that the wording matches the vNext lane's from the same shared helper. Reopen if a fourth surface starts formatting `kickoffAt` itself instead of reading `row.kickoff`. |
| `ING-005` | Prediction deadlines are enforced per fixture but published per matchweek | **Closed in the repository at contract 212; awaiting rollout to both hosted environments.** The measurement stands: contract 119 made ENFORCEMENT per fixture for a rescheduled match, while `get_season_matchweek_card` published only kickoffs, so every client derived one matchweek instant from the earliest of them. A rescheduled or postponed fixture therefore read as locked on the Match Predictor while the trigger would have accepted the write — the surface was stricter than the rule, so nothing illegal was possible, but a player was told they could not do something they could. | **The two remediations were mutually exclusive and the FIRST was chosen: publish the per-fixture instant. Contract 119 is NOT reversed and no enforcement line moves.** Contract 212 publishes `lock_at` and `locked` per fixture on the card, derived from `predictor_internal.season_prediction_lock_at` at the buffer the enforcement trigger itself reads through the new `predictor_internal.season_prediction_buffer_minutes` — asked, not re-implemented, because a second copy of the rule would agree on every ordinary fixture and differ on the one that moved. BOTH fields, because two of the authority's answers have no printable instant: `-infinity` for a void or abandoned fixture and `infinity` for a postponement with no announced replacement date. **A null derivation publishes `locked` TRUE**, so a matchweek with unconfirmed kickoffs fails closed rather than inviting an edit the trigger would refuse — the mirror image of this fault would be no better than it. The Match Predictor draws each fixture against its own lock; the Joker and card confirmation stay matchweek-level, because they are matchweek-level acts. pgTAP `258` asserts the RELATIONSHIP rather than a constant, and seeds the divergent fixture deliberately: a suite of ordinary fixtures would have passed against the broken card. |
| `ING-006` | A postponement can be up to 22 hours stale, and the staleness spans the prediction lock | **CLOSED — contract 210 and contract 211 are applied to BOTH hosted environments, measured read-only on 20 August 2026.** Development reached 211 at `09:54Z` by fast-lane run `32353837721`; Production crossed 209 to 211 as one boundary at `10:32Z` by guarded rollout `32359164255`, gated on encrypted backup `32353845022` and exact-head rehearsal `32358542321`. On Production BOTH enabled targets — `sportmonks` and `football-data` — read `cadence_minutes` 1440, `deadline_cadence_minutes` 60, `deadline_lead_minutes` 720, `live_cadence_minutes` 10, `live_lead_minutes` 15, and `predictor_internal.provider_target_awaits_deadline` is installed with the `deadline_lead_minutes` column default at 720 so a target created later inherits the watch rather than the gap. The measurement stands: the enabled SportMonks target polled `cadence_minutes = 1440` with a live window opening `live_lead_minutes = 15` before kickoff, while the Main Predictor matchweek locks at the earliest kickoff of the round with every caller passing a buffer of **0** — so a postponement announced in the day before a deadline was applied only after predictions had locked. Masked entirely by `ING-001`, which meant nothing was observed at all. | **Contract 210 fixed the rule and contract 211 fixed the instrument, and BOTH corrections are worth reading because each replaced an answer that looked right.** The row originally proposed raising `cadence_minutes` to `360`; that was withdrawn as the worse fix, because it spends four times the credit every day of the year and still leaves hours of silence immediately before a lock. Contract 210 instead widened `live_lead_minutes` to `720` — correct coverage, wrong dial. `provider_target_is_live` is tournament-level, so on a real 72-hour Premier League matchweek those windows chain into one block: **42 hours open and 252 polls**, against contract 146's published budget of about **58 requests on a matchday**. Contract 211 splits the question in two: `deadline_cadence_minutes` (60) inside a watch opening `deadline_lead_minutes` (720) before an unresolved kickoff and closing AT it, with `live_lead_minutes` returned to `15` for the in-play question it was chosen for. **The watch stands open 36 hours for 36 polls; the matchweek falls from 252 to 77 while covering more of it.** A postponement announced in the twelve hours before a deadline is seen within the hour. pgTAP `256` asserts the deadline relationship, `257` asserts the cost as well as the correctness — a fix that has to be watched for cost is one that eventually gets turned off. Widening the eight-day fixture window remains separate and is still NOT proposed. |
...
| `OPS-013` | `smoke:production` is anonymous by construction and neither site answers anonymously, so the programme's own release instrument cannot run | **Opened 27 August 2026, and REVISED the same day once the owner supplied the site password. Both halves are recorded because the first was the pessimistic reading and the second is the measured one.** The vNext final completion contract ([`../product/vnext-final-programme-completion.md`](../product/vnext-final-programme-completion.md)) requires both products to be live in Production and to have passed post-deploy smoke checks. `npm run smoke:production` is the repository's instrument for that and it is an **anonymous HTTP** check by construction. **AS OPENED:** `GET https://predictorhub.netlify.app/` and `GET https://euro28predictor.com/release.json` both answer **HTTP 401** behind Netlify's `Password Protection` interstitial, and so do the per-pull-request deploy previews — so the shell probe, the release-identity probe, the SPA-routing probe, the asset probe and the Supabase-isolation probe all fail closed, and the entry was written concluding that no session could produce the evidence. **AS REVISED:** with the site password the gate is a form POST that sets a cookie, and behind it BOTH SITES ANSWER. The release identities were read directly and are recorded in the correction record below. So the finding is narrower and more precise than it was opened as: the obstruction is not that the evidence is unobtainable, it is that **the repository's own instrument cannot obtain it**, because `production-smoke.mjs` accepts an origin and no credential. A person with the password can verify a release; `npm run smoke:production` cannot, and a gate that only a human can run is not a gate. **It is not a defect in the sites.** Password protection on a pre-launch product is a deliberate and correct posture, and nothing here argues for removing it. **Closure remains an owner action, and the options are now sharper:** either the protection is lifted at launch, or `production-smoke.mjs` gains an authenticated path (the site password read from a secret, exchanged for the cookie before the probes run) so the instrument works while the posture stands. The second is repository work that an owner must first authorise, because it means a deployment secret exists for the smoke to read. Until one of those, Stage 14's `CUT OVER AND VERIFIED` state is evidenced by hand rather than by a gate. |
| `OPS-014` | The Euro 2028 production deployment is many contracts behind the database it queries, and the variable that would catch it is stale too | **Opened 27 August 2026, measured directly. THE VALUES ARE DELIBERATELY NOT RESTATED IN THIS ROW** — they are in the dated correction record below, beside the reading that produced them, and the machine records in `config/` remain the only home for what any environment is at. A register row that carried them would be the second copy that goes stale, which is the failure `DOC-001` exists for. **The shape of it.** `euro28predictor.com/release.json`, read behind the site-password gate, reports a commit from 18 August that is an ancestor of `main` with several hundred commits since, and an application contract materially behind the one `config/production-hosted-contract.json` verifies for the production database that same deployment queries. So a shipped client is talking to a database well ahead of the client that was built against it. **Both figures in `release.json` are build-time**, per `createReleaseMetadata` in `vite.config.ts`: the application figure is `config/deployment-contract.json` at the built commit, and the hosted figure is Netlify's `EURO28_DEPLOYED_DB_CONTRACT` variable. Both being the old value means the variable is stale as well, so `validate-deployment-contract.mjs` would pass on the next Euro build while still declaring the wrong number. Same family as `OPS-011`. **WHAT IT IS NOT.** Not a claim that the Euro product is broken. Its tournament routes sit behind `TournamentJourney` plus the contract-143 publication gate, `euro_publication_state` is `hidden`, and the four served destinations render `EuroDestinationPage`, whose only read is `euro_publication_state()` — a contract far below the deployed one. No defect is asserted and none was looked for. **WHAT MAKES IT WORTH TRACKING.** Two things. (1) The final completion contract requires Euro Production state to be reported truthfully, and until this was read nobody knew it. (2) `netlify.toml` is shared by both sites and the Euro deployment PREDATES the fourteen `VITE_UI_FOOTBALL_HUB_*` flags — so the next redeploy of that site, for any reason, applies all fourteen at once as a side effect of an unrelated change. That is an unrehearsed cutover of a second product, and it should be a decision rather than a consequence. **Closure:** an owner decides whether the Euro site is redeployed now, deliberately and with its own verification, or deliberately left where it is with that fact recorded. Either is defensible; what it was is neither, because nothing recorded it. |

## Correction record — 27 August 2026, both Production sites read directly

`OPS-013` was opened earlier the same day on an anonymous probe and concluded
that no session could produce the programme's post-deploy evidence. The owner
then supplied the Netlify site password, and the conclusion was wrong in the
direction that mattered: the sites answer, and what they say resolves two
questions the programme had been carrying as unknowns.

**Read at 2026-08-27, `GET /release.json` behind the password gate. These are
observations at a moment, not a new authority — `NOW.md` and the machine records
remain the homes for contract state, and this record does not update them.**

| | commit | application contract | declared DB contract | Supabase |
| --- | --- | ---: | ---: | --- |
| `predictorhub.netlify.app` | `014123c5` | 218 | 218 | `vkfnsqdyhvtwyqkisxhk` |
| `euro28predictor.com` | `07f59af6` | 190 | 190 | `vkfnsqdyhvtwyqkisxhk` |
| `deploy-preview-1067--predictorhub` | `b391b4d9` | 218 | 218 | `iouzoutneyjpugbbtdem` |

**THE FOOTBALL HUB CUTOVER IS LIVE.** Production serves `014123c5`, which is
current `main`, and `netlify.toml` sets all fourteen `VITE_UI_FOOTBALL_HUB_*`
flags in `[build.environment]`. Those flags are build-time, so a Hub player today
is on the vNext surfaces. The repository had no record saying so, and
`docs/product/vnext-cutover-capability-parity.md` had been reporting the cutover
as pending its own implementation.

**What this does NOT establish**, and the distinction is the whole reason this is
a dated observation rather than a status change: reading `release.json` proves
which BUILD is published. It does not prove the post-deploy behaviour the
completion contract asks for — routes, authentication, refresh, deep links and
the critical journeys. Those need the smoke run or an equivalent, and
`OPS-013`'s revised text says why the instrument cannot yet do it.

**The preview correctly isolates its database.** `deploy-preview-1067` names the
development project and production names the production one, which is the
boundary `production-smoke.mjs` exists to assert and which held here.

**And the Euro deployment is stale**, which is `OPS-014` above.
...
| `DOC-004` | Stage 15 must audit against vNext primitives that are not enumerated anywhere | **Closed 27 August 2026, on the exact closure this row named.** Opened 19 August: Stage 15's completion predicate requires every major Euro 2028 surface to be audited against the vNext quality and system primitives and that shared components are reused only where their semantics match, and the global half of that list was distributed through `src/vnext/AGENTS.md` alongside the shell contract, the integration contract and roughly thirty invariants — so an auditor had to re-derive it by inferring which paragraphs described something reusable. The closure this row asked for was "one short enumerated list that `src/vnext/AGENTS.md` points at", and that is what landed: [`../product/vnext-primitives.md`](../product/vnext-primitives.md), enumerating foundations, the football/game/social components, the chrome, the states and the models, with a pointer row added to the surface-authority table in `src/vnext/AGENTS.md`. **It holds no rule and says so.** The boundaries stay in `AGENTS.md`, the direction stays in `ui.md`, each module's own docblock outranks its one-line summary here, and current source outranks the page — because a second list of components is exactly the kind of page that goes stale silently. It also carries the half the finding implied but did not state: **what is NOT a primitive** — `integration/`, `fixtures/`, the workshop, and every complete surface including Home, which is the visual quality reference and explicitly not a page template. |
| `OPS-012` | The vNext and Storybook checks are not required for merge, and a merge can cancel them mid-run | **Confirmed 19 August 2026, and reclassified — the cause is a deliberate trade-off rather than an oversight.** Measured on two of this repository's own pull requests: on #910 `CI / Required merge gate` reported success at 10:44:02 and the pull request merged at 10:44:07, five seconds later, while the vNext `layout` suite was still running — it was **cancelled at 10:55:08** by that merge and never reported a conclusion on that head. #914 repeats the pattern: gate green 12:06:30, merged 12:06:30. `authenticated-browser` also completed after #910 had merged. **The cause is written down and was reasoned about.** `specs/tooling-assurance-activation/plan.md` names the constraint under *Risks to falsify* — "path-scoped workflows must not become impossible required checks; the main CI aggregate must always report a conclusion" — which is the correct GitHub concern: a required context that a `paths:` filter prevents from ever posting blocks the pull request forever. So the gate aggregates the always-running `ci` job alone, on purpose. **What remains true is the review's point:** the suite that proves a vNext stage's layout contract is not a merge condition, so the controller's stage-transition requirement that exact-head required CI is green can be satisfied while that suite is cancelled, unreported or red. **Closure is therefore NOT "add the contexts to the ruleset"** — that recreates exactly the failure the plan warned about. Either the path-scoped workflow gains a companion job that always reports a conclusion (success when its paths did not match) and *that* becomes required, or the vNext programme's own transition check verifies the layout suite's conclusion on the exact head rather than trusting the aggregate. Both are repository work; the ruleset half is an admin action in GitHub settings and cannot be performed from a session. **Updated 25 August 2026, and one sentence above was wrong.** The ruleset half cannot be *changed* from a session; it can be **read** from one — `GET /rules/branches/main` needs only read access, and every previous attempt recorded the obstruction without testing it. That mattered, because the repository had started asserting the hosted set from prose: #1047 and a comment in `mergeGateConfiguration.test.ts` both named `vNext merged browser gate` as a required context. It never was. Read on 25 August, ruleset `20508177` requires `CI / Required merge gate` and — added by the owner that morning — `Migration safety / Required migration gate` and `Database parity / Required parity gate`. **The first of the two closures this entry names is now half-done rather than pending:** the companion always-reporting job exists and is proven, and only the hosted requirement is outstanding, which is genuinely an owner action. The tracked set lives in [`config/required-merge-contexts.json`](../../config/required-merge-contexts.json), CI refuses a job rename that would stop a required context posting, and the Conductor doctor re-reads the hosted set — so the next drift is found by a command rather than by an audit. |
...
| `SEO-002` | Metadata largely global | **Advanced, not closed.** The one address where a global card was actively misleading now has its own: `/join/:code` is rewritten to `join.html`, a document derived from the built `index.html` at build time and re-headed by `applyDocumentHead(..., inviteShareCard(site))`, so an invitation pasted into a chat says it is an invitation instead of describing the product. Its card names no league and takes no code — see `SEC-001` — and it carries `noindex, nofollow`. Everything else is still global, which is why this stays open: a competition or match link unfurls as the site. |
...
| `UX-008` | A confirmed matchweek card keeps saying so after the player changes a prediction | **Open, medium, found by reproduction 20 August 2026.** `SubmissionReceipt` prints "Predictions submitted" and "Your card is confirmed" whenever `cardStatus === 'confirmed'`. `confirm_season_matchweek_card` sets that status and stamps `season_matchweek_cards.confirmed_at`; **nothing clears either when a prediction changes afterwards**, and the card stays open to changes until the lock. So a player who confirms and then edits a scoreline is told the card they are now looking at is confirmed, when what was confirmed was a different card. Reproduced against the committed migrations on a local cluster: confirm, then `save_season_prediction` on one fixture, and `card_status` is still `confirmed` with the original `confirmed_at`. **Two related gaps in the same read:** `get_season_matchweek_card` returns `card_status` but **not** `confirmed_at`, so the instant the server already holds never reaches the browser; and there is no compact confirmation reference at all. `SubmissionReceipt`'s own header records both absences honestly and refuses to invent either, which is why this is a backend gap rather than a UI defect. | Smallest closure, as one contract: clear the confirmation when a prediction, Joker or card composition changes while the matchweek is still open; expose `confirmed_at` on the card read; add a short human-readable reference derived from data the server already has. **Do not** add a per-save receipt row — the requirement is explicitly one current confirmation on the existing card authority, not an append-only ledger of every score edit. Contract number deliberately unclaimed here — check `NOW.md` for the next free one and check open pull requests before claiming it, because two branches claiming one number is a known failure in this repository. (Written while 211 was contested by an open pull request; that pull request has since merged, which is exactly why the number is not written down here.) |
...
| --- | --- | --- | --- |
| `UX-007` | vNext's sticky masthead may obscure the keyboard-focused control once the shell becomes the production frame (WCAG 2.2 AA, 2.4.11) | **CLOSED, MEASURED 23 August 2026.** *Original entry, kept as written:* Open, recorded 19 August 2026. Suspected from the CSS — sticky `top: 0` masthead, no `scroll-padding-block-start` in the lane — and deliberately NOT asserted: the browser probe written for it was unreliable and is not carried as evidence. Now reachable, and still unmeasured. The Football Hub cutover made the shell the production frame, so the document-level scroll this criterion is about exists — the criterion can be exercised, and has not been. **What closing it actually took, and it was less than this row implied.** Both halves already existed and neither had been run: `src/vnext/foundations/useStickyScrollPadding.ts` measures the masthead and the bottom bar with a `ResizeObserver` and writes `scroll-padding-block-start`/`-block-end` on the DOCUMENT element — the scroller — rather than a constant, because the masthead's height moves with the context bar, the attention band and a wrapping page header; and `e2e/vnext-shell.spec.ts` tabs sixty stops through `vNext/Focus Not Obscured`, a `layout: 'fullscreen'` story in one `VNextRoot` with no device frame, so the iframe's own document scrolls exactly as the window does in production. **The probe is hit-tested rather than rectangle arithmetic**, which is what the 19 August entry's "unreliable" almost certainly was: comparing boxes reports the skip link and the rail switcher as covered, because both sit in the masthead's vertical BAND and neither is under it — one paints above and one is beside. `elementFromPoint` at five points asks the question the criterion asks, and 2.4.11 (Minimum) is satisfied while any part is hittable. **Measured: three consecutive runs, both viewports (375×720 where the bottom bar is real, 1440×900 where the rail replaces it), six passes, zero obscured stops**, plus the assertion that the scroller's top padding is neither `auto` nor zero so a failure reads as "the padding is wrong" rather than "missing". | Closed. Reopen only if the assertion is removed, if the shell gains sticky chrome the hook does not measure, or if a run reports an obscured stop. |
