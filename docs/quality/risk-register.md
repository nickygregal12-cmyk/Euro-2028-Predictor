...
| `OPS-013` | Neither Production site answers anonymously, so the programme's own completion evidence cannot be gathered from a session | **Opened 27 August 2026, measured rather than inferred.** The vNext final completion contract ([`../product/vnext-final-programme-completion.md`](../product/vnext-final-programme-completion.md)) requires both products to be live in Production and to have passed post-deploy smoke checks covering routes, authentication, refresh, deep links and critical journeys. `npm run smoke:production` is the repository's instrument for the first half of that, and it is an **anonymous HTTP** check by construction. Measured on 27 August: `GET https://predictorhub.netlify.app/` and `GET https://euro28predictor.com/release.json` both answer **HTTP 401** with Netlify's `Password Protection` interstitial. So the shell probe, the `/release.json` release-identity probe, the SPA-routing probe, the asset probe and the Supabase-isolation probe cannot run against either site, and no session can produce the evidence the completion contract asks for. **This is not new and was not previously tracked as a finding.** `docs/ops/records/production-application-release-145.md` recorded it in passing on 10 August — *"Anonymous release smoke could not run against a protected site"* — and it stayed an aside in a dated record rather than an open item, which is why the programme kept reaching this point and re-discovering it. **What it does NOT mean.** It is not evidence that the cutover has not shipped, and it is not a defect in the sites: password protection on a pre-launch product is a deliberate and correct posture. It means the repository cannot *tell*, and a programme predicate that cannot be evaluated must be reported as unevaluated rather than assumed either way. **Closure is an owner action and cannot be performed from a session:** either the protection is lifted for the launch, or an authenticated smoke path is provided (Netlify site-password header or a bypass token held as a secret), or the owner runs the smoke themselves and records the result in `docs/ops/records/`. Until one of those, Stage 14 stays at `READY FOR CUTOVER` and the final programme audit cannot be run to completion. |
