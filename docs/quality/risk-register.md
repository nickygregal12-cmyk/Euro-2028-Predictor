...
| `TYPE-002` | Two compiler checks that catch AI-shaped mistakes are off, so absent-versus-undefined and unchecked index access are unproven | **Half-closed 17 August 2026 — `exactOptionalPropertyTypes` is now ON, at zero errors, project-wide.** `tsconfig.app.json` sets `"exactOptionalPropertyTypes": true`; because `tsconfig.test.json`, `tsconfig.tools.json` and `tsconfig.gates.json` all `extend` it, the flag also covers `tests/`, `scripts/**/*.ts`, `supabase/functions/**/*.ts`, the Playwright configs and the JSDoc-typed `.mjs` deploy gates — not just `src/`. `npx tsc -b` (what `npm run build` runs, what CI runs) is clean at **zero** errors across every referenced project. This is enforcement, not a count reduction: **verified by mutation** — reverting one fix (`SignUpForm.tsx`'s `captchaToken?: string \| undefined` back to `captchaToken?: string`) reproduces the exact original compiler error, so a regression is caught, not silently re-admitted. Closure took two shapes. Most of the ~120 sites were genuine widenings, `prop?: T` becoming `prop?: T \| undefined`, because every caller legitimately passes `T \| undefined` and the property really is optional; each was audited by hand after three separate scripted-pass bugs (an inner-union placed inside a nested object type, an unparenthesised function type, an `>`-as-bracket-closer mis-scan) were each caught only by reading the diff, not by the type checker. A smaller set were **not** widenings, because the sites were real bugs the flag correctly refused to paper over: `AdminResultsPage.tsx` set an error field to `undefined` to clear it — changed to `delete`, since a present-but-undefined key and an absent key are different states for a `Partial<Record<...>>`; `useMatchCentreBackNavigation.ts` built a react-router `To` with an optionally-undefined `search`/`hash` — changed to a conditional spread that omits the key entirely rather than forcing a library type to admit a shape it does not; framer-motion's `motion.*` props (`animate`, `layoutId`, `whileHover`, `style`) took the same conditional-spread treatment for the same reason — the library's `HTMLMotionProps` cannot be widened, so `undefined`-valued props are omitted instead of passed. **What remains open:** `noUncheckedIndexedAccess` is still off. Its cost was measured, not estimated, at the time this row was last written: **158** errors, 77 in the `import.meta.env.DEV`-gated `src/dev` harnesses no production bundle contains, so a real surface of 81, including 37 in `src/domain` where an unchecked `array[i]` is a scoring or elimination question rather than a cosmetic one — that count has not been re-measured since. **Required closure:** carry `noUncheckedIndexedAccess` to zero and turn it on, with `src/domain` taken deliberately rather than swept and a genuinely-safe access asserted with a real guard instead of a non-null assertion — a silenced check that reads as a proven one is the exact defect class this audit was hunting. Suppression by `@ts-expect-error`, `as any` or a widened model does not count as closure. |
