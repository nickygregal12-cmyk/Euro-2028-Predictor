...
The repository is at **contract 104** through `20260805010000_cup_split_group_tables.sql`. Development Supabase is hosted at contract 103, applied 4 August 2026 by fast-lane run 30959460638 on `6dc7ae3`, which carried contracts 98–103 in one batch; production remains at 63. Contract 102 persists the Predictor Championship split as a distinct phase while preserving the tournament Cup path; contract 103 makes a competition instance repeatable behind one live public row per season game, one live row per series and season/game-scoped lineage; contract 104 derives the continuing split table from the initial and split fixtures together, so carried points are read rather than copied. The repository contract and the hosted contracts are distinct facts. Any hosted schema mutation requires the guarded rollout workflow, explicit owner approval and the applicable preflight.
