...
The repository is at **contract 81** through `20260804093000_season_card_status.sql`. Development Supabase is hosted at contract 80, applied 4 August 2026 by fast-lane run 30889736356 on `3e82cca`, which reported `Development is at contract 80.`; production remains at 63. The repository contract and the hosted contracts are distinct facts. Any hosted schema mutation requires the guarded rollout workflow, explicit owner approval and the applicable preflight.
