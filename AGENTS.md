...
Repository, development Supabase and non-production Netlify are aligned at contract `54`; production database and application are intentionally aligned and locked at contract `48` (the H2H rank-history milestone, released 28 July 2026).

- canonical repository migration history contains exactly 54 versions through `20260729010000_predictor_cup_foundation.sql` (48 = `20260728122500_h2h_rank_history`, 49–54 = the Bonus Games chain);
- development Supabase is `iouzoutneyjpugbbtdem` and records the same 54 canonical versions;
- production Supabase is `vkfnsqdyhvtwyqkisxhk` and holds 48 canonical versions through `20260728122500_h2h_rank_history.sql` per the dated contract-48 release record;
- Netlify `dev`, `branch-deploy` and `deploy-preview` use development Supabase and declare `EURO28_DEPLOYED_DB_CONTRACT=54`;
- Netlify `production` declares contract 48 and uses production Supabase;
...
Contract compatibility does not make the product tournament-ready. Operating caps and Stage 3C2 scale/surface evidence are complete. Secure co-member player profiles are delivered through contract 47; contract 48 delivers richer H2H rank history and bracket health (Stage 4B, production-released); contracts 49–54 add the Bonus Games platform, the first two games and the Cup foundation (ADR-0010 B1–B7a: deny-all schema, hub with voluntary entry, shared knockout prediction store, KO Predictor scoring/standings, tournament-format Last Man Standing per `docs/scoring-rules.md` §8, and the audited Cup draw); the next product batch is the remaining Stage 4 core experience (Match Centre/tournament states, account/privacy/contact-admin, post-lock trends).
