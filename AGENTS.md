...
- the repository is contract **68**;
- development Supabase is hosted at contract **67**, so the repository leads it by one additive migration;
...
The repository is at **contract 68** through `20260803193000_season_fixtures.sql`, merged on `main`. Development Supabase is hosted at contract 67; production remains at 63. The repository contract and the hosted contracts are distinct facts. Any hosted schema mutation requires the guarded rollout workflow, explicit owner approval and the applicable preflight.
