...
The repository is at **contract 78** through `20260804053000_cup_league_schedule.sql`, merged on `main`. Development Supabase is hosted at contract 77; production remains at 63. The repository contract and the hosted contracts are distinct facts. Any hosted schema mutation requires the guarded rollout workflow, explicit owner approval and the applicable preflight.
