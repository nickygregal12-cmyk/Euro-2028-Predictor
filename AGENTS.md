...
- the repository is contract **72**;
- development Supabase is hosted at contract **71**, so the repository leads it by one additive migration;
...
The repository is at **contract 72** through `20260803233000_lms_persistence.sql`, merged on `main`. Development Supabase is hosted at contract 71; production remains at 63. The repository contract and the hosted contracts are distinct facts. Any hosted schema mutation requires the guarded rollout workflow, explicit owner approval and the applicable preflight.
