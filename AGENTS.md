...
- the repository is contract **71**;
- development Supabase is hosted at contract **70**, so the repository leads it by one additive migration;
...
The repository is at **contract 71** through `20260803223000_lms_pick_resolution.sql`, merged on `main`. Development Supabase is hosted at contract 70; production remains at 63. The repository contract and the hosted contracts are distinct facts. Any hosted schema mutation requires the guarded rollout workflow, explicit owner approval and the applicable preflight.
