...
The repository candidate is at **contract 81** through `20260804080000_provider_ingestion_custody.sql`. Development Supabase is verified at contract 80; production remains at 63. Contract 81 adds server-only provider custody and strict decoder evidence only: it does not deploy the Edge Function, configure a credential, call a provider or authorise official fixture/result writes. After merge, apply the additive migration to Development through `.github/workflows/development-fast-lane-rollout.yml`; production has no fast lane.
