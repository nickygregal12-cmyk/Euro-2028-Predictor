...
Repository, development Supabase and non-production Netlify are aligned at contract `53`; production database and application are intentionally aligned and locked at contract `48` (the H2H rank-history milestone, released 28 July 2026).

- canonical repository migration history contains exactly 53 versions through `20260728230000_last_man_standing.sql` (48 = `20260728122500_h2h_rank_history`, 49–53 = the Bonus Games chain);
- development Supabase is `iouzoutneyjpugbbtdem` and records the same 53 canonical versions;
...
- Netlify `dev`, `branch-deploy` and `deploy-preview` use development Supabase and declare `EURO28_DEPLOYED_DB_CONTRACT=53`;
...
Contract compatibility does not make the product tournament-ready. Operating caps and Stage 3C2 scale/surface evidence are complete. Secure co-member player profiles are delivered through contract 47; contract 48 delivers richer H2H rank history and bracket health (Stage 4B, production-released); contracts 49–53 add the Bonus Games platform and the first two games (ADR-0010 B1–B6: deny-all schema, hub with voluntary entry, shared knockout prediction store, KO Predictor scoring/standings and tournament-format Last Man Standing per `docs/scoring-rules.md` §8); the next product batch is the remaining Stage 4 core experience (Match Centre/tournament states, account/privacy/contact-admin, post-lock trends).
