...
The repository candidate is at **contract 94** through `20260804223000_provider_ingestion_custody.sql`. Development Supabase is hosted at contract 91, applied 4 August 2026 by fast-lane run 30916033941 on `a0af06d`, which reported `Development is at contract 91.`; production remains at 63. Contracts 92–94 are additive and may be applied together through the ADR-0024 fast lane after contract 94 merges. Repository inclusion does not deploy `provider-poll`, configure a credential, call a provider or authorise official fixture/result writes.
