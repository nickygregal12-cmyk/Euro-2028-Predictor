...
The repository is at **contract 93** through `20260804213000_season_matchweek_scoring_job.sql`. Development Supabase is hosted at contract 91, applied 4 August 2026 by fast-lane run 30916033941 on `a0af06d`, which reported `Development is at contract 91.`; production remains at 63. The repository contract and the hosted contracts are distinct facts. Any hosted schema mutation requires the guarded rollout workflow, explicit owner approval and the applicable preflight.
