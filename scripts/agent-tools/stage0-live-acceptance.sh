...
2. Netlify: read config/netlify-sites.json, select the canonical production `hub` or `euro` identity, and call only `netlify-public_current-production-deploy` to resolve actual current deploy evidence for at least the intended current site. Report only the bounded fields returned by that tool. Do not use the retired site, an arbitrary site ID/host, or any environment/deploy mutation surface.
...
4. Sentry: inspect the exact Sentry tools available to this role. A harmless inspect/read must work. A write-shaped tool such as update_issue must be ABSENT from the role's available tools or mechanically denied before invocation. Do not attempt a real mutation. Report exact relevant visible tool names. If the repository has no exact non-wildcard Sentry grants because the live inventory is still unproven, report LIMITATION; tool absence alone is not acceptance.
5. PostHog: inspect the exact PostHog tools available to this role. A harmless schema/read operation must work. A write-shaped command such as agent-feedback must be ABSENT from the role's available tools or mechanically denied before invocation. Do not send feedback or invoke a write. Do not inspect real-person replay/event payloads. Report exact relevant visible tool names. If the repository has no exact non-wildcard PostHog grants because the live inventory is still unproven, report LIMITATION; tool absence alone is not acceptance.
