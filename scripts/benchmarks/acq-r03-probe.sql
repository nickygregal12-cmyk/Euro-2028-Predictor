...
begin
  ...
  -- 1. Real auth users. Triggers are suspended FIRST: auth.users carries a
  --    handle_new_user trigger that creates a profile row automatically, which
  --    would collide with the explicit profiles insert below. Suspending makes
  --    the seed deterministic rather than dependent on trigger order.
  ...
  -- 2. Profiles, entries, predictions.
  insert into public.profiles (id, display_name)
  select ('acb3acb3-0000-0000-0001-' || lpad(s::text, 12, '0'))::uuid,
         'R03 Probe ' || s
  from generate_series(1, 5) as s;

  insert into public.entries (id, user_id, tournament_id, submitted_at)
  select ('acb3acb3-0000-0000-0002-' || lpad(s::text, 12, '0'))::uuid,
         ('acb3acb3-0000-0000-0001-' || lpad(s::text, 12, '0'))::uuid,
         v_tournament, now() - interval '2 hours'
  from generate_series(1, 5) as s;

  insert into public.match_predictions (entry_id, match_id, home_score, away_score)
  select ('acb3acb3-0000-0000-0002-' || lpad(s::text, 12, '0'))::uuid,
         m.id, (s + m.matchday) % 4, (s * 2 + m.matchday) % 3
  from generate_series(1, 5) as s
  cross join (
