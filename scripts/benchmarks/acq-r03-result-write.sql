...
begin
  ...
  -- Triggers must already be suspended when this runs: auth.users carries a
  -- handle_new_user trigger that auto-creates a profile, which would collide
  -- with the explicit profiles insert below.
...
-- Triggers suspended: deleting predictions on a locked tournament trips
-- enforce_entry_lock_generic, which would abort the transaction.
set local session_replication_role = replica;
