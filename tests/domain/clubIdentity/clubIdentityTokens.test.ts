 ...
 *
 * So the tests below spend most of their effort on the *unlisted* club and the
 * degraded inputs, because those are the common path in production and the rare
 * path in review.
 */
...
describe('parseClubColours', () => {
  it('preserves order, because the first colour is the shirt', () => {
    ...
    // Reversed input must give the reversed result, or "order preserved" is
    // untested — both colours appearing would satisfy a weaker assertion.
  ...
  it('drops an unknown colour name rather than guessing a shade', () => {
    ...
    // Whatever it recognises, it never invents. A guessed shade is worse than
    // a neutral fallback because it looks deliberate.
...
describe('deriveMonogram', () => {
  ...
  it('drops noise words that carry no distinguishing information', () => {
    // 'Arsenal FC' and 'Arsenal' must agree, or the same club gets two marks
    // depending on which provider field arrived.
  ...
  it('returns a placeholder rather than throwing when a name yields nothing', () => {
    // A fixture list must not break because one club name is punctuation.
...
describe('resolveClubName', () => {
  it('prefers the provider short name', () => {
    // 'Wolves', not 'Wolverhampton Wanderers FC' — and a screen reader
    // repeating the registered name on every row of twenty is worse, not
    // better.
...
describe('resolveClubIdentity — precedence', () => {
  it('lets the overlay win over the provider', () => {
    ...
    // The overlay supplies the one thing no provider does: nothing in
    // "Black / White" says Newcastle plays in stripes.
...
describe('resolveClubIdentity — the unlisted club is the common case', () => {
  it('renders a club with no overlay entry as solid, in its provider colour', () => {
    // This is what makes a promoted club work on day one. If this ever needs an
    // overlay entry to look right, the design has quietly become a roster.
    const resolved = resolveClubIdentity(
  ...
  it('omits secondary entirely when the pattern is solid', () => {
    // Not `undefined` by accident — the token object should not claim to know a
    // second colour it has no use for.
  ...
  it('leaves secondary undefined when an overlay pattern has no resolvable second colour', () => {
    // The degenerate case that reaches the component: a pattern is set but the
    // provider gave one colour. `ClubIdentity` falls back to solid, and this
    // keeps the token object honest rather than inventing a colour to match.
...
describe('the overlay stays an overlay', () => {
  it('is small enough to be a set of judgements rather than a roster', () => {
    // The guard on scope. Each entry is a per-club design decision — whether
    // Arsenal is solid red or red with white sleeves is a judgement, not a
    // lookup — so growth here is a signal the model has drifted into
    // maintaining a club table that goes stale every summer.
    expect(Object.keys(clubOverlay).length).toBeLessThanOrEqual(12)
  })

  it('only carries what no provider supplies', () => {
    // Providers give colours and codes. They do not give kit pattern, and they
    // do not say a gold shirt needs a dark monogram. Every entry must justify
    // itself by one of those two.
