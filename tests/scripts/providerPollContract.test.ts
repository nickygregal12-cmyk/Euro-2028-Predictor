...
const migrationSource = readFileSync(
  'supabase/migrations/20260803230000_provider_ingestion_custody.sql',
  'utf8',
)
...
describe('provider poll contract', () => {
  ...
  it('rejects credential-shaped query parameters instead of silently rewriting them', () => {
    expect(edgeSource).toContain('SENSITIVE_URL_QUERY_KEY')
    expect(edgeSource).toContain(
      "throw new Error('Provider URL must not contain credential-shaped query parameters.')",
    )
    expect(edgeSource).not.toMatch(/delete\s+url\.searchParams/i)
  })
