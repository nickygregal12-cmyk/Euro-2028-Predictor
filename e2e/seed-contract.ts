 ...
 *
 * Historical verification through contract 131 is retained in git history and
 * in the contract-specific pgTAP/database-parity suites. Contract 132 adds the
 * provider initial-fixture approval boundary: its public entry point is an
 * administrator-only RPC and its canonical projection helpers are internal and
 * revoked from browser roles. It does not tighten or replace any existing
 * authenticated read used by the deterministic Euro seed. Exact-head Database
 * parity and the authenticated Browser E2E suite both passed against contract
 * 132 before this marker was raised, providing direct seeded-session evidence.
 */
